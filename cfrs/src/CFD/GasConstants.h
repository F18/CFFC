...
/* Coefficients to find Viscosity */
...
inline double mu_gottlieb(double c1, double c2, double c3, double c4, double T) {
  return c1*pow(T,1.5)/(c2 + pow(T,c3) + c4/T);
}

// JJG's kappa correlation.
inline double kappa_gottlieb(double c1, double c2, double c3, double c4, 
			     double T, double g, double cp) {
  return mu_gottlieb(c1,c2,c3,c4,T)*cp/Pr(g);  
}
