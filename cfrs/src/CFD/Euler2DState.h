 ...
 *     Mr       -- Reference Mach number used in        *
 *                 Weiss-Smith low-Mach-number local    *
 *                 preconditioner.                      *
 ...
 *     lambda_precon_WS -- Return x-direction           *
 *                 eigenvalue(s) of Weiss-Smith         *
 ...
 *                 (y-direction).                       *
 *     rp_precon_WS -- Return primitive right           *
 *                 eigenvector of Weiss-Smith           *
 *                 preconditioned system (x-direction). *
 ...
 *                 (y-direction).                       *
 *     rc_precon_WS -- Return conservative right        *
 *                 eigenvector of Weiss-Smith           *
 *                 preconditioned system (x-direction). *
 ...
 *                 (y-direction).                       *
 *     lp_precon_WS -- Return primitive left            *
 *                 eigenvector of Weiss-Smith           *
 *                 preconditioned system (x-direction). *
 *     S        -- Return axisymmetric source term      *
 *                 vector.                              *
 *     dUdW     -- Return Jacobian of conserved         *
 *                 solution variables wrt primitive     *
 *                 solution variables.                  *
 *     dWdU     -- Return Jacobian of primitive         *
 *                 solution variables wrt conserved     *
 *                 solution variables.                  *
 *     P_U_WS   -- Weiss-Smith preconditioner for the   *
 *                 conservative solution variables.     *
 *     P_U_WS_inv -- Inverse of Weiss-Smith             *
 *                 preconditioner for the conservative  *
 *                 solution variables.                  *
 *                                                      *
 * Member operators                                     *
...
class Euler2D_pState{
  ...
  public:
    ...
    /* Reference Mach number used by Weiss-Smith
    ...
    /* Primitive right eigenvector (y-direction). */
   ...
   /* Conserved right eigenvector (y-direction). */
    ...
    /* Weiss-Smith low-Mach-number local preconditioner. */
    ...
    friend Euler2D_pState &operator +=(Euler2D_pState &W1, const Euler2D_pState &W2);
    friend Euler2D_pState &operator -=(Euler2D_pState &W1, const Euler2D_pState &W2);
...
class Euler2D_cState{
  ...
  public:
    ...
    static double   gm1i;   // 1/(g-1)
    static double      R;   // Gas constant.
    static double Mr_min;   // Gas constant.
	                    // Made public so can access them.
    ...
    /* Shortcut arithmetic operators. */
    friend Euler2D_cState &operator +=(Euler2D_cState &U1, const Euler2D_cState &U2);
    friend Euler2D_cState &operator -=(Euler2D_cState &U1, const Euler2D_cState &U2);
 ...
 ********************************************************/
inline Euler2D_pState &operator +=(Euler2D_pState &W1, const Euler2D_pState &W2) {
  W1.d += W2.d;
  W1.v += W2.v;
  W1.p += W2.p;
  return (W1);
 ...
 ********************************************************/
inline Euler2D_cState &operator +=(Euler2D_cState &U1, const Euler2D_cState &U2) {
  U1.d += U2.d;
  U1.dv += U2.dv;
  U1.E += U2.E;
  return (U1);
...
extern Euler2D_pState RinglebFlow(const Vector2D X);
