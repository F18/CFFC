...
namespace tut
{
  ...
  /*Test2: ComputationalCell:: operator <<*/
  template<>
  template<>
  void ComputationalCell_TestObject::test<2>()
  {
   GaussianComputationalCell2DType G_CompCell2D_Test;
   G_CompCell2D_Test = G_CompCell2D;
   //   std::cout << G_CompCell2D_Test << endl;
  ...
  void ComputationalCell_TestObject::test<4>()
  {
    ...
    GaussianComputationalCell2DType G_CompCell2D_Test;
    G_CompCell2D_Test.SetCell(Cell,SubgridPoints,RecOrder);
    //    cout << G_CompCell2D_Test << endl;

    CompCell3D.GenerateCell(Nx,Ny,Nz,RecOrder);
    //    std::cout << CompCell3D << endl;
  }
  ...
  void ComputationalCell_TestObject::test<6>()
  {
    ...
    for (int i=0; i<=Memb-1; ++i){
      //      CartesianCoeff.GeomCoeff()[]
      //      std::cout << CartesianCoeff[i] << std::endl;
    }
  }

  /*Test7: ComputationalCell::ComputeReconstructionError() */
  template<>
  template<>
  void ComputationalCell_TestObject::test<7>()
  {
    Nx = 4; Ny = 6;
    RecOrder = 3;
    digits = numeric_limits<double>::digits10;
    ...
    ComputationalCell<TwoD,Cell2D_Quad,double> CompCell2D_Test;

    CompCell2D_Test.SetCell(SubgridPoints,RecOrder);
    ...
    CompCell2D_Test.CellGeometry().setnodes(Node1,Node4,Node3,Node2);
    CompCell2D_Test.SetSubGridGeometry();
    CompCell2D_Test.ComputeGeometricCoefficients();

    CompCell2D_Test.CellSolution() = 11.5;

    for(int i=CompCell2D_Test.CellDeriv().FirstElem(); i<=CompCell2D_Test.CellDeriv().LastElem(); ++i){
      CompCell2D_Test.CellDeriv(i,true,true,true).D() = i*0.24 + 1.23;
    }

    CompCell2D_Test.UpdateSubgridSolution();
    // Obs. There is no function to access the subgrid and therefore I cannot use the "ensure" method
    //      Visual inspection of the coordinates was used instead 

    FunctionType2D Ptr_Funct = Test_Default2D;

    CompCell2D_Test.ComputeReconstructionError(Ptr_Funct);
    //    cout <<  "The Computational Cell" << endl << CompCell2D_Test << endl;

    ensure_equals("Access field", CompCell2D_Test.CellDeriv(1,0), 2.19 );
    ensure_equals("Access field", CompCell2D_Test.CellDeriv(1,0,1), CompCell2D_Test.CellDeriv(1,0));


    Vector2D Point1, Point2;

    // north face
    CompCell2D_Test.CellGeometry().GaussQuadPointsFaceN(Point1,Point2);
    Print(Point1);
    Print(Point2);

    // south face
    CompCell2D_Test.CellGeometry().GaussQuadPointsFaceS(Point1,Point2);
    Print(Point1);
    Print(Point2);

    // east face
    CompCell2D_Test.CellGeometry().GaussQuadPointsFaceE(Point1,Point2);
    Print(Point1);
    Print(Point2);

    // west face
    CompCell2D_Test.CellGeometry().GaussQuadPointsFaceW(Point1,Point2);
    Print(Point1);
    Print(Point2);

    // Output the subgrid nodes in Tecplot format
    // Set Path
    ...
    strcat(output_file_name,"ComputationalCell_Nodes_Tecplot.dat");
    output_file.open(output_file_name_ptr, ios::out);
    if (output_file.bad()) std::cout << "Problems\n";

    CompCell2D_Test.OutputSubgridTecplot(output_file,1,1,true);
    CompCell2D_Test.OutputSubgridTecplot(output_file,2,2,false);
    output_file.close();


    // access geometric coefficients
  }
...
namespace tut
{
  ComputationalCell_TestGroup ComputationalCell_Test("ComputationalCell_Test");
}
