...
namespace tut{
  ...
  const char* const TestData::Global_Output_Path = "UnitTesting/Results_Tests/";
  const char* const TestData::Global_Input_Path = "UnitTesting/InputFiles_Tests/";

  void TestData::check_global_paths(void){
    DIR *dp;
    std::string FailureMsg;

    if ((dp = opendir(Global_Output_Path)) == NULL){
      FailureMsg = "Critical ERROR: Cannot open directory: " + std::string(Global_Output_Path);
      FailureMsg += "\n\t       Most likely you are NOT running the executable under the '/src' directory!";
    ...
    if ((dp = opendir(Global_Input_Path)) == NULL){
      FailureMsg = "Critical ERROR: Cannot open directory: " + std::string(Global_Input_Path);
      FailureMsg += "\n\t       Most likely you are NOT running the executable under the '/src' directory!";
      throw tut::bad_ctor(FailureMsg);
    } else {
      // close the directory stream
  ...
  void TestData::set_local_output_path(char * dir_name){ 
    ...
    Path = std::string(Global_Output_Path) + std::string(dir_name) + "/";
  ...
  void TestData::set_local_input_path(char * dir_name){
    ...
    Path = std::string(Global_Input_Path) + std::string(dir_name) + "/";
								  ...
								  tut::failure) 
  {
	...
	File1 = new char [strlen(Global_Output_Path) + strlen(_CurrentFile_) + 1];
	strcpy(File1,Global_Output_Path);
	strcat(File1,_CurrentFile_);

	File2 = new char [strlen(Global_Output_Path) + strlen(_MasterFile_) + 1];
	strcpy(File2,Global_Output_Path);
	strcat(File2,_MasterFile_);
      } // endif
