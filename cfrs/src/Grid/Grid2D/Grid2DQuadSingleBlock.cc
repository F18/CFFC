...
void Set_BCs(Grid2D_Quad_Block &Grid) {
    ...
    if (Grid.BndNorthSpline.np == 0) {
       ...
       // Update BC for the rest of ghost cells
...
void Update_Exterior_Nodes(Grid2D_Quad_Block &Grid) {
    ...
    for ( j = Grid.JNl-Grid.Nghost ; j <= Grid.JNl ; ++j) {
    ...
    for ( j = Grid.JNu ; j <= Grid.JNu+Grid.Nghost ; ++j) {
...
void Update_Corner_Ghost_Nodes(Grid2D_Quad_Block &Grid) {
  ...
  if (Grid.BCtypeS[Grid.INu+1] == BC_NONE &&
	...
	X_norm = ((Grid.Node[Grid.INu-ng][j].X - 
		   Grid.Node[Grid.INu][j].X)*norm_dir)*norm_dir;
	X_tan = (Grid.Node[Grid.INu-ng][j].X - 
		 Grid.Node[Grid.INu][j].X) - X_norm;
	Grid.Node[Grid.INu+ng][j].X = Grid.Node[Grid.INu][j].X -
  	                              X_norm + X_tan;
      }
    }
...
void Rotate_Quad_Block(Grid2D_Quad_Block &Grid,
    ...
    if (Grid.BndNorthSpline.np != 0 ) 
       Scale_Spline(Grid.BndNorthSpline, Angle);
    if (Grid.BndSouthSpline.np != 0 ) 
       Scale_Spline(Grid.BndSouthSpline, Angle);
    if (Grid.BndEastSpline.np != 0 ) 
       Scale_Spline(Grid.BndEastSpline, Angle);
    if (Grid.BndWestSpline.np != 0 )
       Scale_Spline(Grid.BndWestSpline, Angle);
...
/********************************************************
 * Routine: Unsmooth_Interior_Nodes                     *
 *                                                      *
 * Disturbs randomly the interior nodes for the         *
 ...
 ********************************************************/
void Unsmooth_Interior_Nodes(Grid2D_Quad_Block &Grid,
			     const int Number_of_Iterations) {
  ...
  for (int num_iter=1; num_iter<=Number_of_Iterations; ++num_iter){
	...
	MinDistance = MinimumNodeEdgeDistance(Grid,i,j);

	// Generate a random angle for the displacement direction
	Angle = 2.0*PI*drand48();

	// Calculate the displacement -> 25% of the MinDistance
	Displacement = 0.25 * MinDistance;

	// Calculate the new node location
	Grid.Node[i][j].setloc(Grid.Node[i][j].x() + Displacement*cos(Angle),
			       Grid.Node[i][j].y() + Displacement*sin(Angle)); 
      }	// endfor (i)
    } // endfor (j)
  } //endfor (num_iter)
...
/**********************************************************
 * Routine: MinimumNodeEdgeDistance                       *
 *                                                        *
 * Determines the minimum distance between the Node[i][j] *
 * and all the neighbour edges, for a quadrilateral mesh. *
 *********************************************************/
double MinimumNodeEdgeDistance(Grid2D_Quad_Block &Grid, const int &i, const int &j) {
  ...
  // Edge 1
  MinDistance = DistanceFromPointToLine(Grid.Node[i][j].X, Grid.Node[i-1][j-1].X, Grid.Node[i][j-1].X);
  // Edge 2
  MinDistance = min(MinDistance, DistanceFromPointToLine(Grid.Node[i][j].X, Grid.Node[i][j-1].X, Grid.Node[i+1][j-1].X));
  // Edge 3
  MinDistance = min(MinDistance, DistanceFromPointToLine(Grid.Node[i][j].X, Grid.Node[i+1][j-1].X, Grid.Node[i+1][j].X));
  // Edge 4
  MinDistance = min(MinDistance, DistanceFromPointToLine(Grid.Node[i][j].X, Grid.Node[i+1][j].X, Grid.Node[i+1][j+1].X));
  // Edge 5
  MinDistance = min(MinDistance, DistanceFromPointToLine(Grid.Node[i][j].X, Grid.Node[i+1][j+1].X, Grid.Node[i][j+1].X));
  // Edge 6
  MinDistance = min(MinDistance, DistanceFromPointToLine(Grid.Node[i][j].X, Grid.Node[i][j+1].X, Grid.Node[i-1][j+1].X));
  // Edge 7
  MinDistance = min(MinDistance, DistanceFromPointToLine(Grid.Node[i][j].X, Grid.Node[i-1][j+1].X, Grid.Node[i-1][j].X));
  ...
  // Diagonal 1
  MinDistance = min(MinDistance, DistanceFromPointToLine(Grid.Node[i][j].X, Grid.Node[i][j-1].X, Grid.Node[i+1][j].X));
  // Diagonal 2
  MinDistance = min(MinDistance, DistanceFromPointToLine(Grid.Node[i][j].X, Grid.Node[i+1][j].X, Grid.Node[i][j+1].X));
  // Diagonal 3
  MinDistance = min(MinDistance, DistanceFromPointToLine(Grid.Node[i][j].X, Grid.Node[i][j+1].X, Grid.Node[i-1][j].X));
  // Diagonal 4
  MinDistance = min(MinDistance, DistanceFromPointToLine(Grid.Node[i][j].X, Grid.Node[i-1][j].X, Grid.Node[i][j-1].X));
 ...
 *                                                        *
 * Determines the distance between the Point and the line *
 * defined by the two points, LinePoint1 and LinePoint2.  *
 *********************************************************/
double DistanceFromPointToLine(const Vector2D &Point, const Vector2D &LinePoint1, const Vector2D &LinePoint2){
