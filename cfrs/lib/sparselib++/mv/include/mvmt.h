...
{                                                                      
    ...
    public:                                                            
        ...
        /*::::::::::::::::::::::::::*/                                 
            ...
            MV_ColMat_$TYPE();                             
            MV_ColMat_$TYPE(unsigned int, unsigned int); 

    // some compilers have difficulty with inlined 'for' statements.
    MV_ColMat_$TYPE(unsigned int, unsigned int, const $TYPE&);   

    // usual copy by value
    // (can't use default parameter lda=m, because m is not a constant...)
    //
    MV_ColMat_$TYPE($TYPE*, unsigned int m, unsigned int n);
    MV_ColMat_$TYPE($TYPE*, unsigned int m, unsigned int n, unsigned int lda);

    // the "reference" versions
    //
    //
    MV_ColMat_$TYPE(MV_ColMat_$TYPE &A, MV_Matrix_::ref_type i);
    MV_ColMat_$TYPE($TYPE*, unsigned int m, unsigned int n, MV_Matrix_::ref_type i);
    MV_ColMat_$TYPE($TYPE*, unsigned int m, unsigned int n, unsigned int lda,
    ...
    MV_ColMat_$TYPE&        newsize(unsigned int, unsigned int);
    ...
    friend ostream& operator<<(ostream &s, const MV_ColMat_$TYPE &A);

};                                                                     

inline $TYPE& MV_ColMat_$TYPE::operator()(unsigned int i, unsigned int j)
{
#ifdef MV_MATRIX_BOUNDS_CHECK
    assert(0<=i && i<dim(0));
    assert(0<=j && j<dim(1));
...
inline MV_ColMat_$TYPE::MV_ColMat_$TYPE($TYPE* d, unsigned int m, 
...
inline MV_ColMat_$TYPE::MV_ColMat_$TYPE($TYPE* d, unsigned int m, unsigned int n,
