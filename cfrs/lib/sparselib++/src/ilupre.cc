...
CompCol_ILUPreconditioner(const CompCol_MatDouble &A)
: l_colptr_(A.dim(1) + 1), u_colptr_(A.dim(1) + 1), 
  l_rowind_(0), u_rowind_(0),
  l_val_(0), u_val_(0)
{
  int i, j, k, pn, qn, rn;
  double multiplier;
  ...
  for (i = 0; i < dim_[0] - 1; i++) {
    ...
    for (j = u_colptr_[i+1]; j < u_colptr_[i+2]-1; j++) {
      ...
      for (pn = l_colptr_[u_rowind_[j]]; 
       l_rowind_[pn] <= i + 1 && pn < l_colptr_[u_rowind_[j]+1]; 
       pn++) {
    while (u_rowind_[qn] < l_rowind_[pn] && qn < u_colptr_[i+2])
      qn++;
    if (l_rowind_[pn] == u_rowind_[qn] && qn < u_colptr_[i+2])
      u_val_[qn] -= multiplier * l_val_[pn];
      }
      for (; pn < l_colptr_[u_rowind_[j]+1]; pn++) {
    while (l_rowind_[rn] < l_rowind_[pn] && rn < l_colptr_[i+2])
      rn++;
    if (l_rowind_[pn] == l_rowind_[rn] && rn < l_colptr_[i+2])
...
CompRow_ILUPreconditioner(const CompRow_MatDouble &A)
: l_rowptr_(A.dim(1) + 1), u_rowptr_(A.dim(1) + 1), 
  l_colind_(0), u_colind_(0),
  l_val_(0), u_val_(0)
{
  int i, j, k, pn, qn, rn;
  double multiplier;
  ...
  for (i = 1; i < dim_[0]; i++) {
    for (j = l_rowptr_[i]; j < l_rowptr_[i+1]; j++) {
      ...
      for (pn++; u_colind_[pn] < i && pn < u_rowptr_[l_colind_[j]+1]; pn++) {
    while (l_colind_[qn] < u_colind_[pn] && qn < l_rowptr_[i+1])
      qn++;
    if (u_colind_[pn] == l_colind_[qn] && qn < l_rowptr_[i+1])
      l_val_[qn] -= multiplier * u_val_[pn];
      }
      for (; pn < u_rowptr_[l_colind_[j]+1]; pn++) {
    while (u_colind_[rn] < u_colind_[pn] && rn < u_rowptr_[i+1])
      rn++;
    if (u_colind_[pn] == u_colind_[rn] && rn < u_rowptr_[i+1])
...
MV_Vector<double>
CompCol_ILUPreconditioner::solve(const MV_Vector<double> &x) const 
{
  int M = x.size();
  MV_Vector<double> y(M);
...
MV_Vector<double>
CompCol_ILUPreconditioner::trans_solve(const MV_Vector<double> &x) const 
{
  int M = x.size();
  MV_Vector<double> y(M);
  MV_Vector<double> work(M);
...
MV_Vector<double>
CompRow_ILUPreconditioner::solve(const MV_Vector<double> &x) const 
{
  int M = x.size();
  MV_Vector<double> y(M);
  MV_Vector<double> work(M);
...
MV_Vector<double>
CompRow_ILUPreconditioner::trans_solve(const MV_Vector<double> &x) const 
{
  int M = x.size();
  MV_Vector<double> y(M);
  MV_Vector<double> work(M);
