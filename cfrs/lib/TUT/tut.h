 ...
 */
template <typename T>
void ensure_fail(const T msg, bool cond)
{
    if ( cond )
    {
        throw failure(msg);
...
{
  /* Added by L. Ivan, UTIAS, April 08, 2007 */
    if (expected != actual)
    {
        std::stringstream ss;
        ss << std::setprecision(16) 
...
void ensure_distance(const char* msg, const T& actual, const T& expected,
		     const T& distance)
{
  /* Modified by L. Ivan, UTIAS, March 08, 2007 */
  if ( !((actual >= expected-distance) && (actual <= expected+distance)) )
    {
      std::stringstream ss;
      ss << std::setprecision(16)
	 << (msg ? msg : "") 
...
template <class T>
void ensure_distance(const std::string & msg, const T& actual, const T& expected,
		     const T& distance)
{
  /* Added by L. Ivan, UTIAS, April 08, 2007 */
  if ( !((actual >= expected-distance) && (actual <= expected+distance)) )
