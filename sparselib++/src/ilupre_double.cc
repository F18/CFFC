...
: l_val_(0), l_rowptr_(0), l_colind_(0), l_nz_(0),
    u_val_(0), u_rowptr_(0), u_colind_(0), u_nz_(0)
{
        dim_[0] = 0;
        dim_[1] = 0;
}
...
CompRow_Mat_double 
CompRow_ILUPreconditioner_double::l(void) const
{
  int i, j, n;
  VECTOR_double new_val;
  VECTOR_int    new_rowptr;
  VECTOR_int    new_colind;
  int new_nz;
  n = dim_[0];
  new_nz = l_nz_ + n;
  new_val = VECTOR_double(new_nz);
  new_colind = VECTOR_int(new_nz);
  new_rowptr = VECTOR_int(n+1);
  j = 0;
  for ( i = 0; i < n ; ++i) {
     new_rowptr(i) = j;
     if (l_rowptr_[i+1]-l_rowptr_[i] > 0) {
        new_colind(MV_VecIndex(j, j+l_rowptr_[i+1]-l_rowptr_[i]-1)) = 
           l_colind_(MV_VecIndex(l_rowptr_[i], l_rowptr_[i+1]-1));
        new_val(MV_VecIndex(j, j+l_rowptr_[i+1]-l_rowptr_[i]-1)) = 
           l_val_(MV_VecIndex(l_rowptr_[i], l_rowptr_[i+1]-1));
        j = j + l_rowptr_[i+1] - l_rowptr_[i];
     } /* endif */
     new_colind(j) = i;
     new_val(j) = 1.00;
     j = j + 1;
  ...
  new_rowptr(n) = new_nz;
  return(CompRow_Mat_double(dim_[0], dim_[1], new_nz, 
                            new_val, new_rowptr, new_colind));
}
  
CompRow_Mat_double 
CompRow_ILUPreconditioner_double::u(void) const
{
  return(CompRow_Mat_double(dim_[0], dim_[1], u_nz_, 
                            u_val_, u_rowptr_, u_colind_));
}
