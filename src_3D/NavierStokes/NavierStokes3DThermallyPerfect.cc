 ...
 *******************************************************************
 **********                                               **********
 ******       NavierStokes3DThermallyPerfect Version 1.00    *******
 ****                       (07/19/07)                          ****
 ****                                                           ****
 ****    Solver for Navier Stokes Equations Governing Flow of   ****
...
#include "NavierStokes3DThermallyPerfect.h"
#include "../UnitTesting/UnitTesting.h"

/* Begin NavierStokes3DThermallyPerfect program. */
...
int main(int num_arg, char *arg_ptr[]) {
   ...
   char *program_title_ptr = 
     "NavierStokes3DThermallyPerfect: Solver for the Navier Stokes Equations Governing Thermally Perfect Gases in 3 Space Dimensions.";
   ...
   // Input file name:
   char *Input_File_Name_ptr = "navierstokes3D.in";
   ...
   /* Parse and interpret command line arguments.  Note that there
      ...
      the standard input data file "navierstokes3D.in". */
   ...
   if (error_flag) {
     cout << "\n NavierStokes3DThermallyPerfect ERROR: Invalid command line argument.\n";
     return (error_flag);
   } /* endif */
   ...
   ********************************************************/
   ...
   if (CFFC_Primary_MPI_Processor() && help_flag) {
      cout << "Usage:\n";
      cout << "navierstokes3Dthermallyperfect [-v] [-h] [-i] [-b] [-f name] [-t name number]\n";
      ...
      cout << " -f name  use `name' input data file (`navierstokes3D.in' is default)\n";
      cout << " -t              run all available test suites\n" 
	   ...
	   << "                 a particular test-number \n"
 	   << "                 example: navierstokes3Dthermallyperfect -t MyTestSuit 3\n"
   ...
   error_flag = HexaSolver<NavierStokes3D_ThermallyPerfect_pState, 
   ...
   if (CFFC_Primary_MPI_Processor() && !batch_flag) 
      cout << "\n\nNavierStokes3DThermallyPerfect: Execution complete.\n";

   //Ending properly
   return (0);
   
/* End NavierStokes3DThermallyPerfect program. */
