 ...
 *******************************************************************
 **********                                               **********
 ******          NavierStokes3DPolytropic Version 1.00       *******
 ****                       (07/19/07)                          ****
 ****                                                           ****
...
#include "NavierStokes3DPolytropic.h"
#include "../UnitTesting/UnitTesting.h"

/* Begin NavierStokes3DPolytropic program. */
...
int main(int num_arg, char *arg_ptr[]) {
   ...
   char *program_title_ptr = 
     "NavierStokes3DPolytropic: Solver for the Navier Stokes Equations Governing Polytropic in 3 Space Dimensions.";
   ...
   if (error_flag) {
     cout << "\n NavierStokes3DPolytropic ERROR: Invalid command line argument.\n";
     return (error_flag);
   } /* endif */
   ...
   ********************************************************/
   ...
   if (CFFC_Primary_MPI_Processor() && help_flag) {
      cout << "Usage:\n";
      cout << "navierstokes3Dpolytropic [-v] [-h] [-i] [-b] [-f name] [-t name number]\n";
      ...
      cout << " -t              run all available test suites\n" 
	   ...
	   << "                 a particular test-number \n"
 	   << "                 example: navierstokes3Dpolytropic -t MyTestSuit 3\n"
   ...
   error_flag = HexaSolver<NavierStokes3D_Polytropic_pState, 
   ...
   if (CFFC_Primary_MPI_Processor() && !batch_flag) 
      cout << "\n\nNavierStokes3DPolytropic: Execution complete.\n";

   //Ending properly
   return (0);
   
/* End NavierStokes3DPolytropic program. */
