...
inline double Omega11(const double &Ts) {
  ...
  if ( Ts<0.3 || Ts>100.0 ) {
    cerr << endl 
         << "NASARP1311data::Omega11() - Warning: Reduced temperature T*= " 
	 << Ts << " outside applicable range (0.3 < T* < 100).";
    //exit(0);
  }
...
inline double dOmega22dT(const double &T, const double &eps) {
  ...
  if ( Ts<0.3 || Ts>100.0 ) {
    cerr << endl
         << "Warning: Reduced temperature T*= " << Ts 
	 << " outside applicable range (0.3 < T* < 100).";
    //exit(0);
  }
