...
template<>
int Hexa_Block<LES3D_Polytropic_pState, LES3D_Polytropic_cState>::
dUdt_Residual_HighOrder(Input_Parameters<LES3D_Polytropic_pState,
                        LES3D_Polytropic_cState> &IPs,
    ...
    LES3D_Polytropic_pState Wl, Wr, W_face;
    LES3D_Polytropic_pState dWdxL, dWdyL, dWdxR, dWdyR, dWdx_face, dWdy_face, dWdz_face; // Solution gradients at the inter-cellular face
    LES3D_Polytropic_cState Flux, FaceFlux, SourceTerm(0);
    ...
    HighOrderVariable.ComputeUnlimitedSolutionReconstruction(*this, 
                                                             &Hexa_Block<LES3D_Polytropic_pState, 
        ...
        for ( j = JCl-1 ; j <= JCu ; ++j ){
            for ( i = ICl-1 ; i <= ICu ; ++i ) {
                ...
                for (GQPoint = 0; GQPoint < NumGQP; ++GQPoint) { // for each Gauss Quadrature point
                    ...
                    Flux += GaussQuadWeights[GQPoint] * LES3D_Polytropic_pState::FluxViscous_HighOrder_n(W_face, 
                                                                                                         dWdx_face, 
                                                                                                         dWdy_face, 
                                                                                                         dWdz_face,
                                                                                                         Grid.Cell[i][j][k].V,
