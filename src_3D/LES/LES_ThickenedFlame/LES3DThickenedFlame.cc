 ...
 *******************************************************************
 **********                                               **********
 ******          LES3DThickenedFlame Version 1.00            *******
 ****                       (10/19/07)                          ****
 ****                                                           ****
...
/* Begin LES3DThickenedFlame program. */
...
int main(int num_arg, char *arg_ptr[]) {
   ...
   char *program_title_ptr = 
     "LES3DThickenedFlame: Solver for performing LES of Turbulence Premixed Combustion Processes Using Thickened Flame Approach.";
   ...
   // Input file name:
   char *Input_File_Name_ptr = "les3Dtf.in";
   ...
   /* Parse and interpret command line arguments.  Note that there
      ...
      the standard input data file "les3Dtf.in". */
   ...
   if (error_flag) {
     cout << "\n LES3DThickenedFlame ERROR: Invalid command line argument.\n";
     return (error_flag);
   } /* endif */
   ...
   ********************************************************/
   ...
   if (CFFC_Primary_MPI_Processor() && help_flag) {
      cout << "Usage:\n";
      cout << "les3Dtf [-v] [-h] [-i] [-b] [-f name] [-t name number]\n";
      ...
      cout << " -f name  use `name' input data file (`les3Dtf.in' is default)\n";
      cout << " -t              run all available test suites\n" 
	   ...
	   << "                 a particular test-number \n"
 	   << "                 example: les3Dtf -t MyTestSuit 3\n"
   ...
   error_flag = HexaSolver<LES3DTF_pState, 
                           LES3DTF_cState>
   ...
   if (CFFC_Primary_MPI_Processor() && !batch_flag) 
      cout << "\n\nLES3DThickenedFlame: Execution complete.\n\n";

   //Ending properly
   return (0);
   
   /* End LES3DThickenedFlame program. */
