 ...
 *******************************************************************
 **********                                               **********
 ******          Euler3DThermallyPerfect Version 1.00        *******
 ****                       (07/19/07)                          ****
 ****                                                           ****
 ****        Solver for Euler Equations Governing Flow of       ****
...
#include "Euler3DThermallyPerfect.h"
#include "../UnitTesting/UnitTesting.h"

/* Begin Euler3DThermallyPerfect program. */
...
int main(int num_arg, char *arg_ptr[]) {
   ...
   char *program_title_ptr = 
     "Euler3DThermallyPerfect: Solver for the Euler Equations Governing Thermally Perfect Gases in 3 Space Dimensions.";
   ...
   // Input file name:
   char *Input_File_Name_ptr = "euler3D.in";
   ...
   if (error_flag) {
     cout << "\n Euler3DThermallyPerfect ERROR: Invalid command line argument.\n";
     return (error_flag);
   } /* endif */
   ...
   ********************************************************/
   ...
   if (CFFC_Primary_MPI_Processor() && help_flag) {
      cout << "Usage:\n";
      cout << "euler3Dthermallyperfect [-v] [-h] [-i] [-b] [-f name] [-t name number]\n";
      ...
      cout << " -f name  use `name' input data file (`euler3D.in' is default)\n";
      cout << " -t              run all available test suites\n" 
	   ...
	   << "                 a particular test-number \n"
 	   << "                 example: euler3Dthermallyperfect -t MyTestSuit 3\n";
      cout.flush();
   } /* endif */
   ...
   error_flag = HexaSolver<Euler3D_ThermallyPerfect_pState, 
   ...
   if (CFFC_Primary_MPI_Processor() && !batch_flag) 
      cout << "\n\nEuler3DThermallyPerfect: Execution complete.\n";

   //Ending properly
   return (0);
   
/* End Euler3DThermallyPerfect program. */
