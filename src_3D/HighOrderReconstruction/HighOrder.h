/*!\file HighOrder.h
  \brief Header file implementing the templated HighOrder class.*/
 ...
 * \class HighOrder
 *
 * \brief Template class for high-order variables in 3D.
 * \nosubgrouping
 *********************************************************/
template<class SOLN_STATE>
class HighOrder{
...
public: 
  ...
  //! Get the container of Taylor derivatives for cell (ii,jj,kk) of the 3D block.
  const DerivativesContainer & CellTaylorDeriv(const int &ii, const int &jj, const int &kk) const {return TD[ii][jj][kk];}
  //! Get the container of Taylor derivatives for cell (ii,jj,kk) of the 3D block.
  DerivativesContainer & CellTaylorDeriv(const int &ii, const int &jj, const int &kk) {return TD[ii][jj][kk];}
  //! Get the solution state of the Taylor derivative of cell (ii,jj,kk) for the (p1,p2,p3) powers.
  const Soln_State & CellTaylorDerivState(const int &ii, const int &jj, const int &kk,
					  const int & p1, const int & p2, const int & p3) const {return TD[ii][jj][kk](p1,p2,p3);}
  //! Get the solution state of the Taylor derivative of cell (ii,jj,kk) for the (p1,p2,p3) powers.
  Soln_State & CellTaylorDerivState(const int &ii, const int &jj, const int &kk,
				    const int & p1, const int & p2, const int & p3) {return TD[ii][jj][kk](p1,p2,p3);}
  //! Get the value of Taylor derivative of cell (ii,jj,kk) for the (p1,p2,p3) powers and the specified 'Variable'.
  const double & CellTaylorDerivValue(const int & ii, const int & jj, const int & kk,
				      const int & p1, const int & p2, const int & p3, const int & Variable) const {
    return TD[ii][jj][kk](p1,p2,p3)[Variable];
  }
  //! Get the value of Taylor derivative of cell (ii,jj,kk) for the (p1,p2,p3) powers and the specified Variable.
  double & CellTaylorDerivValue(const int & ii, const int & jj, const int & kk,
				const int & p1, const int & p2, const int & p3, const int & Variable) { return TD[ii][jj][kk](p1,p2,p3)[Variable];}
  //! Get the Taylor derivative of cell (ii,jj,kk) which is stored in the 'position' place (i.e. powers and value).
  Derivative & CellTaylorDeriv(const int & ii, const int & jj, const int & kk,
			       const int & position) {return TD[ii][jj][kk](position);}
  //! Get the Taylor derivative of cell (ii,jj,kk) which is stored in the 'position' place (i.e. powers and value).
  const Derivative & CellTaylorDeriv(const int & ii, const int & jj ,const int & kk,
				     const int & position) const {return TD[ii][jj][kk](position);}
  //! Get the solution state of the Taylor derivative of cell (ii,jj,kk) which is stored in the 'position' place.
  Soln_State & CellTaylorDerivState(const int & ii, const int & jj, const int & kk,
				    const int & position) {return TD[ii][jj][kk](position).D();}
  //! Get the solution state of the Taylor derivative of cell (ii,jj,kk) which is stored in the 'position' place.
  const Soln_State & CellTaylorDerivState(const int & ii, const int & jj, const int & kk,
					  const int & position) const {return TD[ii][jj][kk](position).D();}
  //! Get the number of Taylor derivatives in each cell container
  const int NumberOfTaylorDerivatives(void) const {return TD[ICl][JCl][KCl].size();}
  //@} 

  //! @name Geometric moments
  //@{
  //! Get the geometric coefficients of cell (ii,jj,kk).
  const GeometricMoments & CellGeomCoeff(const int & ii, const int & jj, const int & kk) const { return Geom->CellGeomCoeff(ii,jj,kk); }
  //! Get the geometric coefficients of cell (ii,jj,kk) which is stored in the 'position' place.
  const GeomMoment & CellGeomCoeff(const int & ii, const int & jj, const int & kk,
				   const int & position) {return Geom->CellGeomCoeff(ii,jj,kk,position);}
  //! Get the value of cell (ii,jj,kk) geometric coefficient with x-power 'p1', y-power 'p2' and z-power 'p3'.
  const double & CellGeomCoeffValue(const int & ii, const int & jj, const int & kk,
				    const int & p1, const int & p2, const int & p3) {return Geom->CellGeomCoeffValue(ii,jj,kk,p1,p2,p3);}
  //! Get the cell (ii,jj,kk) geometric coefficient which is store in the 'position' place (i.e. powers and value)
  const double & CellGeomCoeffValue(const int & ii, const int & jj, const int & kk,
				    const int & position) {return Geom->CellGeomCoeffValue(ii,jj,kk,position);}
  //@}
  ...
  //! Get the monotonicity flags for reconstruction of cell (ii,jj,kk)
  const FlagType & CellInadequateFit(const int & ii, const int & jj, const int & kk) const { return LimitedCell[ii][jj][kk];}
  //! Get the monotonicity flags for reconstruction of cell (ii,jj,kk)
  FlagType & CellInadequateFit(const int & ii, const int & jj, const int & kk){ return LimitedCell[ii][jj][kk];}
  /*!
   * Get the monotonicity flag for reconstruction of cell (ii,jj,kk) and the variable stored in the position 'VarPosition'.
   * It is assumed that 'VarPosition' starts from ONE!
   */
  const int & CellInadequateFitValue(const int & ii, const int & jj, const int & kk,
				     const int VarPosition) const { return LimitedCell[ii][jj][kk][VarPosition-1];}
  //! Get the monotonicity flag for reconstruction of cell (ii,jj,kk) and the variable stored in the position 'VarPosition'.
  int & CellInadequateFitValue(const int & ii, const int & jj, const int & kk,
			       const int VarPosition){ return LimitedCell[ii][jj][kk][VarPosition-1];}
  const int & Previous_CellInadequateFitValue(const int & ii, const int & jj, const int & kk,
					      const int VarPosition) const { return PreviousLimitedCell[ii][jj][kk][VarPosition-1];}
  int & Previous_CellInadequateFitValue(const int & ii, const int & jj, const int & kk, 
					const int VarPosition){ return PreviousLimitedCell[ii][jj][kk][VarPosition-1];}
  //@}
  ...
  //! Get the smoothness indicators for reconstruction of cell (ii,jj,kk)
  const DoubleArrayType & CellSmoothnessIndicator(const int & ii, const int & jj, const int & kk) const { return SI[ii][jj][kk];}
  //! Get the smoothness indicators for reconstruction of cell (ii,jj,kk) 
  DoubleArrayType & CellSmoothnessIndicator(const int & ii, const int & jj, const int & kk){ return SI[ii][jj][kk];}
  //! Get the smoothness indicators for reconstruction of cell (ii,jj,kk) and the variable stored in the position 'VarPosition'.
  const double & CellSmoothnessIndicatorValue(const int & ii, const int & jj, const int & kk,
					      const int & VarPosition) const { return SI[ii][jj][kk][VarPosition-1];}
  //! Get the smoothness indicators for reconstruction of cell (ii,jj,kk) and the variable stored in the position 'VarPosition'.
  double & CellSmoothnessIndicatorValue(const int & ii, const int & jj, const int & kk,
					const int & VarPosition){ return SI[ii][jj][kk][VarPosition-1];}
  //@}
  ...
  DenseMatrix & Cell_LHS_Inv(const int & ii, const int & jj, const int & kk) {return CENO_LHS[ii][jj][kk];}
  //! Get the pseudo-inverse matrix for the reconstruction of cell (ii,jj,kk)
  ...
  //! Get the array of geometric weights for cell (ii,jj,kk)
  DoubleArrayType & GeomWeights(const int & ii, const int & jj, const int & kk){return CENO_Geometric_Weights[ii][jj][kk];}
  //! Get the array of geometric weights for cell (ii,jj,kk)
  const DoubleArrayType & GeomWeights(const int & ii, const int & jj, const int & kk) const {return CENO_Geometric_Weights[ii][jj][kk];}
  //! Get the geometric weight for the cell with the index 'CellPosition' which is part of the stencil of cell (ii,jj,kk) 
  double & GeomWeightValue(const int & ii, const int & jj, const int & kk,
			   const int & CellPosition){return CENO_Geometric_Weights[ii][jj][kk][CellPosition];}
  //! Get the geometric weight for the cell with the index 'CellPosition' which is part of the stencil of cell (ii,jj,kk) 
  const double & GeomWeightValue(const int & ii, const int & jj, const int & kk,
				 const int & CellPosition) const {return CENO_Geometric_Weights[ii][jj][kk][CellPosition];}
  //@}
  ...
  GeometryType & CellGeometry(const int & ii, const int & jj, const int & kk){return Geom->Cell[ii][jj][kk];}
  //! Get the cell (ii,jj,kk) of the associated block mesh
...
//  // --> RR: comment out ConstrainedReconstruction flags on boundaries
...
//  const bool IsSouthConstrainedReconstructionRequired(void) const { return SouthBnd.IsReconstructionConstrained(); }
  ...
  //! @name Block boundary information --> RR: no boundaries yet
  //@{
...
//  const HighOrder2D_BlockBoundary & get_West_SouthBnd(void) const { return W_SouthBnd; }
  ...
  void InitializeMonotonicityVariables(const int & ii, const int & jj, const int & kk);
  ...
  //@{
  //! Evaluate the interpolant at a given location (X_Coord,Y_Coord,Z_Coord) for a specified solution variable (i.e. parameter),
  //  using the reconstruction of cell (ii,jj,kk)
  double SolutionAtCoordinates(const int & ii, const int & jj, const int & kk,
			       const double & X_Coord, const double & Y_Coord, const double & Z_Coord, const unsigned & parameter) const {
    return TD[ii][jj][kk].ComputeSolutionFor(X_Coord - XCellCenter(ii,jj,kk), Y_Coord - YCellCenter(ii,jj,kk), Z_Coord - ZCellCenter(ii,jj,kk))[parameter];
  }
  //! Evaluate the interpolant at a given position vector for a specified solution variable (i.e. parameter),
  //  using the reconstruction of cell (ii,jj,kk)
  double SolutionAtLocation(const int & ii, const int & jj, const int & kk,
			    const Vector3D &CalculationPoint, const unsigned & parameter) const {
    return SolutionAtCoordinates(ii,jj,kk,CalculationPoint.x,CalculationPoint.y,CalculationPoint.z,parameter);
  }
  
  //! Evaluate the interpolant at a given location (X_Coord,Y_Coord,Z_Coord) for all solution variables,
  //  using the reconstruction of cell (ii,jj,kk)
  Soln_State SolutionAtCoordinates(const int & ii, const int & jj, const int & kk,
  ...
  //! Evaluate the interpolant at a given location (X_Coord,Y_Coord,Z_Coord) for all solution variables,
  //  using the reconstruction of cell (ii,jj,kk)
  Soln_State SolutionStateAtCoordinates(const int & ii, const int & jj, const int & kk,
					const double & X_Coord, const double & Y_Coord, const double & Z_Coord) const {
    return TD[ii][jj][kk].ComputeSolutionFor(X_Coord - XCellCenter(ii,jj,kk), Y_Coord - YCellCenter(ii,jj,kk), Z_Coord - ZCellCenter(ii,jj,kk));
  }

  //! Evaluate the interpolant at a given position vector for all solution variables, using the reconstruction of cell (ii,jj,kk).
  Soln_State SolutionStateAtLocation(const int & ii, const int & jj, const int & kk, const Vector3D &CalculationPoint) const {
  ...
  //! Evaluate the pressure provided by the interpolant at a given location (X_Coord,Y_Coord,Z_Coord),
  //  using the reconstruction of cell (ii,jj,kk)
  double SolutionPressureAtCoordinates(const int & ii, const int & jj, const int & kk, 
				      const double & X_Coord, const double & Y_Coord, const double & Z_Coord) const {
    return SolutionStateAtCoordinates(ii,jj,kk,X_Coord,Y_Coord,Z_Coord).pressure();
  }

  // --> RR: comment out gradient calculation for now

//  //! Evaluate the gradient in the normal direction of the interpolant 
//  //  at given position coordinate for all solution variables, using the reconstruction of cell (ii,jj,kk).
//  Soln_State NormalGradientStateAtCoordinates(const int & ii, const int & jj, const int & kk,
//					      const double & X_Coord, const double & Y_Coord, const double & Z_Coord,
//					      const Vector3D &norm_dir) const;
//
//  //! Evaluate the gradient in the normal direction of the interpolant 
//  //  at given position coordinates for a specified solution variable (i.e. parameter), using the reconstruction of cell (ii,jj,kk).
//  double NormalGradientAtCoordinates(const int & ii, const int & jj, const int & kk,
//				     const double & X_Coord, const double & Y_Coord, const double & Z_Coord,
//				     const Vector3D &norm_dir, const unsigned & parameter) const;
//
//  //! Evaluate the gradient in the normal direction of the interpolant 
//  //  at a given position vector for all solution variables, using the reconstruction of cell (ii,jj,kk).
//  Soln_State NormalGradientStateAtLocation(const int & ii, const int & jj, const int & kk,
//					   const Vector3D &CalculationPoint, const Vector3D &norm_dir) const {
//    return NormalGradientStateAtCoordinates(ii,jj,kk,CalculationPoint.x,CalculationPoint.y,CalculationPoint.z,norm_dir);
//  }
//
//  //! Evaluate the gradient in the normal direction of the interpolant 
//  //  at a given position vector for a specified solution variable (i.e. parameter), using the reconstruction of cell (ii,jj,kk).
//  double NormalGradientAtLocation(const int & ii, const int & jj, const int & kk,
//				  const Vector3D &CalculationPoint, const Vector3D &norm_dir,
//				  const unsigned & parameter) const {
//    return NormalGradientAtCoordinates(ii,jj,kk,CalculationPoint.x,CalculationPoint.y,CalculationPoint.z,norm_dir,parameter);
//  }
  ...
  ReturnType IntegrateOverTheCell(const int &ii, const int &jj, const int & kk, const FO FuncObj,
				  const int & digits, ReturnType _dummy_param) const;

  
  /*! @brief Integrate the reconstructed polynomial of cell (ii,jj,kk) over an arbitrary quad domain. */
  template<typename Node3DType>
...
// --> RR: comment out access functions for ReconstructionTypeMap
//  char getCellReconstructionType(const int& iCell, const int& jCell, const int& kCell) const;
//  char** const getReconstructionTypeMap(void) const {return ReconstructionTypeMap; }
  ...
  void ComputeCellReconstructionPseudoInverseNearConstrainedBoundaries(const int& iCell, const int& jCell, const int& kCell);

  //! @brief Compute the piecewise linear solution reconstruction of cell (iCell,jCell,kCell). 
  template<class Soln_Block_Type>
  ...
  // --> RR: comment out Constrained reconstruction functions!

//  //! @brief Compute the constrained unlimited high-order solution reconstruction of cell (iCell,jCell,kCell). 
//  template<class Soln_Block_Type>
...
//							 const IndexType & i_index, const IndexType & j_index, const IndexType & k_index);
//
//  //! @brief Compute the individually constrained unlimited high-order solution reconstruction of cell (iCell,jCell,kCell).
//  template<class Soln_Block_Type>
...
//  //@}
  ...
  //@{
  friend ostream & operator<< <Soln_State> (ostream & os, const HighOrder<Soln_State> & Obj);
  friend istream & operator>> <Soln_State> (istream & os, HighOrder<Soln_State> & Obj);
  //@}
...
//  unsigned int ObserverCornerGhostCellGeometryState; //!< Observer to memorize the state of the corner ghost cells.
  ...
  //@{
  GeometryType* Geom;          //!< Pointer to the associated geometry which is a 3D mesh block
  ...
  // --> RR: HighOrder2D_BlockBoundary declarations
...
HighOrder<SOLN_STATE> & HighOrder<SOLN_STATE>::operator=(const HighOrder<SOLN_STATE> & rhs){
  ...
  if (rhs._allocated_block){
    ...
    if (_si_calculation != rhs._si_calculation){
      deallocate();
      throw runtime_error("HighOrder<SOLN_STATE>::operator=() ERROR! The object cannot be assigned due to incompatibilities between the CENO_Execution_Mode class settings and the object settings");
    }
...
void HighOrder<SOLN_STATE>::allocate(const int &NC_IDir,
  ...
  if (CENO_Execution_Mode::CENO_RECONSTRUCTION_WITH_MESSAGE_PASSING){
	...
	throw runtime_error("HighOrder<SOLN_STATE>::allocate() ERROR! Too few ghost cells provided for the required reconstruction!");
      };
...
template<class SOLN_STATE> inline
void HighOrder<SOLN_STATE>::SetReconstructionOrder(int ReconstructionOrder){
...
// */
//template<class SOLN_STATE> inline
//void HighOrder<SOLN_STATE>::CheckConsistencyOfGeometricSplineProperties(void){
//  // empty function
//}
 ...
 */
template<class SOLN_STATE> inline
void HighOrder<SOLN_STATE>::AssociateGeometry(GeometryType & Block){
 ...
 */
template<class SOLN_STATE> inline
int HighOrder<SOLN_STATE>::getNumberOfRings(int number_of_Taylor_derivatives){
 ...
 */
template<class SOLN_STATE> inline
int HighOrder<SOLN_STATE>::getMinimumNghost(const int &ReconstructionOrder){
 ...
 */
template<class SOLN_STATE> inline
void HighOrder<SOLN_STATE>::SetRings(void){
 ...
 */
template<class SOLN_STATE> inline
int HighOrder<SOLN_STATE>::getStencilSize(const int &ReconstructionOrder) {
  static int temp;
...
// */
//template<class SOLN_STATE> inline
//SOLN_STATE HighOrder<SOLN_STATE>::NormalGradientStateAtCoordinates(const int & ii, const int & jj,
...
// */
//template<class SOLN_STATE> inline
//double HighOrder<SOLN_STATE>::NormalGradientAtCoordinates(const int & ii, const int & jj,
//							    const double &X_Coord, const double &Y_Coord,
//							    const Vector3D &norm_dir, const unsigned & parameter) const {
//  
 ...
 */
template<class SOLN_STATE>
template<typename Node3DType> inline
SOLN_STATE HighOrder<SOLN_STATE>::
...
// */
//template<class SOLN_STATE> inline
//char HighOrder<SOLN_STATE>::getCellReconstructionType(const int& iCell, const int& jCell) const {
//  if (ReconstructionTypeMap != NULL){
 ...
 */
template<class SOLN_STATE>
void HighOrder<SOLN_STATE>::Read_Object(istream & in_file) {
  ...
  if (_si_calc_ != CENO_Execution_Mode::CENO_SMOOTHNESS_INDICATOR_COMPUTATION_WITH_ONLY_FIRST_NEIGHBOURS){
    throw runtime_error("HighOrder<SOLN_STATE>::Read_Object() ERROR! The object cannot be read due to incompatibilities between the CENO_Execution_Mode class settings and the object settings");
  }
 ...
 */
template<class SOLN_STATE>
void HighOrder<SOLN_STATE>::Broadcast_HighOrder_Data(GeometryType & Block_Geometry){
 ...
 */
template<class SOLN_STATE>
void HighOrder<SOLN_STATE>::Broadcast_HighOrder_Data(MPI::Intracomm &Communicator, 
						       const int &Source_CPU,
						       GeometryType & Block_Geometry){
...
// */
//template<class SOLN_STATE>
//void HighOrder<SOLN_STATE>::BuildReconstructionTypeMap(void){
//
//  int i,j;
//
...
template<> inline
int HighOrder<double>::NumberOfVariables(void) const {
  return 1;
}
