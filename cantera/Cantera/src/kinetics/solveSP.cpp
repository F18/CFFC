 ...
 */
/*
 * $Id: solveSP.cpp,v 1.2 2007/08/29 19:57:48 hkmoffa Exp $
 */
/*
...
#  define DAMPING true
			     ...
			     double PGas, double reltol, double abstol)
  {
	...
	printResJac(m_ioflag, m_neq, m_Jac, DATA_PTR(m_resid),
      ...
      if (info==0) {
	//(void) dgetrs_(&cflag, &m_neq, &nrhs, m_JacCol[0], &m_neq, 
	//	       DATA_PTR(m_ipiv), DATA_PTR(m_resid), &m_neq,
	//	       &info, 1);
        ct_dgetrs(ctlapack::NoTranspose, m_neq, nrhs, m_JacCol[0], m_neq, 
		  DATA_PTR(m_ipiv), DATA_PTR(m_resid), m_neq,
		  info);
      }
      /*
