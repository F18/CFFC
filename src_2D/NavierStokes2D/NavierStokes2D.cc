...
int main(int num_arg, char *arg_ptr[]) {
  ...
  char *program_title_ptr = 
     "NavierStokes2D: Stand alone solver for the Navier-Stokes Equations in 2 space dimensions.";
  ...
  // Input file name:
  char *Input_File_Name_ptr = "navierstokes2D.in";

  // Equation type indicator:
  char *Equation_Type_ptr = "NavierStokes2D";
  ...
  //                be solve to "type" (default is "NavierStokes2D"),
  // 6) -f name  uses "name" as the input data file rather than
  //             the standard input data file "navierstokes2D.in".
  ...
  if (error_flag) {
    cout << "\n NavierStokes2D ERROR: Invalid command line argument.\n";
    return error_flag;
  }
  ...
  if (CFFC_Primary_MPI_Processor() && help_flag) {
     cout << "Usage:\n";
     cout << "navierstokes2D [-v] [-h] [-i] [-b] [-f name] [-t name number]\n";
     ...
     cout << " -f name  use `name' input data file (`navierstokes2D.in' is default)\n";
     cout << " -t              run all available test suites\n" 
	  ...
	  << "                 example: navierstokes2D -t MyTestSuit 3\n";
     cout.flush();
  }
  if (help_flag) {
     CFFC_Finalize_MPI();
  ...
  if (CFFC_Primary_MPI_Processor() && !batch_flag) 
    cout << "\n\nNavierStokes2D: Execution complete.\n";

  //Ending properly
  return 0;

/* End NavierStoke2D program. */
