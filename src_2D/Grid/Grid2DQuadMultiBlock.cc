...
/********************************************************
 * Routine: Search_Multi_Block_Grid                     *
 *                                                      *
 * Search a multiblock grid for the block which contains*
 * the point 'Xpos' using bisection.                    *
 ********************************************************/
int Search_Multi_Block_Grid(const Grid2D_Quad_Block **Grid_ptr,
			    const int &Number_of_Blocks_Idir,
			    const int &Number_of_Blocks_Jdir,
			    const Vector2D Xpos,
			    int &i, int &j)
{
  ...
  // Set low and high points.  We store two opposite corners.
  int lo_i(0), lo_j(0);
  int hi_i(Number_of_Blocks_Idir-1) ,hi_j(Number_of_Blocks_Jdir-1);
  int mi_i, mi_j;

  // the direction of increasing ordered values
  Vector2D X_SW = Grid_ptr[lo_i][lo_j].Node[Grid_ptr[lo_i][lo_j].INl]
                                           [Grid_ptr[lo_i][lo_j].JNl].X;
  Vector2D X_NE = Grid_ptr[hi_i][hi_j].Node[Grid_ptr[hi_i][hi_j].INu]
                                           [Grid_ptr[hi_i][hi_j].JNu].X;
  Vector2D temp = X_NE - X_SW;
  bool ascnd_i = (temp.x >= 0); // true for x increasing in +i dir
  bool ascnd_j = (temp.y >= 0); // true for y increasing in +j dir


  // make sure it is bracketed
  if (Xpos.x<X_SW.x || Xpos.x>X_NE.x) return (-1);
  else if (Xpos.y<X_SW.y || Xpos.y>X_NE.y) return (-1);

  // 
  // loop over blocks
  //
  while (hi_i-lo_i>1 && hi_j-lo_j>1) {

      // compute the middle block
      mi_i = (lo_i + hi_i) / 2;
      mi_j = (lo_j + hi_j) / 2;

      // get the corners of the middle block
      X_SW = Grid_ptr[mi_i][mi_j].Node[Grid_ptr[mi_i][mi_j].INl]
	                              [Grid_ptr[mi_i][mi_j].JNl].X;
      X_NE = Grid_ptr[mi_i][mi_j].Node[Grid_ptr[mi_i][mi_j].INu]
                                      [Grid_ptr[mi_i][mi_j].JNu].X;

      //
      // test for location
      //
      // i-dir
      if ((Xpos.x<X_SW.x) == ascnd_i) 
	hi_i = mi_i-1;
      else if ((Xpos.x>X_NE.x) == ascnd_i) 
	lo_i = mi_i+1;
      else 
	hi_i = lo_i = mi_i;

      // j-dir
      if ((Xpos.y<X_SW.y) == ascnd_j) 
	hi_j = mi_j-1;
      else if ((Xpos.y>X_NE.y) == ascnd_j) 
	lo_j = mi_j+1;
      else
	hi_j = lo_j = mi_j;
