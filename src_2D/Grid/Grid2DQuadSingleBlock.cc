...
// Why return but then have further code? 
// -- Alistair Wood. Fri Jul 20 2007. 
//  
//    double s, t, Radius;
//    int NNi, NNj;
//  
...
//      NNi = (NRi-1)*int(pow(2.0,level))*(Grid.INu-Grid.INl);
//      NNj = NRj*int(pow(2.0,level))*(Grid.JNu-Grid.JNl);
//  
//      for (int j = Grid.JNl; j <= Grid.JNu; j++) {
//        t = StretchingFcn(double(j-Grid.JNl+(rj*int(pow(2.0,level))+dj)*(Grid.JNu-Grid.JNl))/double(NNj),
//  			Stretching_Factor_Jdir,ZERO,STRETCHING_FCN_MAX_CLUSTERING);
//        for (int i = Grid.INl; i <= Grid.INu; i++) {
//  	s = StretchingFcn(double(i-Grid.INl+(ri*int(pow(2.0,level))+di)*(Grid.INu-Grid.INl))/double(NNi),
//  			  Stretching_Factor_Idir,ZERO,STRETCHING_FCN_MIN_CLUSTERING);
//  	Grid.Node[i][j].X = Vector2D((s-ONE)*Length_Chamber,t*Radius_Chamber);
//        }
//      }
//  
//    } else {
//  
//      // In the nozzle.
//      Spline2D BndNorthSpline;
//      Create_Spline_Area_Variation(BndNorthSpline,
//  				 ZERO,
...
//  				 501);
//  
//      NNi = int(pow(2.0,level))*(Grid.INu-Grid.INl);
//      NNj = int(pow(2.0,level))*(Grid.JNu-Grid.JNl);
//  
//      for (int j = Grid.JNl; j <= Grid.JNu; j++) {
//        t = StretchingFcn(double(j-Grid.JNl+(rj*int(pow(2.0,level))+dj)*(Grid.JNu-Grid.JNl))/double(NNj),
//  			Stretching_Factor_Jdir,ZERO,STRETCHING_FCN_MAX_CLUSTERING);
//        for (int i = Grid.INl; i <= Grid.INu; i++) {
//  	s = StretchingFcn(double(i-Grid.INl+((ri-NRi+1)*int(pow(2.0,level))+di)*(Grid.INu-Grid.INl))/double(NNi),
//  			  Length_Chamber_To_Throat/Length_Nozzle,3.00,STRETCHING_FCN_MIDPT_CLUSTERING);
//  	Radius = getminY(Vector2D(s*Length_Nozzle,ZERO),BndNorthSpline).y;
//  	Grid.Node[i][j].X = Vector2D(s*Length_Nozzle,t*Radius);
//        }
//      }
//  
//      double d, l, cos_angle; 
//      Vector2D Xp, Xo, nhat_p, nhat_o;
//      int i_min, i_max, j_min, j_max;
//  
//      if (Grid.Node[Grid.INl][Grid.JNl].X.x > ZERO) i_min = Grid.INl;
//      else i_min = Grid.INl+1;
//  
//      if (Grid.BCtypeE[2] == BC_NONE) i_max = Grid.INu;
//      else i_max = Grid.INu-1;
//  
//      if (Grid.BCtypeS[2] == BC_NONE) j_min = Grid.JNl;
//      else j_min = Grid.JNl+1;
//  
//      if (Grid.BCtypeN[2] == BC_NONE) j_max = Grid.JNu;
//      else j_max = Grid.JNu-1;
//  
//      for (int i = i_min; i <= i_max; i++) {
//        Xo = Vector2D(Grid.Node[i][Grid.JNl].X.x,ZERO);
//        //Xp = getminY(Grid.Node[i][Grid.JNl].X,BndNorthSpline);
//        Xp = getminY(Xo,BndNorthSpline);
//        //nhat_p = getnormal(Grid.Node[i][Grid.JNl].X,BndNorthSpline);
//        //nhat_p = getnormal(Xp,BndNorthSpline);
//        nhat_p = getnormal(Xo,BndNorthSpline);
//        d = abs(Xo-Xp);
//        nhat_o = (Xo-Xp)/d;
//        cos_angle = dot(nhat_p,nhat_o);
//        l = d*cos_angle;
//        for (int j = j_max; j >= j_min; j--) {
//  	s = ONE - StretchingFcn(double(j-Grid.JNl+(rj*int(pow(2.0,level))+dj)*(Grid.JNu-Grid.JNl))/double(NNj),
//  				Stretching_Factor_Jdir,ZERO,STRETCHING_FCN_MAX_CLUSTERING);
//  	Grid.Node[i][j].X = Xp + nhat_p*l*(ONE-s)*s + (Xo-Xp)*s*s;
//        }
//      }
//  
...
//  cell (ci, cj). We would first find the value at node NE using the
...
//  the values at NW, NE, the centre of (ci, cj) and the centre of (ci,
//  cj+1).
//  
...
//  
//  In what follows, then, LL and RR are the weights used to obtain the
//  nodal values. If the cell only contributes to one of the nodal values
//  then one of LL or RR is zero. "intp" then is simply the average of LL
//  and RR.
//  
//  And what about for other methods of finding the face gradients?
//  Shouldn't we try to do the same thing here as in
//  dUdt_Residual_Evaluation()? Ahh, we're not that sophisticated yet.
//  
//  The "source" cell is to the "Orient_cell" of cell (Rii, Rjj).
//  
...
//  Orient_cell = East
//
//  Now we wish to know how the cell to the east of (Rii, Rjj) influences
//  the value on the north face of (Rii, Rjj). In this case, the cell to
...
//  face.  So LL is zero. Also the node to the right of the face is the NW
//  node of the cell to the east.
//
void 
node_weights(double *LL, double *RR, 
		...
		const Grid2D_Quad_Block &Grid)
{
   ...
   switch (Orient_face){
   case NORTH:
		 lnodei = Rii  ; lnodej = Rjj+1;

		 rnodei = Rii+1; rnodej = Rjj+1;

     if(Orient_cell == CENTER){    
       *LL = dWn_dWc(lnodei, lnodej, NORTH_WEST, Grid);
       *RR = dWn_dWc(rnodei, rnodej, NORTH_EAST, Grid);
     } else if(Orient_cell == NORTH){     
       *LL = dWn_dWc(lnodei, lnodej, SOUTH_WEST, Grid);
       *RR = dWn_dWc(rnodei, rnodej, SOUTH_EAST, Grid);
     } else if(Orient_cell == EAST){
       *LL = ZERO;
       *RR = dWn_dWc(rnodei, rnodej, NORTH_WEST, Grid);
     } else if(Orient_cell == WEST){ 
       *LL = dWn_dWc(lnodei, lnodej, NORTH_EAST, Grid);      
       *RR = ZERO;                   
     } else if(Orient_cell == NORTH_WEST){
       *LL = dWn_dWc(lnodei, lnodej, SOUTH_EAST, Grid);      
       *RR = ZERO;   
     } else if(Orient_cell == NORTH_EAST){
       *LL = ZERO;
       *RR = dWn_dWc(rnodei, rnodej, SOUTH_WEST, Grid);    
     }
     break;
      
   case EAST:
		 // Rotate your frame of reference -90 degrees.
		 lnodei = Rii+1; lnodej = Rjj+1;

		 rnodei = Rii+1; rnodej = Rjj  ;
   ...
   case SOUTH:
		 // Rotate your frame of reference -180 degrees.
		 lnodei = Rii+1; lnodej = Rjj;

		 rnodei = Rii  ; rnodej = Rjj;
   ...
   case WEST:
		 // Rotate your frame of reference -270 degrees.
		 lnodei = Rii; lnodej = Rjj  ;

		 rnodei = Rii; rnodej = Rjj+1;
...
//  dWn_dWc:
//
//  Derivative of node primitive solution with respect to 
//  the cell primitive solution.
//
//  Alternatively, the geometric weight used to find the node value
//  from a cell value.
//
double 
dWn_dWc(int ni, int nj, int Orient_cell, const Grid2D_Quad_Block &Grid)
{
 
  double eta = 0.0, zeta = 0.0;
  BilinearInterpolationCoefficients(&eta, &zeta, ni, nj, Grid);

  switch(Orient_cell) {
  case NORTH_WEST: return (eta - zeta*eta); break;
  case NORTH_EAST: return (ONE - zeta - eta + zeta*eta); break;
  case SOUTH_WEST: return (zeta*eta); break;
  case SOUTH_EAST: return (zeta - zeta*eta); break;
  default: cerr<<"\n Improper Orient in dWn_dWc\n"; break; 
	}

	return 0.0; // for compiler warnings.
}

//  BilinearInterpolationCoefficients:
//  
//  Returns in eta and zeta the coefficients of the bilinear interpolation
//  of Zingg and Yarrow (SIAM J. Sci. Stat. Comput. Vol. 13 No. 3 1992).
//
//  There is code duplication between this function and the templated
//  Bilinear_Interpolation() in CFD.h. An eager student could eliminate
//  the duplication and perhaps furthermore ensure that any interpolation
//  algorithms are only in CFD.h whereas grid-specific code is here.
//
void 
BilinearInterpolationCoefficients(double *eta, double *zeta, int ii, int jj,
		const Grid2D_Quad_Block &Grid)
{
...
//
void 
diamond_gradient_weights(double *d_dWdx_dW, double *d_dWdy_dW, 
		double LEFT, double RIGHT, 
		int Orient_face, int i, int j, int Orient_cell, 
		const Grid2D_Quad_Block &Grid)
{
  ...
  case EAST:
    ...
    norm[0].y = -(Grid.Cell[i+1][j].Xc.x - Grid.nodeSE(i,j).X.x);
    //  normal vector of the NE side of a diamond 
    norm[1].x = Grid.nodeNE(i,j).X.y -  Grid.Cell[i+1][j].Xc.y ;
    norm[1].y = -(Grid.nodeNE(i,j).X.x -  Grid.Cell[i+1][j].Xc.x );
    //  normal vector of the NW side of a diamond 
    norm[2].x =   Grid.Cell[i][j].Xc.y - Grid.nodeNE(i,j).X.y ;
    norm[2].y = -(Grid.Cell[i][j].Xc.x - Grid.nodeNE(i,j).X.x);
    //  normal vector of the SW side of a diamond 
    norm[3].x = Grid.nodeSE(i,j).X.y - Grid.Cell[i][j].Xc.y;
    norm[3].y = -(Grid.nodeSE(i,j).X.x - Grid.Cell[i][j].Xc.x);
  ...
  case SOUTH:
    ...
    norm[0].y = -(Grid.nodeSE(i,j).X.x - Grid.Cell[i][j-1].Xc.x  );
    //  normal vector of the NE side of a diamond 
    norm[1].x = Grid.Cell[i][j].Xc.y -  Grid.nodeSE(i,j).X.y;
    norm[1].y = -(Grid.Cell[i][j].Xc.x -  Grid.nodeSE(i,j).X.x);
    //  normal vector of the NW side of a diamond 
    norm[2].x =   Grid.nodeSW(i,j).X.y - Grid.Cell[i][j].Xc.y ;
    norm[2].y = -(Grid.nodeSW(i,j).X.x - Grid.Cell[i][j].Xc.x);
    //  normal vector of the SW side of a diamond 
    norm[3].x =  Grid.Cell[i][j-1].Xc.y - Grid.nodeSW(i,j).X.y;
    norm[3].y = -(Grid.Cell[i][j-1].Xc.x- Grid.nodeSW(i,j).X.x);
  ...
  case WEST:
    ...
    //  normal vector of the SE side of a diamond 
    norm[0].x =   Grid.Cell[i][j].Xc.y - Grid.nodeSW(i,j).X.y;
    norm[0].y = -(Grid.Cell[i][j].Xc.x - Grid.nodeSW(i,j).X.x);
    //  normal vector of the NE side of a diamond 
    norm[1].x =  Grid.nodeNW(i,j).X.y - Grid.Cell[i][j].Xc.y;
    norm[1].y = -(Grid.nodeNW(i,j).X.x - Grid.Cell[i][j].Xc.x);
    //  normal vector of the NW side of a diamond 
    norm[2].x =  Grid.Cell[i-1][j].Xc.y - Grid.nodeNW(i,j).X.y ;
    norm[2].y = -( Grid.Cell[i-1][j].Xc.x - Grid.nodeNW(i,j).X.x);
    //  normal vector of the SW side of a diamond 
    norm[3].x =  Grid.nodeSW(i,j).X.y - Grid.Cell[i-1][j].Xc.y;
    norm[3].y = -(Grid.nodeSW(i,j).X.x - Grid.Cell[i-1][j].Xc.x);
