...
/********************************************************
 * Routine: Search_Single_Block_Grid                    *
 *                                                      *
 * Search a single block grid for the cel which contains*
 * the point 'Xpos' using bisection.                    *
 ********************************************************/
int Search_Single_Block_Grid(const Grid2D_Quad_Block &Grid_ptr,
			     const Vector2D Xpos,
			     int &i, int &j)
{
  ...
  // Set low and high points.  We store two opposite corners of the block.
  int lo_i(Grid_ptr.ICl), lo_j(Grid_ptr.JCl);
  int hi_i(Grid_ptr.ICu) ,hi_j(Grid_ptr.JCu);
  int mi_i, mi_j;

  // the direction of increasing ordered values
  Vector2D X_SW = Grid_ptr.Node[Grid_ptr.INl][Grid_ptr.JNl].X;
  Vector2D X_NE = Grid_ptr.Node[Grid_ptr.INu][Grid_ptr.JNu].X;
  ...
  while (hi_i-lo_i>1 && hi_j-lo_j>1) {
      ...
      // compute the middle cell index
      mi_i = (lo_i + hi_i) / 2;
      mi_j = (lo_j + hi_j) / 2;

      // get the corners of the middle cell
      X_SW = Grid_ptr.Node[mi_i  ][mi_j  ].X;
      X_NE = Grid_ptr.Node[mi_i+1][mi_j+1].X;
