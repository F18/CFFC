 ...
 * Dusty2DQuadGrid.cc                                                 *
 *                                                                    *
 * Grid manipulation subroutines for 2D Dusty multi-block             *
...
/**********************************************************************
 * Dusty2D_Quad_Block -- Multiple Block External Subroutines for      *
 * Quadrilateral Mesh.                                                *
 **********************************************************************/
...
Grid2D_Quad_Block** Multi_Block_Grid(Grid2D_Quad_Block **Grid_ptr,
  ...
  case GRID_READ_FROM_DEFINITION_FILE :
    ...
    if (Grid_ptr == NULL) {
      cout << "\n " << CFFC_Name() 
	   << " Dusty2D ERROR: Unable to open multi-block mesh definition file "
  ...
  case GRID_READ_FROM_GRID_DATA_FILE :
    ...
    if (Grid_ptr == NULL) {
      cout << "\n " << CFFC_Name() 
	   << " Dusty2D ERROR: Unable to open multi-block mesh data file "
	   << IP.Grid_File_Name << ".\n";
    }
    if (IP.i_Grid == GRID_NASA_ROTOR_37) {
      IP.NASA_Rotor37.init(IP.Rotor_Flow_Type,IP.NASA_Rotor37_Data_Directory);
  ...
  case GRID_ROCKET_MOTOR :
    if (IP.Flow_Type == FLOWTYPE_INVISCID) IP.BC_North = BC_REFLECTION;
  ...
  if (abs(IP.X_Shift) > TOLER) Translate_Multi_Block_Grid(Grid_ptr,
							  IP.Number_of_Blocks_Idir,
							  IP.Number_of_Blocks_Jdir,
							  IP.X_Shift);

  // Next scale quadrilateral mesh as specified by input parameters.
  if (fabs(IP.X_Scale-ONE) > TOLER) Scale_Multi_Block_Grid(Grid_ptr,
							   IP.Number_of_Blocks_Idir,
							   IP.Number_of_Blocks_Jdir,
							   IP.X_Scale);

  // Finally rotate quadrilateral mesh as specified by input parameters.
  if (fabs(IP.X_Rotate) > TOLER) Rotate_Multi_Block_Grid(Grid_ptr,
							 IP.Number_of_Blocks_Idir,
							 IP.Number_of_Blocks_Jdir,
