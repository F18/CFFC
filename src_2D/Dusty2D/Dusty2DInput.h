/**********************************************************************
 * Dusty2DInput.h: Header file defining 2D Dusty input parameter      *
 *                 class.                                             *
 **********************************************************************/

#ifndef _DUSTY2D_INPUT_INCLUDED
#define _DUSTY2D_INPUT_INCLUDED
 ...
 *
 * @brief Definition and manipulation of dusty2D input variables.
 *
 */
class Dusty2D_Input_Parameters{
private:
public:
  ...
  char Time_Integration_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int i_Time_Integration;
  int Time_Accurate, Local_Time_Stepping, 
      Maximum_Number_of_Time_Steps, N_Stage;
  double CFL_Number, Time_Max, Particle_CFL_Number;
  //! Residual variable:
  ...
  char Reconstruction_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int i_Reconstruction;
  //@}

  //@{ @name Limiter type indicator and related input parameters:
  char Limiter_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int i_Limiter;
  int i_Particle_Phase_Limiter;
  int  Freeze_Limiter;
  double Freeze_Limiter_Residual_Level;
  //@}

  //@{ @name Flux function type and related input parameters:
  char Flux_Function_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int i_Flux_Function;
  char Particle_Flux_Function_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int i_Particle_Flux_Function;
  char Viscous_Reconstruction_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int i_Viscous_Reconstruction;
  //@}

  //@{ @name Initial condition type indicator and related input parameters:
  char ICs_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  char Gas_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  char Particle_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int i_ICs;
  Dusty2D_pState Wo, W1, W2;
  Dusty2D_cState Uo;
  double Pressure, Temperature, Mach_Number, Flow_Angle, Reynolds_Number, dp, Re_lid;
  Vector2D Wave_Position;
  double Wave_Width;
  int num_cmp_part;
  double alphap, Mp, Tp, thetap;
  char Drag_Law[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int i_drag_law;
  //@}

  //@{ @name Propellant parameters:
  char Propellant_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  //@}

  //@{ @name Particle-phase formulation flag:
  char Particle_Phase[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int Particles;
  //@}

  //@{ @name Phase-interaction flag;
  char Phase_Interaction[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int PhaseInteraction;
  //@}

  //@{ @name Flow type flag:
  char Flow_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int FlowType;
  //@}

  //@{ @name Flow geometry (planar or axisymmetric):
  char Flow_Geometry_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int Axisymmetric;
  //@}

  //@{ @name  Electrostatic forces:
  char Electrostatic_Force[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int Electrostatic;
  double Electric_Field_Strength, Electric_Field_Angle;
  //@}

  //@{ @name  Measure mass, momentum, and energy conservation flag:
  char Measure_Conservation[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int MeasureConservation;
  //@}

  //@{ @name Turbulence parameters:
  char Turbulence_BC_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int i_Turbulence_BCs;
  char Friction_Velocity_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  ...
  char Grid_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  char NACA_Aerofoil_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int i_Grid;
  ...
  char Boundary_Conditions_Specified[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int BCs_Specified;
  char BC_North_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  char BC_South_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  char BC_East_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  char BC_West_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int BC_North, BC_South, BC_East, BC_West;
  //@}

  //@{ @name NASA rotor input variables:
  char NASA_Rotor37_Data_Directory[INPUT_PARAMETER_LENGTH_DUSTY2D],
       NASA_Rotor67_Data_Directory[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int Rotor_Flow_Type;
  ...
  //! Gradient of the gas-phase density field refinement criteria.
  int Refinement_Criteria_Gradient_Gas_Density;
  //! Divergence of the gas-phase velocity field refinement criteria.
  int Refinement_Criteria_Divergence_Gas_Velocity;
  //! Curl of the gas-phase velocity field refinement criteria.
  int Refinement_Criteria_Curl_Gas_Velocity;
  //! Gradient of the particle-phase concentration field refinement criteria.
  int Refinement_Criteria_Gradient_Particle_Concentration;
  //! Divergence of the particle-phase velocity field refinement criteria.
  int Refinement_Criteria_Divergence_Particle_Velocity;
  //! Curl of the particle-phase velocity field refinement criteria.
  int Refinement_Criteria_Curl_Particle_Velocity;
  //! Maximum and minimum particle-phase concetration ratio.
  int Refinement_Criteria_Ratio_Particle_Concentration;
  ...
  //! Output file name:
  char Output_File_Name[INPUT_PARAMETER_LENGTH_DUSTY2D];
  //! Multi-block mesh definition input file names:
  char Grid_File_Name[INPUT_PARAMETER_LENGTH_DUSTY2D];
  char Grid_Definition_File_Name[INPUT_PARAMETER_LENGTH_DUSTY2D];
  //! Restart file name:
  char Restart_File_Name[INPUT_PARAMETER_LENGTH_DUSTY2D];
  //! Gnuplot file name:
  char Gnuplot_File_Name[INPUT_PARAMETER_LENGTH_DUSTY2D];
  //! Next_Control_Parameter:
  char Next_Control_Parameter[INPUT_PARAMETER_LENGTH_DUSTY2D];
  //! Output format type indicator:
  char Output_Format_Type[INPUT_PARAMETER_LENGTH_DUSTY2D];
  int i_Output_Format;
...
inline ostream &operator << (ostream &out_file,
  ...
  if (IP.i_Grid == GRID_CARTESIAN_UNIFORM) {
    ...
    out_file << "\n\n Solving 2D Dusty equations (IBVP/BVP) on multi-block solution-adaptive quadrilateral mesh.";
  }
  ...
  if (IP.FlowType == FLOWTYPE_INVISCID && IP.Particles == PARTICLE_PHASE_NONE) {
    out_file << "\n  -> 2D Euler equations (no particles)";   
  } else if (IP.FlowType == FLOWTYPE_INVISCID) {
    out_file << "\n  -> 2D Inviscid gas-particle equations";   
  } else if (IP.FlowType == FLOWTYPE_LAMINAR && IP.Particles == PARTICLE_PHASE_NONE) {
    out_file << "\n  -> 2D Laminar Navier-Stokes equations (no particles)";
  } else if (IP.FlowType == FLOWTYPE_LAMINAR) {
    out_file << "\n  -> 2D Laminar gas-particle equations";
  } else if (IP.FlowType == FLOWTYPE_TURBULENT_RANS_K_OMEGA && IP.Particles == PARTICLE_PHASE_NONE) {
    out_file << "\n  -> 2D Turbulent Navier-Stokes equations with the k-omega turbulence model (no particles)";
  } else if (IP.FlowType == FLOWTYPE_TURBULENT_RANS_K_OMEGA) {
    out_file << "\n  -> 2D Turbulent gas-particle equations with the k-omega turbulence model";
  } else {
    out_file << "\n  -> 2D Inviscid Flow";   
  }
  if (IP.FlowType == FLOWTYPE_TURBULENT_RANS_K_OMEGA) {
    out_file << "\n  -> Turbulence boundary conditions: " << IP.Turbulence_BC_Type;
  ...
  if (IP.Particles == PARTICLE_PHASE_EULERIAN_FORMULATION) {
    out_file << "\n  -> Eulerian formulation for the particle-phase";
  }
  if (IP.Particles != PARTICLE_PHASE_NONE) out_file << "\n  -> Particle drag law: " << IP.Drag_Law;
  if (IP.Phase_Interaction) out_file << "\n  -> Phase-interaction: " << IP.Phase_Interaction;
  if (!IP.Axisymmetric) out_file << "\n  -> 2D Planar Flow";
  else out_file << "\n  -> 2D Axisymmetric Flow";
  if (IP.Electrostatic) out_file << "\n  -> 2D Electrostatic forces are in effect";
  if (IP.MeasureConservation) out_file << "\n  -> Conservation properties are being measured";
  ...
  if (IP.Particles && IP.i_Particle_Phase_Limiter) out_file << "\n  -> Particle-phase limiter: " << OFF;
  ...
  out_file << "\n  -> Gas Phase Flux Function: " << IP.Flux_Function_Type;
  if (IP.Particles)
    out_file << "\n  -> Particle Phase Flux Function: " << IP.Particle_Flux_Function_Type;
  if (IP.FlowType)
  ...
  if (IP.Particles) {
    out_file << "\n  -> Particle Type: "
	     ...
	     << " cm = " << IP.Wo.cm << ","
	     << " qe = " << IP.Wo.qe << ","
	     << " tauv = " << IP.Wo.mp/(THREE*PI*IP.Wo.dp*IP.Wo.mu()) << ","
	     << " tauT = " << IP.Wo.mp*IP.Wo.cp/(TWO*PI*IP.Wo.dp*IP.Wo.kappa()) << ")";
  }
  if (IP.i_Grid == GRID_ROCKET_MOTOR) {
    ...
    out_file << "\n     -> Percent concentration of particles = " << IP.Wo.alphas;
  }
  switch(IP.i_ICs) {
  case IC_CONSTANT :
    out_file << "\n  -> Pressure (kPa): " 
  ...
  case GRID_BACKWARD_FACING_STEP :
    out_file << "\n  -> Inflow height: " << IP.Inflow_Height;
  ...
  out_file << "\n  -> CFL Number: "
	   << IP.CFL_Number;
  if (IP.Particles) out_file << "\n  -> Particle CFL Number: " 
			     << IP.Particle_CFL_Number;
  out_file << "\n  -> Maximum Time (ms): " 
...
inline istream &operator >> (istream &in_file,Dusty2D_Input_Parameters &IP) {
  return in_file;
}
...
extern void Open_Input_File(Dusty2D_Input_Parameters &IP);

extern void Close_Input_File(Dusty2D_Input_Parameters &IP);

extern void Set_Default_Input_Parameters(Dusty2D_Input_Parameters &IP);

extern void Broadcast_Input_Parameters(Dusty2D_Input_Parameters &IP);
...
extern void Get_Next_Input_Control_Parameter(Dusty2D_Input_Parameters &IP);

extern int Parse_Next_Input_Control_Parameter(Dusty2D_Input_Parameters &IP);

extern int Process_Input_Control_Parameter_File(Dusty2D_Input_Parameters &IP,
                                                char *Input_File_Name_ptr,
                                                int &Command_Flag);

extern void Initialize_Reference_State(Dusty2D_Input_Parameters &IP);

extern void Reinitialize_Reference_State(Dusty2D_Input_Parameters &IP);
