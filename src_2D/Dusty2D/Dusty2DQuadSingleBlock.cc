/**********************************************************************
 * Dusty2DQuadSingleBlock.cc                                          *
 *                                                                    *
 * Single-block versions of subroutines for 2D dusty multi-block      *
 ...
 *                                                                    *
 **********************************************************************/
void Broadcast_Solution_Block(Dusty2D_Quad_Block &SolnBlk) {

#ifdef _MPI_VERSION
  int ni, nj, ng, nv, nr, block_allocated, buffer_size;
  double *buffer;
  ...
  MPI::COMM_WORLD.Bcast(&nv,1,MPI::INT,0);
  ...
  if (!CFDkit_Primary_MPI_Processor()) {
    ...
    if (SolnBlk.NUM_VAR_DUSTY2D != nv) SolnBlk.NUM_VAR_DUSTY2D = nv;
    if (SolnBlk.residual_variable != nr) SolnBlk.residual_variable = nr;
  }

  // Broadcast the particle-phase formulation indicator.
  MPI::COMM_WORLD.Bcast(&(SolnBlk.Particles),1,MPI::INT,0);

  // Broadcast the phase interaction indicator.
  MPI::COMM_WORLD.Bcast(&(SolnBlk.PhaseInteraction),1,MPI::INT,0);
  ...
  // Broadcast the electrostatic flow indicator.
  MPI::COMM_WORLD.Bcast(&(SolnBlk.Electrostatic),1,MPI::INT,0);
...
void Broadcast_Solution_Block(Dusty2D_Quad_Block &SolnBlk,
  ...
  Communicator.Bcast(&nv,1,MPI::INT,Source_Rank);
  ...
  // Broadcast the particle-phase formulation indicator.
  Communicator.Bcast(&(SolnBlk.Particles),1,MPI::INT,Source_Rank);

  // Broadcast the phase interaction indicator.
  Communicator.Bcast(&(SolnBlk.PhaseInteraction),1,MPI::INT,Source_Rank);
  ...
  // Broadcast the electrostatic flow indicator.
  Communicator.Bcast(&(SolnBlk.Electrostatic),1,MPI::INT,Source_Rank);
 ...
 *                                                                    *
 **********************************************************************/
void Copy_Solution_Block(Dusty2D_Quad_Block &SolnBlk1,
  ...
  // Copy the particle-phase formulation indicator.
  SolnBlk1.Particles = SolnBlk2.Particles;

  // Copy the phase interaction indicator.
  SolnBlk1.PhaseInteraction = SolnBlk2.PhaseInteraction;
  ...
  // Copy the electrostatic flow indicator.
  SolnBlk1.Electrostatic = SolnBlk2.Electrostatic;
 ...
 *                                                                    *
 **********************************************************************/
int Prolong_Solution_Block(Dusty2D_Quad_Block &SolnBlk_Fine,
  ...
  Vector2D dX;
  Dusty2D_cState Dusty2D_U_STDATM, Ucoarse;
  ...
  if (mesh_refinement_permitted) {
    ...
    // Copy the particle-phase formulation indicator.
    SolnBlk_Fine.Particles = SolnBlk_Original.Particles;

    // Copy the phase interaction indicator.
    SolnBlk_Fine.PhaseInteraction = SolnBlk_Original.PhaseInteraction;
    ...
    // Copy the electrostatic flow indicator.
    SolnBlk_Fine.Electrostatic = SolnBlk_Original.Electrostatic;
    ...
    // Prolong the solution information using pure injection (area-
    // weighted average commented out).
 ...
 *                                                                    *
 **********************************************************************/
int Restrict_Solution_Block(Dusty2D_Quad_Block &SolnBlk_Coarse,
  ...
  double A, total_area;
  Polygon Pc, Pf;
  Dusty2D_pState Dusty2D_W_STDATM; Dusty2D_W_STDATM.Standard_Atmosphere();
  ...
  if (mesh_coarsening_permitted) {
    ...
    // Copy the particle-phase formulation indicator.
    SolnBlk_Coarse.Particles = SolnBlk_Original_SW.Particles;

    // Copy the phase interaction indicator.
    SolnBlk_Coarse.PhaseInteraction = SolnBlk_Original_SW.PhaseInteraction;
    ...
    // Copy the electrostatic flow indicator.
    SolnBlk_Coarse.Electrostatic = SolnBlk_Original_SW.Electrostatic;
 ...
 *                                                                    *
 **********************************************************************/
void ICs(Dusty2D_Quad_Block &SolnBlk,
  ...
  case IC_SOD_XDIR :
    ...
    Wl = Dusty2D_W_STDATM;
    Wr = Dusty2D_pState(DENSITY_STDATM/EIGHT,ZERO,ZERO,PRESSURE_STDATM/TEN);
    for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
  ...
  case IC_GROTH_XDIR :
    ...
    Wl = Dusty2D_pState(4.696,ZERO,ZERO,404.4e03);
    Wr = Dusty2D_pState(1.408,ZERO,ZERO,101.1e03);
    for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
	if (SolnBlk.Grid.Cell[i][j].Xc.x <= ZERO) {
	  SolnBlk.W[i][j] = Wl;
  ...
  case IC_EINFELDT_XDIR :
    ...
    Wl = Dusty2D_pState(ONE,-TWO,ZERO,FOUR/TEN);
    Wr = Dusty2D_pState(ONE, TWO,ZERO,FOUR/TEN);
    for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
	if (SolnBlk.Grid.Cell[i][j].Xc.x <= ZERO) {
	  SolnBlk.W[i][j] = Wl;
  ...
  case IC_EINFELDT_YDIR :
    ...
    Wl = Dusty2D_pState(ONE,ZERO,-TWO,FOUR/TEN);
    Wr = Dusty2D_pState(ONE,ZERO, TWO,FOUR/TEN);
    for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
	if (SolnBlk.Grid.Cell[i][j].Xc.y <= ZERO) {
	  SolnBlk.W[i][j] = Wl;
  ...
  case IC_SHOCK_WAVE_XDIR :
    ...
    //Wl = Dusty2D_pState(2.281, 164.83, ZERO, 201.17e03);
    //Wr = Dusty2D_pState(1.408, ZERO, ZERO, 101.1e03);
    //Wl = Dusty2D_pState(1.3189655, 57.543675, ZERO, 126.65625e03);
    //Wr = Dusty2D_pState(1.125, ZERO, ZERO, 101.325e03);
    //Wl = Dusty2D_pState(1.828125, 186.120997, ZERO, 202650.5);
    //Wr = Dusty2D_pState(1.125, ZERO, ZERO, 101325.0);
    Wl = Wo[0];
    Wr = Dusty2D_W_STDATM;
    for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
	if (SolnBlk.Grid.Cell[i][j].Xc.x < TOLER) {
	  SolnBlk.W[i][j] = Wl;
	} else {
	  SolnBlk.W[i][j] = Wr;
	}
  ...
  case IC_SHOCK_WAVE_YDIR :
    ...
    Wl = Dusty2D_pState(2.281,ZERO,164.83,201.17e03);
    Wr = Dusty2D_pState(1.408,ZERO,ZERO,  101.10e03);
   for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
	if (SolnBlk.Grid.Cell[i][j].Xc.y <= ZERO) {
	  SolnBlk.W[i][j] = Wl;
  ...
  case IC_CONTACT_SURFACE_XDIR :
    ...
    Wl = Dusty2D_pState(1.045,200.0,ZERO,PRESSURE_STDATM);
    Wr = Dusty2D_pState(3.483,200.0,ZERO,PRESSURE_STDATM);
    for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
	if (SolnBlk.Grid.Cell[i][j].Xc.x <= ZERO) {
	  SolnBlk.W[i][j] = Wl;
  ...
  case IC_CONTACT_SURFACE_YDIR :
    ...
    Wl = Dusty2D_pState(1.045,ZERO,200.0,PRESSURE_STDATM);
    Wr = Dusty2D_pState(3.483,ZERO,200.0,PRESSURE_STDATM);
    for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
	if (SolnBlk.Grid.Cell[i][j].Xc.y <= ZERO) {
	  SolnBlk.W[i][j] = Wl;
  ...
  case IC_RAREFACTION_WAVE_XDIR :
    ...
    Wl = Dusty2D_pState(1.598,-383.64,ZERO, 91.88e03);
    Wr = Dusty2D_pState(2.787,-216.97,ZERO,200.09e03);
    for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
	if (SolnBlk.Grid.Cell[i][j].Xc.x <= ZERO) {
	  SolnBlk.W[i][j] = Wl;
  ...
  case IC_RAREFACTION_WAVE_YDIR :
    ...
    Wl = Dusty2D_pState(1.598,ZERO,-383.64,91.88e03);
    Wr = Dusty2D_pState(2.787,ZERO,-216.97,200.0e03);
    for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
	if (SolnBlk.Grid.Cell[i][j].Xc.y <= ZERO) {
	  SolnBlk.W[i][j] = Wl;
  ...
  case IC_SHOCK_BOX :
    ...
    Wl = Dusty2D_W_STDATM;
    Wr = Dusty2D_pState(DENSITY_STDATM*FOUR,ZERO,ZERO,PRESSURE_STDATM*FOUR);
    for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
	if (SolnBlk.Grid.Cell[i][j].Xc.x <= ZERO &&
  ...
  case IC_HIGH_PRESSURE_RESERVOIR :
    ...
    Wl = Dusty2D_pState(HUNDRED*DENSITY_STDATM,ZERO,ZERO,HUNDRED*PRESSURE_STDATM);
    Wr = Dusty2D_W_STDATM;
    for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
  ...
  case IC_LOW_PRESSURE_RESERVOIR :
    ...
    Wl = Dusty2D_W_STDATM;
    Wr = Dusty2D_pState(Wo[0].rho,ZERO,ZERO,Wo[0].p);
  ...
  case IC_CYLINDRICAL_EXPLOSION :
    ...
    Wl = Dusty2D_W_STDATM;
    Wr = Dusty2D_pState(EIGHT*DENSITY_STDATM,ZERO,ZERO,TEN*PRESSURE_STDATM);
    if (SolnBlk.Particles == PARTICLE_PHASE_EULERIAN_FORMULATION) {
      Wl.Wp = Particle2D_pComponents(Particle2D_pState(0.05,ZERO,ZERO,Wl.T()));
      Wr.Wp = Particle2D_pComponents(Particle2D_pState(0.05,ZERO,ZERO,Wr.T()));
    }
    for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
  ...
  case IC_CYLINDRICAL_IMPLOSION :
    ...
    Wl = Dusty2D_pState(EIGHT*DENSITY_STDATM,ZERO,ZERO,TEN*PRESSURE_STDATM);
    Wr = Dusty2D_W_STDATM;
    for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
	if (abs(SolnBlk.Grid.Cell[i][j].Xc) < 0.25) {
  ...
  case IC_SQUARE_WAVE_XDIR :
	...
	if (SolnBlk.Grid.Cell[i][j].Xc.x <= IP.Wave_Position.x+HALF*IP.Wave_Width &&
	  ...
	  SolnBlk.W[i][j] = Dusty2D_W_STDATM;
	}
...
// 	      SolnBlk.W[i][j].Wp[nc] = Wo[0].Wp[nc];
// 	      SolnBlk.W[i][j].Wp[nc].sigma = Wo[0].Wp[nc].sigma*sin(PI*(SolnBlk.Grid.Cell[i][j].Xc.x+HALF*IP.Wave_Width)/IP.Wave_Width);
// 	    }
// 	} else {
// 	  SolnBlk.W[i][j] = Dusty2D_W_STDATM;
// 	}
...
//       }
//     }
    Wp1 = Particle2D_pState(0.08,Wo[0].v.x,Wo[0].v.y,Wo[0].T());
    Wp2 = Particle2D_pState(0.10,-Wo[0].v.x,Wo[0].v.y,Wo[0].T());
    for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
	SolnBlk.W[i][j].Wp.Vacuum();
	if (SolnBlk.Grid.Cell[i][j].Xc.x <= IP.Wave_Position.x+HALF*IP.Wave_Width &&
	    SolnBlk.Grid.Cell[i][j].Xc.x >= IP.Wave_Position.x-HALF*IP.Wave_Width) {
	  SolnBlk.W[i][j].Wp += Particle2D_pComponents(Particle2D_pState(Wp2.sigma*sqr(sin(PI*(SolnBlk.Grid.Cell[i][j].Xc.x+HALF*IP.Wave_Width+IP.Wave_Position.x)/IP.Wave_Width)),
									 Wp2.u.x,Wp2.u.y,Wp2.Tp));
	}
	//SolnBlk.W[i][j].Wp += Particle2D_pComponents(Wp1);
	SolnBlk.W[i][j].rho = Wo[0].rho;
	SolnBlk.W[i][j].v.x = Wo[0].v.x;
	SolnBlk.W[i][j].v.y = Wo[0].v.y;
	SolnBlk.W[i][j].p = Wo[0].p;
  ...
  case IC_COMPRESSION_XDIR :
    ...
    Wl = Dusty2D_pState(ONE, 200.0,ZERO,PRESSURE_STDATM);
    Wr = Dusty2D_pState(ONE,-200.0,ZERO,PRESSURE_STDATM);
    if (SolnBlk.Particles == PARTICLE_PHASE_EULERIAN_FORMULATION) {
      Wl.Wp = Particle2D_pComponents(Particle2D_pState(0.05, 200.0,ZERO,PRESSURE_STDATM/Wl.R));
      Wr.Wp = Particle2D_pComponents(Particle2D_pState(0.05,-200.0,ZERO,PRESSURE_STDATM/Wr.R));
    }
    for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
	if (SolnBlk.Grid.Cell[i][j].Xc.x <= ZERO) {
  ...
  case IC_COMPRESSION_YDIR :
    ...
    Wl = Dusty2D_pState(ONE,ZERO, 200.0,PRESSURE_STDATM);
    Wr = Dusty2D_pState(ONE,ZERO,-200.0,PRESSURE_STDATM);
    if (SolnBlk.Particles == PARTICLE_PHASE_EULERIAN_FORMULATION) {
      Wl.Wp = Particle2D_pComponents(Particle2D_pState(0.05,ZERO, 200.0,PRESSURE_STDATM/Wl.R));
      Wr.Wp = Particle2D_pComponents(Particle2D_pState(0.05,ZERO,-200.0,PRESSURE_STDATM/Wr.R));
    }
    for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
      for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
	if (SolnBlk.Grid.Cell[i][j].Xc.y <= ZERO) {
  ...
  case IC_EXPANSION_YDIR :
    ...
    break;
  case IC_CROSSING_JETS :
    // Set the initial data for a two-dimensional crossing jets IBVP.
    Wl = Dusty2D_pState(Wo[0].rho,Wo[0].v.x,-Wo[0].v.x,Wo[0].p);
    Wr = Dusty2D_pState(Wo[0].rho,Wo[0].v.x, Wo[0].v.x,Wo[0].p);
    if (SolnBlk.Particles == PARTICLE_PHASE_EULERIAN_FORMULATION) {
      f = IP.Mp*sqrt(GAMMA_AIR*(R_UNIVERSAL/(MOLE_WT_AIR*MILLI))*TEMPERATURE_STDATM);
      Wl.Wp = Particle2D_pComponents(Particle2D_pState(0.05,f,-f,Wo[0].T()));
      Wr.Wp = Particle2D_pComponents(Particle2D_pState(0.05,f, f,Wo[0].T()));
    }
    for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
      for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
	if (i <= SolnBlk.ICl && SolnBlk.Grid.BCtypeW[j] == BC_FIXED) {
	  if (SolnBlk.Grid.Cell[i][j].Xc.y >= 0.80-TOLER) {
	    SolnBlk.W[i][j] = Wl;
 	  } else if (SolnBlk.Grid.Cell[i][j].Xc.y <= 0.20+TOLER) {
 	    SolnBlk.W[i][j] = Wr;
	  } else {
	    SolnBlk.W[i][j] = Dusty2D_pState(Wo[0].rho,ZERO,ZERO,Wo[0].p);
	  }
	} else {
	  SolnBlk.W[i][j] = Dusty2D_pState(Wo[0].rho,ZERO,ZERO,Wo[0].p);
	}
  ...
  case IC_VISCOUS_BACKWARD_FACING_STEP :
	...
	SolnBlk.W[i][j] = BackwardFacingStep(Wo[0],SolnBlk.Grid.Cell[i][j].Xc,
					     IP.Inflow_Height,IP.Step_Height,
 ...
 * to the local solution block for the specified quadrilateral        *
 * solution blocks.                                                   *
 *                                                                    *
 **********************************************************************/
int Copy_Electrostatic_Field_Variables(Dusty2D_Quad_Block &SolnBlk,
				       Electrostatic2D_Quad_Block &ES_SolnBlk) {

  // Copy the electrostatic field solution information.
  for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
    for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
      SolnBlk.E[i][j] = ES_SolnBlk.U[i][j];
    }
  }
 ...
 *                                                                    *
 **********************************************************************/
void BCs(Dusty2D_Quad_Block &SolnBlk, Dusty2D_Input_Parameters &IP) {
  ...
  for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
	...
	break;
      case BC_ABSORPTION :
	SolnBlk.W[SolnBlk.ICl-1][j] = Absorb(SolnBlk.W[SolnBlk.ICl][j],SolnBlk.Grid.nfaceW(SolnBlk.ICl,j));
	SolnBlk.U[SolnBlk.ICl-1][j] = U(SolnBlk.W[SolnBlk.ICl-1][j]);
	SolnBlk.W[SolnBlk.ICl-2][j] = Absorb(SolnBlk.W[SolnBlk.ICl+1][j],SolnBlk.Grid.nfaceW(SolnBlk.ICl,j));
	SolnBlk.U[SolnBlk.ICl-2][j] = U(SolnBlk.W[SolnBlk.ICl-2][j]);
	break;
	...
	break;
      case BC_ABSORPTION :
	SolnBlk.W[SolnBlk.ICu+1][j] = Absorb(SolnBlk.W[SolnBlk.ICu][j],SolnBlk.Grid.nfaceE(SolnBlk.ICu,j));
	SolnBlk.U[SolnBlk.ICu+1][j] = U(SolnBlk.W[SolnBlk.ICu+1][j]);
	SolnBlk.W[SolnBlk.ICu+2][j] = Absorb(SolnBlk.W[SolnBlk.ICu-1][j],SolnBlk.Grid.nfaceE(SolnBlk.ICu,j));
	SolnBlk.U[SolnBlk.ICu+2][j] = U(SolnBlk.W[SolnBlk.ICu+2][j]);
	break;
      case BC_WALL_VISCOUS_HEATFLUX :
  ...
  for (int i = SolnBlk.ICl-SolnBlk.Nghost; i <= SolnBlk.ICu+SolnBlk.Nghost; i++) {
	...
	SolnBlk.W[i][SolnBlk.JCl-2].omega = SolnBlk.W[i][SolnBlk.JCl].omega;
      ...
      break;
    case BC_ABSORPTION :
      SolnBlk.W[i][SolnBlk.JCl-1] = Absorb(SolnBlk.W[i][SolnBlk.JCl],
					   SolnBlk.Grid.nfaceS(i,SolnBlk.JCl));
      SolnBlk.U[i][SolnBlk.JCl-1] = U(SolnBlk.W[i][SolnBlk.JCl-1]);
      SolnBlk.W[i][SolnBlk.JCl-2] = Absorb(SolnBlk.W[i][SolnBlk.JCl+1],
					   SolnBlk.Grid.nfaceS(i,SolnBlk.JCl));
      SolnBlk.U[i][SolnBlk.JCl-2] = U(SolnBlk.W[i][SolnBlk.JCl-2]);
      break;
    case BC_WALL_VISCOUS_HEATFLUX :
      SolnBlk.W[i][SolnBlk.JCl-1] = WallViscousHeatFlux(SolnBlk.W[i][SolnBlk.JCl],
							SolnBlk.Grid.nfaceS(i,SolnBlk.JCl));
      SolnBlk.U[i][SolnBlk.JCl-1] = U(SolnBlk.W[i][SolnBlk.JCl-1]);
      SolnBlk.W[i][SolnBlk.JCl-2] = WallViscousHeatFlux(SolnBlk.W[i][SolnBlk.JCl+1],
							SolnBlk.Grid.nfaceS(i,SolnBlk.JCl));
      SolnBlk.U[i][SolnBlk.JCl-2] = U(SolnBlk.W[i][SolnBlk.JCl-2]);
      break;
    case BC_WALL_VISCOUS_ISOTHERMAL :
      SolnBlk.W[i][SolnBlk.JCl-1] = WallViscousIsothermal(SolnBlk.W[i][SolnBlk.JCl],
							  SolnBlk.Grid.nfaceS(i,SolnBlk.JCl),
							  SolnBlk.Twall);
      SolnBlk.U[i][SolnBlk.JCl-1] = U(SolnBlk.W[i][SolnBlk.JCl-1]);
      SolnBlk.W[i][SolnBlk.JCl-2] = WallViscousIsothermal(SolnBlk.W[i][SolnBlk.JCl+1],
							  SolnBlk.Grid.nfaceS(i,SolnBlk.JCl),
							  SolnBlk.Twall);
      SolnBlk.U[i][SolnBlk.JCl-2] = U(SolnBlk.W[i][SolnBlk.JCl-2]);
      break;
    case BC_MOVING_WALL_HEATFLUX :
      SolnBlk.W[i][SolnBlk.JCl-1] = MovingWallHeatFlux(SolnBlk.W[i][SolnBlk.JCl],
						       -SolnBlk.Grid.nfaceS(i,SolnBlk.JCl),
						       SolnBlk.Vwall.x);
      SolnBlk.U[i][SolnBlk.JCl-1] = U(SolnBlk.W[i][SolnBlk.JCl-1]);
      SolnBlk.W[i][SolnBlk.JCl-2] = MovingWallHeatFlux(SolnBlk.W[i][SolnBlk.JCl+1],
						       -SolnBlk.Grid.nfaceS(i,SolnBlk.JCl),
						       SolnBlk.Vwall.x);
      SolnBlk.U[i][SolnBlk.JCl-2] = U(SolnBlk.W[i][SolnBlk.JCl-2]);
      break;
    case BC_MOVING_WALL_ISOTHERMAL :
      SolnBlk.W[i][SolnBlk.JCl-1] = MovingWallIsothermal(SolnBlk.W[i][SolnBlk.JCl],
							 SolnBlk.Grid.nfaceS(i,SolnBlk.JCl),
							 SolnBlk.Vwall.x,SolnBlk.Twall);
      SolnBlk.U[i][SolnBlk.JCl-1] = U(SolnBlk.W[i][SolnBlk.JCl-1]);
      SolnBlk.W[i][SolnBlk.JCl-2] = MovingWallIsothermal(SolnBlk.W[i][SolnBlk.JCl+1],
							 SolnBlk.Grid.nfaceS(i,SolnBlk.JCl),
							 SolnBlk.Vwall.x,SolnBlk.Twall);
      SolnBlk.U[i][SolnBlk.JCl-2] = U(SolnBlk.W[i][SolnBlk.JCl-2]);
      break;
    ...
    case BC_REFLECTION :
      ...
      break;
    case BC_ABSORPTION :
      SolnBlk.W[i][SolnBlk.JCu+1] = Absorb(SolnBlk.W[i][SolnBlk.JCu],SolnBlk.Grid.nfaceN(i,SolnBlk.JCu));
      SolnBlk.U[i][SolnBlk.JCu+1] = U(SolnBlk.W[i][SolnBlk.JCu+1]);
      SolnBlk.W[i][SolnBlk.JCu+2] = Absorb(SolnBlk.W[i][SolnBlk.JCu-1],SolnBlk.Grid.nfaceN(i,SolnBlk.JCu));
      SolnBlk.U[i][SolnBlk.JCu+2] = U(SolnBlk.W[i][SolnBlk.JCu+2]);
      break;
    case BC_WALL_VISCOUS_HEATFLUX :
  ...
  if ((SolnBlk.Grid.BCtypeW[SolnBlk.JCl] == BC_REFLECTION ||
       SolnBlk.Grid.BCtypeW[SolnBlk.JCl] == BC_ABSORPTION ||
      ...
      (SolnBlk.Grid.BCtypeS[SolnBlk.ICl] == BC_REFLECTION ||
       SolnBlk.Grid.BCtypeS[SolnBlk.ICl] == BC_ABSORPTION ||
  ...
  if ((SolnBlk.Grid.BCtypeW[SolnBlk.JCu] == BC_REFLECTION ||
       SolnBlk.Grid.BCtypeW[SolnBlk.JCu] == BC_ABSORPTION ||
      ...
      (SolnBlk.Grid.BCtypeN[SolnBlk.ICl] == BC_REFLECTION ||
       SolnBlk.Grid.BCtypeN[SolnBlk.ICl] == BC_ABSORPTION ||
  ...
  if ((SolnBlk.Grid.BCtypeE[SolnBlk.JCl] == BC_REFLECTION ||
       SolnBlk.Grid.BCtypeE[SolnBlk.JCl] == BC_ABSORPTION ||
      ...
      (SolnBlk.Grid.BCtypeS[SolnBlk.ICu] == BC_REFLECTION ||
       SolnBlk.Grid.BCtypeS[SolnBlk.ICu] == BC_ABSORPTION ||
  ...
  if ((SolnBlk.Grid.BCtypeE[SolnBlk.JCu] == BC_REFLECTION ||
       SolnBlk.Grid.BCtypeE[SolnBlk.JCu] == BC_ABSORPTION ||
      ...
      (SolnBlk.Grid.BCtypeN[SolnBlk.ICu] == BC_REFLECTION ||
       SolnBlk.Grid.BCtypeN[SolnBlk.ICu] == BC_ABSORPTION ||
 ...
 *                                                                    *
 **********************************************************************/
double CFL(Dusty2D_Quad_Block &SolnBlk,
	   Dusty2D_Input_Parameters &IP) {

  int np;
  double dtMin = MILLION, d_i, d_j, v_i, v_j, u_i, u_j, a;
  double nu, dt_vis;
  ...
  for (int j = SolnBlk.JCl-SolnBlk.Nghost; j <= SolnBlk.JCu+SolnBlk.Nghost; j++) {
	...
	if (SolnBlk.Particles == PARTICLE_PHASE_EULERIAN_FORMULATION) {
	  u_i = HALF*((SolnBlk.W[i][j].Wp.u())*
		      (SolnBlk.Grid.nfaceE(i,j)-SolnBlk.Grid.nfaceW(i,j)));
	  u_j = HALF*((SolnBlk.W[i][j].Wp.u())*
		      (SolnBlk.Grid.nfaceN(i,j)-SolnBlk.Grid.nfaceS(i,j)));
	  SolnBlk.dt[i][j] = min(SolnBlk.dt[i][j],min(d_i/(TOLER+fabs(u_i)),d_j/(TOLER+fabs(u_j))));

	  SolnBlk.dt[i][j] = min(SolnBlk.dt[i][j],IP.Particle_CFL_Number*SolnBlk.U[i][j].tauv());
	  SolnBlk.dt[i][j] = min(SolnBlk.dt[i][j],IP.Particle_CFL_Number*SolnBlk.U[i][j].tauT());
	}
 ...
 *                                                                    *
 **********************************************************************/
void Set_Global_TimeStep(Dusty2D_Quad_Block &SolnBlk,
                         const double &Dt_min) {
 ...
 *                                                                    *
 **********************************************************************/
double L1_Norm_Residual(Dusty2D_Quad_Block &SolnBlk) {
 ...
 *                                                                    *
 **********************************************************************/
double L2_Norm_Residual(Dusty2D_Quad_Block &SolnBlk) {
 ...
 *                                                                    *
 **********************************************************************/
double Max_Norm_Residual(Dusty2D_Quad_Block &SolnBlk) {
 ...
 *                                                                    *
 **********************************************************************/
void Linear_Reconstruction_GreenGauss(Dusty2D_Quad_Block &SolnBlk,
				      const int i,
                                      const int j,
                                      const int Limiter) {
  ...
  Dusty2D_pState W_nw, W_ne, W_sw, W_se, W_face,
                 DU, DUDx_ave, DUDy_ave;
  int NUM_VAR_DUSTY2D = SolnBlk.NumVar();
  Dusty2D_pState Dusty2D_W_VACUUM; Dusty2D_W_VACUUM.Vacuum();
...
//       SolnBlk.dWdy[i][j] = SolnBlk.dWdy[i][j]/SolnBlk.Grid.Cell[i][j].A;
      ...
      DUDx_ave = Dusty2D_W_VACUUM;
      DUDy_ave = Dusty2D_W_VACUUM;
      DxDx_ave = ZERO;
      DxDy_ave = ZERO;
      DyDy_ave = ZERO;
    ...
    if (!SolnBlk.Freeze_Limiter) {
      for (int n = 1; n <= NUM_VAR_DUSTY2D; n++) {
    ...
    SolnBlk.dWdx[i][j] = Dusty2D_W_VACUUM;
    SolnBlk.dWdy[i][j] = Dusty2D_W_VACUUM;
    SolnBlk.phi[i][j] = Dusty2D_W_VACUUM;
  }
 ...
 *                                                                    *
 **********************************************************************/
void Linear_Reconstruction_LeastSquares(Dusty2D_Quad_Block &SolnBlk,
				        const int i,
                                        const int j,
                                        const int Limiter) {
 ...
 *                                                                    *
 **********************************************************************/
void Linear_Reconstruction_LeastSquares_2(Dusty2D_Quad_Block &SolnBlk,
				          const int i,
                                          const int j,
                                          const int Limiter) {
 ...
 *                                                                    *
 **********************************************************************/
void Residual_Smoothing(Dusty2D_Quad_Block &SolnBlk,
...
void Calculate_Refinement_Criteria(double *refinement_criteria,
  ...
  grad_sigma_criteria_max = ZERO;
  div_U_criteria_max = ZERO;
  curl_U_criteria_max = ZERO;
  min_sigma = ONE;
  max_sigma = ZERO;
  ...
  for (int j = SolnBlk.JCl-1; j <= SolnBlk.JCu+1; j++) {
	...
	if (IP.Refinement_Criteria_Gradient_Particle_Concentration && SolnBlk.W[i][j].Wp.sigma() > ZERO) {
	  grad_sigma_x = SolnBlk.dWdx[i][j].Wp.sigma();
	  grad_sigma_y = SolnBlk.dWdy[i][j].Wp.sigma();
	  grad_sigma_abs = sqrt(sqr(grad_sigma_x) + sqr(grad_sigma_y));
	  grad_sigma_criteria = sqrt(SolnBlk.Grid.Cell[i][j].A)*grad_sigma_abs/(SolnBlk.W[i][j].Wp.sigma() + TOLER);
	} else {
	...
	if (IP.Refinement_Criteria_Divergence_Particle_Velocity && SolnBlk.W[i][j].Wp.sigma() > ZERO) {
	  div_U = SolnBlk.dWdx[i][j].Wp.u().x + SolnBlk.dWdy[i][j].Wp.u().y;
	  div_U_criteria = sqrt(SolnBlk.Grid.Cell[i][j].A)*fabs(div_U)/SolnBlk.W[i][j].a();
	} else {
	  div_U_criteria = ZERO;
	}
	...
	// particle-phase velocity vector.
	if (IP.Refinement_Criteria_Curl_Particle_Velocity && SolnBlk.W[i][j].Wp.sigma() > ZERO) {
	  curl_U_z = SolnBlk.dWdx[i][j].Wp.u().y - SolnBlk.dWdy[i][j].Wp.u().x; 
	  curl_U_abs = sqrt(sqr(curl_U_z));
	  curl_U_criteria = sqrt(SolnBlk.Grid.Cell[i][j].A)*curl_U_abs/SolnBlk.W[i][j].a();
	} else {
	...
	if (IP.Refinement_Criteria_Ratio_Particle_Concentration) {
	  min_sigma = min(min_sigma,SolnBlk.W[i][j].Wp.sigma());
	  max_sigma = max(max_sigma,SolnBlk.W[i][j].Wp.sigma());
	}
  ...
  if (IP.Refinement_Criteria_Gradient_Particle_Concentration) {
    refinement_criteria[refinement_criteria_number] = grad_sigma_criteria_max;
    refinement_criteria_number++;
  }
  if (IP.Refinement_Criteria_Divergence_Particle_Velocity) {
    refinement_criteria[refinement_criteria_number] = div_U_criteria_max;
    refinement_criteria_number++;
  }
  if (IP.Refinement_Criteria_Curl_Particle_Velocity) {
    refinement_criteria[refinement_criteria_number] = curl_U_criteria_max;
    refinement_criteria_number++;
  }
  if (IP.Refinement_Criteria_Ratio_Particle_Concentration) {
    if (min_sigma < 0.001 && max_sigma < 0.001) {
      refinement_criteria[refinement_criteria_number] = HALF*IP.Threshold_for_Coarsening;
    } else if (min_sigma < 0.001 && max_sigma > 0.001) {
      refinement_criteria[refinement_criteria_number] = HALF*(ONE+IP.Threshold_for_Refinement);
    } else if (max_sigma/min_sigma > TWO) {
      refinement_criteria[refinement_criteria_number] = HALF*(ONE+IP.Threshold_for_Refinement);
    } else {
      refinement_criteria[refinement_criteria_number] = HALF*(IP.Threshold_for_Refinement +
 ...
 * Routine: Calculate_Refinement_Criteria                             *
 *                                                                    *
 * Calculate refinement criteria for the specified cell of the given  *
 * solution block.                                                    *
 *                                                                    *
 **********************************************************************/
void Calculate_Refinement_Criteria(Dusty2D_Quad_Block &SolnBlk,
				   Dusty2D_Input_Parameters &IP,
  ...
  double grad_rho_x, grad_rho_y, grad_rho_abs, grad_rho_criteria,
         ...
         grad_sigma_x, grad_sigma_y, grad_sigma_abs, grad_sigma_criteria,
         div_U, div_U_criteria, curl_U_z, curl_U_abs, curl_U_criteria;
  int refinement_criteria_number;
  ...
  // (4) Refinement criteria based on the gradient of the particle concentration field;
  // (5) Refinement criteria based on the divergence of the particle-phase velocity vector;
  // (6) Refinement criteria based on the curl of the particle-phase velocity vector.
 ...
 *                                                                    *
 **********************************************************************/
void Fix_Refined_Block_Boundaries(Dusty2D_Quad_Block &SolnBlk,
 ...
 *                                                                    *
 **********************************************************************/
void Unfix_Refined_Block_Boundaries(Dusty2D_Quad_Block &SolnBlk) {
 ...
 *                                                                    *
 **********************************************************************/
void Apply_Boundary_Flux_Corrections(Dusty2D_Quad_Block &SolnBlk,
 ...
 *                                                                    *
 **********************************************************************/
void Apply_Boundary_Flux_Corrections_Multistage_Explicit(Dusty2D_Quad_Block &SolnBlk,
                                                         const int i_stage,
                                                         Dusty2D_Input_Parameters &IP,
                                                         const int Number_Neighbours_North_Boundary,
                                                         const int Number_Neighbours_South_Boundary,
 ...
 * HLLL, or HLLC flux functions for the gas-phase and the single- or  *
 * multi-velocity treatments for the particle-phase.  The residual is *
 * stored in dUdt[][][0].                                             *
 *                                                                    *
 **********************************************************************/
int dUdt_Residual_Evaluation(Dusty2D_Quad_Block &SolnBlk,
			     Dusty2D_Input_Parameters &IP) {
  ...
  Dusty2D_pState Wu, Wd, dWdx, dWdy;
  ...
  for (int j = SolnBlk.JCl-1; j <= SolnBlk.JCu+1; j++) {
    ...
    SolnBlk.dUdt[SolnBlk.ICl-1][j][0] = Dusty2D_U_VACUUM;

    for (int i = SolnBlk.ICl-1; i <= SolnBlk.ICu; i++) {

      SolnBlk.dUdt[i+1][j][0] = Dusty2D_U_VACUUM;

      if (j >= SolnBlk.JCl && j <= SolnBlk.JCu) {

	if (i == SolnBlk.ICl-1 && 
	    (SolnBlk.Grid.BCtypeW[j] == BC_REFLECTION ||
	     SolnBlk.Grid.BCtypeW[j] == BC_ABSORPTION ||
	  ...
	  if (SolnBlk.Grid.BCtypeW[j] == BC_REFLECTION) {
	  ...
	  } else if (SolnBlk.Grid.BCtypeW[j] == BC_ABSORPTION) {
	    // WEST face of cell (i+1,j) is an ABSORPTION boundary.
	    Wl = Absorb(Wr,SolnBlk.Grid.nfaceW(i+1,j));
	    ...
	    Wl = BC_Characteristic_Pressure(Wr,SolnBlk.WoW[j],SolnBlk.Grid.nfaceW(i+1,j));
		   ...
		   (SolnBlk.Grid.BCtypeE[j] == BC_REFLECTION ||
		    SolnBlk.Grid.BCtypeE[j] == BC_ABSORPTION ||
	  ...
	  } else if (SolnBlk.Grid.BCtypeE[j] == BC_ABSORPTION) {
	    // EAST face of cell (i,j) is an ABSORPTION boundary.
	    Wr = Absorb(Wl,SolnBlk.Grid.nfaceE(i,j));
	...
	// Determine EAST face particle flux if necessary.
	if (SolnBlk.Particles == PARTICLE_PHASE_EULERIAN_FORMULATION) {
	  switch(IP.i_Particle_Flux_Function) {
	  case PARTICLE_PHASE_FLUX_FUNCTION_SAUREL :
	    Flux += FluxSaurel_n(Wl,Wr,SolnBlk.Grid.nfaceE(i,j));
	    break;
	  case PARTICLE_PHASE_FLUX_FUNCTION_MULTIVELOCITY :
	    Flux += FluxMultiVelocity_n(Wl,Wr,SolnBlk.Grid.nfaceE(i,j));
	    break;
	  default :
	    Flux += FluxSaurel_n(Wl,Wr,SolnBlk.Grid.nfaceE(i,j));
	    break;
	...
	// Compute the cell centred stress tensor and heat flux vector if required.
 	if (SolnBlk.Flow_Type && SolnBlk.Axisymmetric) {
	  // Compute the cell-centered viscous terms.
	...
	if (SolnBlk.Flow_Type) {
	  ...
	  case VISCOUS_RECONSTRUCTION_ARITHMETIC_AVERAGE :
	    ...
	    Wl = HALF*(SolnBlk.W[i][j] + SolnBlk.W[i+1][j]);
	    ...
	    Flux -= ViscousFlux_n(SolnBlk.Grid.xfaceE(i,j),Wl,dWdx,dWdy,
	  ...
	  case VISCOUS_RECONSTRUCTION_DIAMOND_PATH :
	    // Select the appropriate nodes and states for the diamond-path integration.
	    Wl = SolnBlk.W[i  ][j]; Xl = SolnBlk.Grid.Cell[i  ][j].Xc;
	    Wr = SolnBlk.W[i+1][j]; Xr = SolnBlk.Grid.Cell[i+1][j].Xc;
		...
		Wu.rho = Wr.rho; Wu.v = Vector2D_ZERO; Wu.p = Wr.p;
		Wd.rho = Wr.rho; Wd.v = Vector2D_ZERO; Wd.p = Wr.p;
	      } else if (SolnBlk.Grid.BCtypeW[j] == BC_WALL_VISCOUS_ISOTHERMAL) {
		// WEST face of cell (i+1,j) is a WALL_VISCOUS_ISOTHERMAL boundary.
		viscous_bc_flag = DIAMONDPATH_RIGHT_TRIANGLE_ISOTHERMAL;
		Wu.rho = Wr.rho; Wu.v = Vector2D_ZERO; Wu.p = Wr.rho*Wr.R*SolnBlk.Twall;
		Wd.rho = Wr.rho; Wd.v = Vector2D_ZERO; Wd.p = Wr.rho*Wr.R*SolnBlk.Twall;
	      } else if (SolnBlk.Grid.BCtypeW[j] == BC_MOVING_WALL) {
		// WEST face of cell (i+1,j) is a MOVINGWALL_HEATFLUX boundary.
		viscous_bc_flag = DIAMONDPATH_RIGHT_TRIANGLE_HEATFLUX;
		Wu.rho = Wr.rho; Wu.v = SolnBlk.Vwall; Wu.p = Wr.p;
		Wd.rho = Wr.rho; Wd.v = SolnBlk.Vwall; Wd.p = Wr.p;
	      } else if (SolnBlk.Grid.BCtypeW[j] == BC_MOVING_WALL_ISOTHERMAL) {
		// WEST face of cell (i+1,j) is a MOVINGWALL_ISOTHERMAL boundary.
		viscous_bc_flag = DIAMONDPATH_RIGHT_TRIANGLE_ISOTHERMAL;
		Wu.rho = Wr.rho; Wu.v = SolnBlk.Vwall; Wu.p = Wr.rho*Wr.R*SolnBlk.Twall;
		Wd.rho = Wr.rho; Wd.v = SolnBlk.Vwall; Wd.p = Wr.rho*Wr.R*SolnBlk.Twall;
	      } else if (SolnBlk.Grid.BCtypeW[j] == BC_BURNING_SURFACE) {
		// WEST face of cell (i+1,j) is a BURNING_SURFACE boundary.
		viscous_bc_flag = DIAMONDPATH_RIGHT_TRIANGLE_ISOTHERMAL;
		...
		Wu.rho = Wl.rho; Wu.v = Vector2D_ZERO; Wu.p = Wl.p;
		Wd.rho = Wl.rho; Wd.v = Vector2D_ZERO; Wd.p = Wl.p;
	      } else if (SolnBlk.Grid.BCtypeE[j] == BC_WALL_VISCOUS_ISOTHERMAL) {
		// EAST face of cell (i,j) is a WALL_VISCOUS_ISOTHERMAL boundary.
		viscous_bc_flag = DIAMONDPATH_LEFT_TRIANGLE_ISOTHERMAL;
		Wu.rho = Wl.rho; Wu.v = Vector2D_ZERO; Wu.p = Wl.rho*Wl.R*SolnBlk.Twall;
		Wd.rho = Wl.rho; Wd.v = Vector2D_ZERO; Wd.p = Wl.rho*Wl.R*SolnBlk.Twall;
	      } else if (SolnBlk.Grid.BCtypeE[j] == BC_MOVING_WALL_HEATFLUX) {
		// EAST face of cell (i,j) is a MOVINGWALL_HEATFLUX boundary.
		viscous_bc_flag = DIAMONDPATH_LEFT_TRIANGLE_HEATFLUX;
		Wu.rho = Wl.rho; Wu.v = SolnBlk.Vwall; Wu.p = Wl.p;
		Wd.rho = Wl.rho; Wd.v = SolnBlk.Vwall; Wd.p = Wl.p;
	      } else if (SolnBlk.Grid.BCtypeE[j] == BC_MOVING_WALL_ISOTHERMAL) {
		// EAST face of cell (i,j) is a MOVINGWALL_ISOTHERMAL boundary.
		viscous_bc_flag = DIAMONDPATH_LEFT_TRIANGLE_ISOTHERMAL;
		Wu.rho = Wl.rho; Wu.v = SolnBlk.Vwall; Wu.p = Wl.rho*Wl.R*SolnBlk.Twall;
		Wd.rho = Wl.rho; Wd.v = SolnBlk.Vwall; Wd.p = Wl.rho*Wl.R*SolnBlk.Twall;
	      } else if (SolnBlk.Grid.BCtypeE[j] == BC_MOVING_WALL_ISOTHERMAL) {
		// EAST face of cell (i,j) is a BURNING_SURFACE boundary.
		viscous_bc_flag = DIAMONDPATH_LEFT_TRIANGLE_ISOTHERMAL;
		...
		Wd = Wu;
	      ...
	      Wu = SolnBlk.WnNE(i,j); Wd = SolnBlk.WnSE(i,j);
	    }
	...
	// Include phase interaction term if required.
	if (SolnBlk.Particles == PARTICLE_PHASE_EULERIAN_FORMULATION &&
	    SolnBlk.PhaseInteraction) SolnBlk.dUdt[i][j][0] += SolnBlk.W[i][j].Sp(SolnBlk.PhaseInteraction);

	// Include the electrostatic source term if required.
	if (SolnBlk.Electrostatic) SolnBlk.dUdt[i][j][0] += SolnBlk.W[i][j].Se(SolnBlk.E[i][j]);
    ...
    if (j > SolnBlk.JCl-1 && j < SolnBlk.JCu+1) {
      SolnBlk.dUdt[SolnBlk.ICl-1][j][0] = Dusty2D_U_VACUUM;
      SolnBlk.dUdt[SolnBlk.ICu+1][j][0] = Dusty2D_U_VACUUM;
    }
  ...
  for (int i = SolnBlk.ICl; i <= SolnBlk.ICu; i++) {
    for (int j = SolnBlk.JCl-1; j <= SolnBlk.JCu; j++) {
	  ...
	  (SolnBlk.Grid.BCtypeS[i] == BC_REFLECTION ||
	   ...
	   SolnBlk.Grid.BCtypeS[i] == BC_ABSORPTION ||
	...
	if (SolnBlk.Grid.BCtypeS[i] == BC_REFLECTION) {
	...
	} else if (SolnBlk.Grid.BCtypeS[i] == BC_ABSORPTION) {
	  // SOUTH face of cell (i,j+1) is an ABSORPTION boundary.
	  Wl = Absorb(Wr,SolnBlk.Grid.nfaceS(i,j+1));
	  ...
	  Wl = BC_Characteristic_Pressure(Wr,SolnBlk.WoS[i],SolnBlk.Grid.nfaceS(i,j+1));
		 ...
		 (SolnBlk.Grid.BCtypeN[i] == BC_REFLECTION ||
		  SolnBlk.Grid.BCtypeN[i] == BC_ABSORPTION ||
	...
	} else if (SolnBlk.Grid.BCtypeN[i] == BC_ABSORPTION) {
	  // NORTH face of cell (i,j) is an ABSORPTION boundary.
	  Wr = Absorb(Wl,SolnBlk.Grid.nfaceN(i,j));
	...
	break;
      ...
      // Determine NORTH face particle flux if necessary.
      if (SolnBlk.Particles == PARTICLE_PHASE_EULERIAN_FORMULATION) {
	switch(IP.i_Particle_Flux_Function) {
	case PARTICLE_PHASE_FLUX_FUNCTION_SAUREL :
	  Flux += FluxSaurel_n(Wl,Wr,SolnBlk.Grid.nfaceN(i,j));
	  break;
	case PARTICLE_PHASE_FLUX_FUNCTION_MULTIVELOCITY :
	  Flux += FluxMultiVelocity_n(Wl,Wr,SolnBlk.Grid.nfaceN(i,j));
	  break;
	default :
	  Flux += FluxSaurel_n(Wl,Wr,SolnBlk.Grid.nfaceN(i,j));
	  break;
	...
	case VISCOUS_RECONSTRUCTION_DIAMOND_PATH :
	  // Select the appropriate nodes and states for the diamond-path integration.
	  Wl = SolnBlk.W[i][j  ]; Xl = SolnBlk.Grid.Cell[i][j  ].Xc;
	  Wr = SolnBlk.W[i][j+1]; Xr = SolnBlk.Grid.Cell[i][j+1].Xc;
	  ...
	  if (j == SolnBlk.JCl-1 && 
	    ...
	    if (SolnBlk.Grid.BCtypeS[i] == BC_WALL_VISCOUS_HEATFLUX) {
	      ...
	      // SOUTH face of cell (i,j+1) is a MOVINGWALL_HEATFLUX boundary.
	      viscous_bc_flag = DIAMONDPATH_RIGHT_TRIANGLE_HEATFLUX;
	      Wu.rho = Wr.rho; Wu.v = SolnBlk.Vwall; Wu.p = Wl.p;
	      Wd.rho = Wr.rho; Wd.v = SolnBlk.Vwall; Wd.p = Wl.p;
	    } else if (SolnBlk.Grid.BCtypeS[i] == BC_MOVING_WALL_ISOTHERMAL) {
	      // SOUTH face of cell (i,j+1) is a MOVINGWALL_ISOTHERMAL boundary.
	      viscous_bc_flag = DIAMONDPATH_RIGHT_TRIANGLE_ISOTHERMAL;
	      Wu.rho = Wr.rho; Wu.v = SolnBlk.Vwall; Wu.p = Wl.rho*Wr.R*SolnBlk.Twall;
	      Wd.rho = Wr.rho; Wd.v = SolnBlk.Vwall; Wd.p = Wl.rho*Wr.R*SolnBlk.Twall;
	    } else if (SolnBlk.Grid.BCtypeS[i] == BC_BURNING_SURFACE) {
	      // SOUTH face of cell (i,j+1) is a BURNING_SURFACE boundary.
	      viscous_bc_flag = DIAMONDPATH_RIGHT_TRIANGLE_ISOTHERMAL;
	    ...
	    if (SolnBlk.Grid.BCtypeN[i] == BC_WALL_VISCOUS_HEATFLUX) {
	      ...
	      // NORTH face of cell (i,j) is a MOVINGWALL_HEATFLUX boundary.
	      viscous_bc_flag = DIAMONDPATH_LEFT_TRIANGLE_HEATFLUX;
	      Wu.rho = Wl.rho; Wu.v = SolnBlk.Vwall; Wu.p = Wr.p;
	      Wd.rho = Wl.rho; Wd.v = SolnBlk.Vwall; Wd.p = Wr.p;
	    } else if (SolnBlk.Grid.BCtypeN[i] == BC_MOVING_WALL_ISOTHERMAL) {
	      // NORTH face of cell (i,j) is a MOVINGWALL_ISOTHERMAL boundary.
	      viscous_bc_flag = DIAMONDPATH_LEFT_TRIANGLE_ISOTHERMAL;
	      Wu.rho = Wl.rho; Wu.v = SolnBlk.Vwall; Wu.p = Wr.rho*Wl.R*SolnBlk.Twall;
	      Wd.rho = Wl.rho; Wd.v = SolnBlk.Vwall; Wd.p = Wr.rho*Wl.R*SolnBlk.Twall;
	    } else if (SolnBlk.Grid.BCtypeN[i] == BC_BURNING_SURFACE) {
	      // NORTH face of cell (i,j) is a BURNING_SURFACE boundary.
	      viscous_bc_flag = DIAMONDPATH_LEFT_TRIANGLE_ISOTHERMAL;
	    ...
	    // boundary condition.
	    viscous_bc_flag = DIAMONDPATH_QUADRILATERAL_RECONSTRUCTION;
	    Wu = SolnBlk.WnNW(i,j); Wd = SolnBlk.WnNE(i,j);
	  }
    ...
    SolnBlk.dUdt[i][SolnBlk.JCl-1][0] = Dusty2D_U_VACUUM;
    SolnBlk.dUdt[i][SolnBlk.JCu+1][0] = Dusty2D_U_VACUUM;
 ...
 *                                                                    *
 **********************************************************************/
int dUdt_Multistage_Explicit(Dusty2D_Quad_Block &SolnBlk,
                             const int i_stage,
			     Dusty2D_Input_Parameters &IP) {
  ...
  for (int j = SolnBlk.JCl-1; j <= SolnBlk.JCu+1; j++) {
    if (i_stage == 1) {
      ...
      SolnBlk.dUdt[SolnBlk.ICl-1][j][k_residual] = Dusty2D_U_VACUUM;
    } else {
      SolnBlk.dUdt[SolnBlk.ICl-1][j][k_residual] = Dusty2D_U_VACUUM;
    }
	...
	SolnBlk.dUdt[i+1][j][k_residual] = Dusty2D_U_VACUUM;
	...
	if (SolnBlk.Flow_Type) {
	  ...
	  if (i == SolnBlk.ICl-1 && 
	    ...
	    if (SolnBlk.Grid.BCtypeW[j] == BC_WALL_VISCOUS_HEATFLUX) {
	      ...
	      // WEST face of cell (i+1,j) is a WALL_VISCOUS_ISOTHERMAL boundary.
	      viscous_bc_flag = DIAMONDPATH_RIGHT_TRIANGLE_ISOTHERMAL;
	      Wu.rho = Wr.p/(Wr.R*SolnBlk.Twall); Wu.v.x = ZERO; Wu.v.y = ZERO; Wu.p = Wr.p; Wu.k = Wr.k; Wu.omega = Wr.omega;
	    } else if (SolnBlk.Grid.BCtypeW[j] == BC_MOVING_WALL) {
	      // WEST face of cell (i+1,j) is a MOVINGWALL_HEATFLUX boundary.
	      viscous_bc_flag = DIAMONDPATH_RIGHT_TRIANGLE_HEATFLUX;
	      Wu.rho = Wr.rho; Wu.v.x = SolnBlk.Vwall.x; Wu.v.y = SolnBlk.Vwall.y; Wu.p = Wr.p; Wu.k = Wr.k; Wu.omega = Wr.omega;
	    } else if (SolnBlk.Grid.BCtypeW[j] == BC_MOVING_WALL_ISOTHERMAL) {
	      // WEST face of cell (i+1,j) is a MOVINGWALL_ISOTHERMAL boundary.
	      viscous_bc_flag = DIAMONDPATH_RIGHT_TRIANGLE_ISOTHERMAL;
	      Wu.rho = Wr.p/(Wr.R*SolnBlk.Twall); Wu.v.x = SolnBlk.Vwall.x; Wu.v.y = SolnBlk.Vwall.y; Wu.p = Wr.p; Wu.k = Wr.k; Wu.omega = Wr.omega;
	    } else if (SolnBlk.Grid.BCtypeW[j] == BC_BURNING_SURFACE) {
	      // WEST face of cell (i+1,j) is a BURNING_SURFACE boundary.
	      viscous_bc_flag = DIAMONDPATH_RIGHT_TRIANGLE_ISOTHERMAL;
	      Wu = BurningSurface(Wr,SolnBlk.Grid.nfaceW(i+1,j));
	    ...
	    if (SolnBlk.Grid.BCtypeE[j] == BC_WALL_VISCOUS_HEATFLUX) {
	      ...
	      // EAST face of cell (i,j) is a WALL_VISCOUS_ISOTHERMAL boundary.
	      viscous_bc_flag = DIAMONDPATH_LEFT_TRIANGLE_ISOTHERMAL;
	      Wu.rho = Wl.p/(Wl.R*SolnBlk.Twall); Wu.v.x = ZERO; Wu.v.y = ZERO; Wu.p = Wl.p; Wu.k = Wl.k; Wu.omega = Wl.omega;
	    } else if (SolnBlk.Grid.BCtypeE[j] == BC_MOVING_WALL_HEATFLUX) {
	      // EAST face of cell (i,j) is a MOVINGWALL_HEATFLUX boundary.
	      viscous_bc_flag = DIAMONDPATH_LEFT_TRIANGLE_HEATFLUX;
	      Wu.rho = Wl.rho; Wu.v.x = SolnBlk.Vwall.x; Wu.v.y = SolnBlk.Vwall.y; Wu.p = Wl.p; Wu.k = Wl.k; Wu.omega = Wl.omega;
	    } else if (SolnBlk.Grid.BCtypeE[j] == BC_MOVING_WALL_ISOTHERMAL) {
	      // EAST face of cell (i,j) is a MOVINGWALL_ISOTHERMAL boundary.
	      viscous_bc_flag = DIAMONDPATH_LEFT_TRIANGLE_ISOTHERMAL;
	      Wu.rho = Wl.p/(Wl.R*SolnBlk.Twall); Wu.v.x = SolnBlk.Vwall.x; Wu.v.y = SolnBlk.Vwall.y; Wu.p = Wl.p; Wu.k = Wl.k; Wu.omega = Wl.omega;
	    } else if (SolnBlk.Grid.BCtypeE[j] == BC_MOVING_WALL_ISOTHERMAL) {
	      // EAST face of cell (i,j) is a BURNING_SURFACE boundary.
	      viscous_bc_flag = DIAMONDPATH_LEFT_TRIANGLE_ISOTHERMAL;
	      Wu = BurningSurface(Wr,SolnBlk.Grid.nfaceE(i,j));
	...
	SolnBlk.dUdt[i+1][j][k_residual] += (IP.CFL_Number*SolnBlk.dt[i+1][j])*
 	...
 	// Include phase interaction term if required.
	if (SolnBlk.Particles == PARTICLE_PHASE_EULERIAN_FORMULATION &&
	    SolnBlk.PhaseInteraction) {
	  SolnBlk.dUdt[i][j][k_residual] += (IP.CFL_Number*SolnBlk.dt[i][j])*SolnBlk.W[i][j].Sp(SolnBlk.PhaseInteraction);
	}

 	// Include the electrostatic source term if required.
	if (SolnBlk.Electrostatic)
	  SolnBlk.dUdt[i][j][k_residual] += (IP.CFL_Number*SolnBlk.dt[i][j])*SolnBlk.W[i][j].Se(SolnBlk.E[i][j]);
    ...
    if (j > SolnBlk.JCl-1 && j < SolnBlk.JCu+1) {
      SolnBlk.dUdt[SolnBlk.ICl-1][j][k_residual] = Dusty2D_U_VACUUM;
      SolnBlk.dUdt[SolnBlk.ICu+1][j][k_residual] = Dusty2D_U_VACUUM;
    }
  ...
  for (int i = SolnBlk.ICl; i <= SolnBlk.ICu; i++) {
    ...
    SolnBlk.dUdt[i][SolnBlk.JCl-1][k_residual] = Dusty2D_U_VACUUM;
    SolnBlk.dUdt[i][SolnBlk.JCu+1][k_residual] = Dusty2D_U_VACUUM;
 ...
 *                                                                    *
 **********************************************************************/
int Update_Solution_Multistage_Explicit(Dusty2D_Quad_Block &SolnBlk,
					const int i_stage,
					Dusty2D_Input_Parameters &IP) {
  ...
  DenseMatrix dRdU(NUM_VAR_DUSTY2D,NUM_VAR_DUSTY2D);
  DenseSystemLinEqs LinSys;

  // Allocate memory for linear system solver.
  LinSys.allocate(NUM_VAR_DUSTY2D);
  ...
  for (int j = SolnBlk.JCl; j <= SolnBlk.JCu; j++) {
	...
	if (SolnBlk.Axisymmetric) {
	  ...
	  if (SolnBlk.Flow_Type)
	    SolnBlk.W[i][j].dSvdU(dRdU,SolnBlk.dWdy[i][j],SolnBlk.Grid.Cell[i][j].Xc);
	}

	// Calculate source Jacobian for the phase interaction terms if required.
	if (SolnBlk.Particles == PARTICLE_PHASE_EULERIAN_FORMULATION)
	  SolnBlk.W[i][j].dSpdU(dRdU,SolnBlk.PhaseInteraction-1);

	// Calculate source Jacobian for electric field if required.
	if (SolnBlk.Electrostatic) SolnBlk.W[i][j].dSedU(dRdU,SolnBlk.E[i][j]);
	...
	for (int n_residual_reduction = 0; n_residual_reduction < 10; n_residual_reduction++) {
	  ...
	  if (SolnBlk.Particles == PARTICLE_PHASE_EULERIAN_FORMULATION) {
	    SolnBlk.U[i][j].Up.Reset_Unphysical_Properties();
	  }
 ...
 *                                                                    *
 * This routine conducts the particle-phase velocity component        *
 * switching for a given solution block.                              *
 *                                                                    *
 **********************************************************************/
int Multi_Velocity_Component_Particle_Phase_Switch(Dusty2D_Quad_Block &SolnBlk,
						   Dusty2D_Input_Parameters &IP) {

  // Conduct particle-phase component switching in each cell for this stage.
  for (int j = SolnBlk.JCl; j <= SolnBlk.JCu; j++) {
    for (int i = SolnBlk.ICl; i <= SolnBlk.ICu; i++) {
      SolnBlk.W[i][j].MultiVelocity_Switch();
      SolnBlk.U[i][j] = U(SolnBlk.W[i][j]);
    }
  }

  // Particle-phase component switching computed successfully.
  return 0;
 ...
 **********************************************************************/
int Determine_Conservation_Properties(Dusty2D_Quad_Block &SolnBlk,
				      Dusty2D_cState &Umass) {

  for (int j = SolnBlk.JCl; j <= SolnBlk.JCu; j++) {
    for (int i = SolnBlk.ICl; i <= SolnBlk.ICu; i++) {
      Umass.rho  += SolnBlk.U[i][j].rho;
      Umass.dv.x += SolnBlk.U[i][j].dv.x;
      Umass.dv.x += SolnBlk.U[i][j].dv.y;
      Umass.E    += SolnBlk.U[i][j].E;
      Umass.rho  += SolnBlk.U[i][j].Up.sigma();
      Umass.dv.x += SolnBlk.U[i][j].Up.du().x;
      Umass.dv.x += SolnBlk.U[i][j].Up.du().y;
      Umass.E    += SolnBlk.U[i][j].Up.Ep();
    }
  }
