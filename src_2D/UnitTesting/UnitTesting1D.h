...
inline int Perform_UnitTesting(string TestSuite, int TestNumber, string Path_to_Root_Location) {
  ...
  try {
    ...
    /* Change the current working directory to 'Path_to_Root_Location' or 'CFFC_UnitTest_Path_2D'.
       The UnitTesting framework can only find the input and
       output files if it is always launched from "src_2D" directory.
       Therefore, if the executable is launched from a different directory, 
       the 'Path_to_Root_Location' or 'CFFC_UnitTest_Path_2D' variables are 
       used to determined the location of the "src_2D" directory.
       Consequently, the current working directory is changed to 'Path_to_Root_Location' 
       (path provided on the command line) if that exists, or to 'CFFC_UnitTest_Path_2D'
       (path provided as an environment variable) if that is set up.
       If none of these variables are set, it is assumed that the current working directory is "src_2D".
    */

    // check if the environment variable CFFC_UnitTest_Path_2D is set
    CFFC_UnitTest_Path = getenv("CFFC_UnitTest_Path_2D");
	...
	return error_flag;
      ...
      // check if the current directory ends in '/src_2D' (SEVEN characters before the end of string)
      char * buffer = & CurrentDirectory[strlen(CurrentDirectory) - 7];	
