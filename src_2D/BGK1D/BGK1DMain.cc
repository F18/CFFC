 ...
 *******************************************************************
 ...
 This program is a standalone version of the BGK1D Solver
...
int main(int num_arg, char *arg_ptr[]) {
  ...
  char *program_title_ptr =
     "BGK1D: Stand alone Solver for the BGK equations in 1 space dimension.";
  ...
  // Input file name:
  char *Input_File_Name_ptr = "bgk1D.in";

  // Equation type indicator:
  char *Equation_Type_ptr = "BGK1D";
  ...
  /* Parse and interpret command line arguments.  Note that there
     ...
     5) -f name  uses "name" as the input data file rather than
                 the standard input data file "bgk1D.in". */
  ...
  if (error_flag) {
    cout << "\n BGK1D ERROR: Invalid command line argument.\n";
    return (error_flag);
  } /* endif */
  ...
  if ( help_flag) {
     cout << "Usage:\n";
     cout << "bgk1D [-v] [-h] [-i] [-b] [-f name] [-t name number]\n";
     ...
     cout << " -f name         use `name' input data file (`bgk1D.in' is default)\n";
