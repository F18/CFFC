...
// Define this macro to calculate positivity with Green-Gauss gradient reconstruction
//#define CALCULATE_POSITIVITY_WITH_GREEN_GAUSS_GRADIENT_RECONSTRUCTION
...
#ifdef CALCULATE_POSITIVITY_WITH_GREEN_GAUSS_GRADIENT_RECONSTRUCTION

  // This is a very particular situation used only to validate the method with an analytical result

  // Form the influence domain with only first neighbours.
...
Calculate_HighOrder_Discretization_LaplacianOperator(const int &iCell, const int &jCell,
  ...
  if ( jCell == JCu && Grid.BndNorthSplineInfo != NULL){
	...
	if (Grid.BndNorthSpline.getFluxCalcMethod() == ReconstructionBasedFlux){
	  ...
	  // This is a very particular situation used only to validate the method with an analytical result
	  HighOrderVariable(Pos).GreenGauss_FaceGradient_CentroidPathCartesianMesh(*this,
										   iCell,jCell,
										   NORTH,
										   GradU_face);
#else
