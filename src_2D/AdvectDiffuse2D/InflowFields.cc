...
/********************************
 * Top_Hat_InflowField Members  *
 *******************************/

/*! 
 * Parse next input control parameter
 */
void Top_Hat_InflowField::Parse_Next_Input_Control_Parameter(AdvectDiffuse2D_Input_Parameters & IP,
							     int & i_command){
  ...
  } else if (strcmp(IP.Next_Control_Parameter, "Inflow_r0") == 0) {
    i_command = 0;
    ++IP.Line_Number;
    IP.Input_File >> R0;
    IP.Input_File.getline(buffer, sizeof(buffer));

  } else if (strcmp(IP.Next_Control_Parameter, "Inflow_r1") == 0) {
    i_command = 0;
    ++IP.Line_Number;
    IP.Input_File >> R1;
    IP.Input_File.getline(buffer, sizeof(buffer));
 ...
 * Print relevant parameters
 */
void Top_Hat_InflowField::Print_Info(std::ostream & out_file){
  out_file << "\n     -> A : " << A
	   << "\n     -> M : " << Magnitude
	   << "\n     -> r0 : " << R0
	   << "\n     -> r1 : " << R1
...
/*!
 * Broadcast the Top_Hat_InflowField variables to all      
 * processors associated with the specified communicator
 * from the specified processor using the MPI broadcast 
 * routine.
 */
void Top_Hat_InflowField::Broadcast(void){
#ifdef _MPI_VERSION
  ...
  MPI::COMM_WORLD.Bcast(&Magnitude,
			1, 
			MPI::DOUBLE, 0);
  MPI::COMM_WORLD.Bcast(&R0,
			1, 
			MPI::DOUBLE, 0);
  MPI::COMM_WORLD.Bcast(&R1,
			1, 
			MPI::DOUBLE, 0);
...
/*******************************************************************
 * Discontinuous_Exponential_Times_Sinusoidal_InflowField Members  *
 ******************************************************************/

/*! 
 * Parse next input control parameter
 */
void Discontinuous_Exponential_Times_Sinusoidal_InflowField::
Parse_Next_Input_Control_Parameter(AdvectDiffuse2D_Input_Parameters & IP,
  ...
  } else if (strcmp(IP.Next_Control_Parameter, "Inflow_Discontinuity_Location") == 0) {
    i_command = 0;
    ++IP.Line_Number;
    IP.Input_File >> DiscontinuityLocation;
    IP.Input_File.getline(buffer, sizeof(buffer));
 ...
 * Print relevant parameters
 */
void Discontinuous_Exponential_Times_Sinusoidal_InflowField::Print_Info(std::ostream & out_file){
  out_file << "\n     -> A : " << A
	   << "\n     -> M : " << Magnitude
	   << "\n     -> S : " << SteepnessFrequency
	   << "\n     -> P : " << Power
	   << "\n     -> r0: " << ReferencePoint
	   << "\n     -> DiscontinuityLoc: " << DiscontinuityLocation;
}

/*!
 * Broadcast the Discontinuous_Exponential_Times_Sinusoidal_InflowField variables to all      
 * processors associated with the specified communicator
 * from the specified processor using the MPI broadcast 
 * routine.
 */
void Discontinuous_Exponential_Times_Sinusoidal_InflowField::Broadcast(void){
#ifdef _MPI_VERSION
  ...
  MPI::COMM_WORLD.Bcast(&Power,
			1, 
			MPI::INT, 0);
  MPI::COMM_WORLD.Bcast(&DiscontinuityLocation,
			1, 
			MPI::DOUBLE, 0);
