 ...
 ******************************************************************* 
 ...
 is part of the CFFC library of solvers.  
...
int main(int num_arg, char *arg_ptr[]) {
  ...
  /* Parse and interpret command line arguments.  Note that there
     ...
     6) -f name  uses "name" as the input data file rather than
                 the standard input data file "Gaussian2D.in". */
  ...
  if (CFFC_Primary_MPI_Processor() && help_flag) {
     cout << "Usage:\n";
     cout << "gaussian2D [-v] [-h] [-i] [-b] [-pde type] [-f name] [-t name number]\n";
     ...
     cout << " -f name  use `name' input data file (`Gaussian2D.in' is default)\n";
     cout << " -t              run all available test suites\n" 
	  ...
	  << "                 example: gaussian2D -t MyTestSuit 3\n";
     cout.flush();
  } /* endif */
  if (help_flag) {
     CFFC_Finalize_MPI();
