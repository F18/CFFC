...
void Compute_CENO_PseudoInverse(SolutionContainer * SolnBlk,
  ...
  // Execute only if the execution mode is CENO_SPEED_EFFICIENT=ON
  if (CENO_Execution_Mode::CENO_SPEED_EFFICIENT){
	...
	cout << "\n\n --> Preprocessing Step\n" << "---------------------------------------\n"
      ...
      CENO1D_HighOrder_Reconstruction_PseudoInverse(SolnBlk,IP);
...
void CENO1D_HighOrder_Reconstruction_PseudoInverse(SolutionContainer * SolnBlk,
  ...
  if (CENO_Execution_Mode::CENO_SPEED_EFFICIENT){
    ...
    require(SolnBlk[0].Nghost >= 1 + 2*SolnBlk[0].CellRings(),
