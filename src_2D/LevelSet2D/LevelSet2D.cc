...
int main(int num_arg, char *arg_ptr[]) {
  ...
  char *program_title_ptr = 
     "LevelSet2D: Numerical Solvers for the Level Set PDEs.";
  ...
  // Input file name:
  char *Input_File_Name_ptr = "levelset2D.in";

  // Equation type indicator:
  char *Equation_Type_ptr = "LevelSet2D";
  ...
  /* Parse and interpret command line arguments.  Note that there
     ...
     6) -f name  uses "name" as the input data file rather than
                 the standard input data file "levelset2D.in". */
  ...
  if (error_flag) {
    cout << "\n LevelSet2D ERROR: Invalid command line argument.\n";
    return error_flag;
  }
  ...
  if (CFFC_Primary_MPI_Processor() && help_flag) {
     cout << "Usage:\n";
     cout << "levelset2D [-v] [-h] [-i] [-b] [-f name] [-t name number]\\n";
     ...
     cout << " -f name  use `name' input data file (`levelset2D.in' is default)\n";
     cout << " -t              run all available test suites\n" 
	  ...
	  << "                 example: levelset2D -t MyTestSuit 3\n";
     cout.flush();
  }
  if (help_flag) {
     CFFC_Finalize_MPI();
  ...
  if (CFFC_Primary_MPI_Processor() && !batch_flag) 
    cout << "\n\nLevelSet2D: Execution complete.\n";

  // Ending properly.
  return 0;

/* End LevelSet2D program. */
