 ...
 ******************************************************************* 
 ...
 This program is a standalone version of the Euler1D Solver
...
int main(int num_arg, char *arg_ptr[]) {
  ...
  char *program_title_ptr = 
     "Euler1D: Stand alone Solver for the Euler equations in 1 space dimension.";
...
//         strcpy(Equation_Type, Equation_Type_ptr);
  ...
  if (error_flag) {
    cout << "\n Euler1D ERROR: Invalid command line argument.\n";
    return (error_flag);
  } /* endif */
  ...
  if ( help_flag) {
     cout << "Usage:\n";
     cout << "euler1D [-v] [-h] [-i] [-b] [-f name] [-t name number]\n";
     ...
     cout << " -t              run all available test suites\n" 
	  ...
	  << "                 example: euler1D -t MyTestSuit 3\n"
