/**********************************************************************
 * LESPremixed2DQuadMultigrid.h: LESPremixed2D versions of some       *
 *     multigrid functions that could not use the templated version.  *
 **********************************************************************/

#ifndef _LESPREMIXED2D_QUAD_MULTIGRID_INCLUDED
#define _LESPREMIXED2D_QUAD_MULTIGRID_INCLUDED
...
/**********************************************************************
 * LESPremixed2D_Quad_Block -- LESPremixed2D Multigrid Subroutines.   *
 **********************************************************************/
 ...
 *                                                                    *
 **********************************************************************/
template <> int FAS_Multigrid2D_Solver<LESPremixed2D_cState,
...
Zero_Residuals_on_Coarse_Grid(const int &Level,
  ...
  if (IP->FlowType != FLOWTYPE_TURBULENT_RANS_K_OMEGA ||
      IP->FlowType != FLOWTYPE_TURBULENT_LES_TF_K) return 0;
  ...
  for (int nb = 0; nb < List_of_Local_Solution_Blocks[Level].Nblk; nb++) {
	...
	for (int i = Local_SolnBlk[Level][nb].ICl; i <= Local_SolnBlk[Level][nb].ICu; i++) {
	  for(int count=0; count < Local_SolnBlk[Level][nb].W[0][0].nscal; count++) {
	    Local_SolnBlk[Level][nb].dUdt[i][j][k_residual].rhoscalar[count] = ZERO;
...
template <> inline void FAS_Multigrid2D_Solver<LESPremixed2D_cState,
					       LESPremixed2D_Quad_Block,				
					       LESPremixed2D_Input_Parameters>::
...
Update_Primitive_Variables(const int &Level) {
  ...
  for (int b = 0 ; b <= List_of_Local_Solution_Blocks[Level].Nblk-1 ; ++b ) {
	...
	for (int j = Local_SolnBlk[Level][b].JCl; j <= Local_SolnBlk[Level][b].JCu; j++) {
	  ...
	  // ADDED FOR LESPREMIXED2D to check for negative species concentrations
...
#endif // _LESPREMIXED2D_QUAD_MULTIGRID_INCLUDED
