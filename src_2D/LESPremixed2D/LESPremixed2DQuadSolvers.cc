...
/* Include LESPREMIXED2D Multigrid Specializations header file. */
#ifndef _LESPREMIXED_QUAD_MULTIGRID_INCLUDED
#include "LESPremixed2DQuadMultigrid.h"
#endif //_LESPREMIXED_QUAD_MULTIGRID_INCLUDED

/* Include LESPREMIXED2D NKS Specializations header file. */
...
int LESPremixed2DQuadSolver(char *Input_File_Name_ptr,  int batch_flag) {
...
#else
  const int NUM_VAR_LESPREMIXED2D = Input_Parameters.Wo.NUM_VAR_LESPREMIXED2D;
#endif
  
  /*************************************************************************
   ******************** INITIAL GRID & SOLUTION SPACE **********************
   Create initial mesh and allocate LESPremixed2D solution variables for 
  ...
  *******************************************************************************/
  ...
  if (Input_Parameters.i_ICs != IC_RESTART) {
     if (!batch_flag) cout << "\n Performing LESPremixed2D uniform mesh refinement.";
     error_flag = Uniform_AMR(Local_SolnBlk,
                              ...
                              List_of_Local_Solution_Blocks);
     if (error_flag) {
       cout << "\n LESPremixed2D ERROR: Uniform AMR error on processor "
     ...
     ///////////////////////////////////////////////////////////////////////////
     if (!batch_flag) cout << "\n Performing LESPremixed2D boundary mesh refinement.";
     error_flag = Boundary_AMR(Local_SolnBlk,
                               ...
                               List_of_Local_Solution_Blocks);
     if (error_flag) {
       cout << "\n LESPremixed2D ERROR: Boundary AMR error on processor "
	    << List_of_Local_Solution_Blocks.ThisCPU << "." << endl;
       cout.flush();
     } /* endif */
     error_flag = CFFC_OR_MPI(error_flag);
     ...
     ///////////////////////////////////////////////////////////////////////////
     if (!batch_flag) cout << "\n Performing LESPremixed2D initial mesh refinement.";
     error_flag = Initial_AMR(Local_SolnBlk,
		              ...
		              List_of_Local_Solution_Blocks);
     if (error_flag) {
        cout << "\n LESPremixed2D ERROR: Initial AMR error on processor "
             << List_of_Local_Solution_Blocks.ThisCPU
             << ".\n";
        cout.flush();
     } /* endif */
  ...
  ************************************************************************/
  if (Input_Parameters.Morton){
    ...
    error_flag = Morton_ReOrdering_of_Solution_Blocks(QuadTree, 
						      ...
						      processor_cpu_time); 
    if (error_flag) {
      cout <<"\n LESPremixed2D ERROR: Morton re-ordering error on processor "
	   << List_of_Local_Solution_Blocks.ThisCPU
	   << ".\n";
      cout.flush();
      return (error_flag);
  ...
  /*********************** NON-MULTIGRID EXPLICT ***********************************/
	...
	error_flag = Open_Energy_File(energy_file,
        ...
        error_flag = Open_Turbulence_Progress_File(turbulence_progress_file,
				                   Input_Parameters.Output_File_Name,
				                   number_of_time_steps);
      }
      if (error_flag) {
        cout << "\n LESPremixed2D ERROR: Unable to open residual file for LESPremixed2D calculation.\n";
        cout.flush();
      } /* endif */
    } /* endif */
    ...
    **************************************************************************/ 
	...
	(Input_Parameters.Time_Accurate && Input_Parameters.Time_Max > Time)) {
      ...
      if (!batch_flag) { cout << "\n\n Beginning Explicit LESPremixed2D computations on "
	  ...
	  if (!first_step &&
	       ...
	       (number_of_time_steps/Input_Parameters.AMR_Frequency) == 0 ) {
              ...
              error_flag = AMR(Local_SolnBlk,
			       ...
			       ON);
              if (error_flag) {
                 cout << "\n LESPremixed2D ERROR: LESPremixed2D AMR error on processor "
	              << List_of_Local_Solution_Blocks.ThisCPU
	              << ".\n";
	         cout.flush();
              } /* endif */
	  ...
	  ***************************************************************************/ 
	  if (!Input_Parameters.Dual_Time_Stepping  ||  
		...
		number_of_time_steps%Input_Parameters.Restart_Solution_Save_Frequency == 0 ) {
	      if (!batch_flag){
		cout << "\n\n  Saving LESPremixed2D solution to restart data file(s) after"
		     << " n = " << number_of_time_steps << " steps (iterations).";
	      }
	      if (!batch_flag) cout << "\n  Writing LESPremixed2D solution to restart data file(s). \n";
		...
		cout.flush();
	      ...
	      error_flag = Write_Restart_Solution(Local_SolnBlk, 
						  ...
						  processor_cpu_time);
	      if (error_flag) {
		cout << "\n LESPremixed2D ERROR: Unable to open LESPremixed2D restart output data file(s) "
		     << "on processor "
		     << List_of_Local_Solution_Blocks.ThisCPU
		     << ".\n";
		cout.flush();
	  ...
	  ***************************************************************************/
	  ...
	  if (!Input_Parameters.Dual_Time_Stepping || 
		...
		Input_Parameters.i_ICs == IC_HOMOGENEOUS_TURBULENCE) {
	      ...
	      energy = Total_TKE(Local_SolnBlk, List_of_Local_Solution_Blocks, 
                                 Input_Parameters);
	      enstrophy = Total_Enstrophy(Local_SolnBlk, List_of_Local_Solution_Blocks,
                                          Input_Parameters);
              u_prime = u_rms(Local_SolnBlk, List_of_Local_Solution_Blocks, 
                            Input_Parameters);
              viscosity = Average_viscosity(Local_SolnBlk, List_of_Local_Solution_Blocks,
					    Input_Parameters);
              Taylor_scale = Taylor_Scale(Local_SolnBlk, List_of_Local_Solution_Blocks,
                                          Input_Parameters);
		...
		Output_Energy_to_File(energy_file,
				      number_of_time_steps,
				      Time,
				      total_cpu_time,
				      energy,
                ...
                Output_Turbulence_Progress_to_File(turbulence_progress_file,
						   number_of_time_steps,
						   Time,
						   total_cpu_time,
						   u_prime,
...
//         } 
	  ...
	  ***************************************************************************/
	  for ( i_stage  = 1 ; i_stage <= Input_Parameters.N_Stage ; ++i_stage ) {
	    ...
	    error_flag = Send_All_Messages(Local_SolnBlk, 
					   ...
					   OFF);
	    if (error_flag) {
	      cout << "\n LESPremixed2D ERROR: LESPremixed2D message passing error on processor "
		   << List_of_Local_Solution_Blocks.ThisCPU
		   << ".\n";
	      cout.flush();
	    } /* endif */
	    ...
	    error_flag = dUdt_Multistage_Explicit(Local_SolnBlk,
						  ...
						  i_stage);
	    if (error_flag) {
	      cout << "\n LESPremixed2D ERROR: LESPremixed2D solution residual error on processor "
		   << List_of_Local_Solution_Blocks.ThisCPU
		   << ".\n";
	      cout.flush();
	    } /* endif */
	    ...
	    error_flag = Send_Conservative_Flux_Corrections(Local_SolnBlk, 
							    ...
							    NUM_VAR_LESPREMIXED2D);
	    if (error_flag) {
	      cout << "\n LESPremixed2D ERROR: LESPremixed2D flux correction message passing error on processor "
		   << List_of_Local_Solution_Blocks.ThisCPU
		   << ".\n";
	      cout.flush();
	    } /* endif */
	...
	/********** UPDATE STATES FOR DUAL TIME STEPPING ***************************  
	  ...
	  error_flag = Update_Dual_Solution_States(Local_SolnBlk, 
						 List_of_Local_Solution_Blocks);
	  if (error_flag) {
	    cout << "\n LESPremixed2D ERROR: LESPremixed2D solution states update error on processor "
		 << List_of_Local_Solution_Blocks.ThisCPU
		 << ".\n";
	    cout.flush();
	  }
	...
	***************************************************************************/
	...
	flame_x = Average_Flame_Position(Local_SolnBlk, 
     ...
     if (!batch_flag) { cout << "\n\n Explicit LESPremixed2D computations complete on " 
  ...
  if (Input_Parameters.NKS_IP.Maximum_Number_of_NKS_Iterations > 0) {
    time_t start_NKS, end_NKS;
     ...
     if (!batch_flag){ cout << "\n\n Beginning LESPremixed2D NKS computations on " << Date_And_Time() << ".\n\n"; time(&start_NKS); }
     ...
     if (error_flag) {
        if (CFFC_Primary_MPI_Processor()) { 
   	   cout << "\n LESPremixed2D_NKS ERROR: LESPremixed2D solution error on processor " 
                << List_of_Local_Solution_Blocks.ThisCPU << ".\n";
   	   cout.flush();
   	} /* endif */
     } /* endif */
     ...
     /***********************************************************************/
     if (!batch_flag) { cout << "\n\n LESPremixed2D NKS computations complete on " << Date_And_Time() << ".\n"; time(&end_NKS); }
  ...
  while (1) {
    ...
    if (command_flag == EXECUTE_CODE) {
      ...
      if (!batch_flag) cout << "\n Deallocating LESPremixed2D solution variables."; 
      if (Input_Parameters.i_Time_Integration == TIME_STEPPING_MULTIGRID) {
	MGSolver.deallocate();
      }
	...
	MGSolver.deallocate();
      ...
      if (!batch_flag) cout << "\n\n Closing LESPremixed2D input data file.";
    ...
    *************************************************************************/
     ...
     *************************************************************************/
      ...
      if (!batch_flag) cout << "\n Writing LESPremixed2D solution to output data file(s).";
      ...
      error_flag = Output_Tecplot(Local_SolnBlk, 
				  ...
				  Time);
      if (error_flag) {
	cout << "\n LESPremixed2D ERROR: Unable to open LESPremixed2D output data file(s) "
	     << "on processor "
	     << List_of_Local_Solution_Blocks.ThisCPU
	     << ".\n";
	cout.flush();
     ...
     *************************************************************************/
      ...
      // Output solution data.
      if (!batch_flag) cout << "\n Writing cell-centered LESPremixed2D solution to output data file(s).";
     ...
     *************************************************************************/
      ...
      if (CFFC_Primary_MPI_Processor()) {
	if (!batch_flag) cout << "\n Writing LESPremixed2D multi-block mesh to grid data output file.";
	error_flag = Output_Tecplot(MeshBlk,
				    Input_Parameters);
	if (error_flag) {
	  cout << "\n LESPremixed2D ERROR: Unable to open LESPremixed2D mesh data output file.\n";
	  cout.flush();
	} /* endif */
      } /* endif */
      CFFC_Broadcast_MPI(&error_flag, 1);
     ...
     *************************************************************************/
      ...
      if (CFFC_Primary_MPI_Processor()) {
	if (!batch_flag) cout << "\n Writing LESPremixed2D multi-block mesh to grid definition files.";
	error_flag = Write_Multi_Block_Grid_Definition(MeshBlk,
						       Input_Parameters);
	error_flag = Write_Multi_Block_Grid(MeshBlk,
					    Input_Parameters);
	if (error_flag) {
	  cout << "\n LESPremixed2D ERROR: Unable to open LESPremixed2D multi-block mesh definition files.\n";
	  cout.flush();
	} /* endif */
      } /* endif */
      CFFC_Broadcast_MPI(&error_flag, 1);
     ...
     *************************************************************************/
      ...
      if (CFFC_Primary_MPI_Processor()) {
	if (!batch_flag) cout << "\n Writing LESPremixed2D multi-block mesh to node data output file.";
	error_flag = Output_Nodes_Tecplot(MeshBlk,
					  Input_Parameters);
	if (error_flag) {
	  cout << "\n LESPremixed2D ERROR: Unable to open LESPremixed2D mesh node data output file.\n";
	  cout.flush();
	} /* endif */
      } /* endif */
      CFFC_Broadcast_MPI(&error_flag, 1);
     ...
     *************************************************************************/
      ...
      if (CFFC_Primary_MPI_Processor()) {
	if (!batch_flag) cout << "\n Writing LESPremixed2D multi-block mesh to cell data output file.";
	error_flag = Output_Cells_Tecplot(MeshBlk,
					  Input_Parameters);
	if (error_flag) {
	  cout << "\n LESPremixed2D ERROR: Unable to open LESPremixed2D mesh cell data output file.\n";
	  cout.flush();
	} /* endif */
      } /* endif */
      CFFC_Broadcast_MPI(&error_flag, 1);
     ...
     *************************************************************************/
      ...
      error_flag = Output_Ringleb(Local_SolnBlk,
				  ...
				  Input_Parameters);
      if (error_flag) {
	cout << endl << "\n LESPremixed2D ERROR: Unable to open LESPremixed2D Ringleb's flow output file." << endl;
	cout.flush();
      }
      CFFC_Broadcast_MPI(&error_flag,1);
      if (error_flag) return error_flag;
    ...
    *************************************************************************/ 
      ...
      error_flag = Output_Viscous_Channel(Local_SolnBlk,
					  ...
					  Input_Parameters);
      if (error_flag) {
	cout << endl << "\n LESPremixed2D ERROR: Unable to open LESPremixed2D viscous channel flow output file." << endl;
	cout.flush();
      }
      CFFC_Broadcast_MPI(&error_flag,1);
      if (error_flag) return error_flag;
     ...
     *************************************************************************/ 
      ...
      error_flag = Output_Flat_Plate(Local_SolnBlk,
				     ...
				     Input_Parameters);
      if (error_flag) {
	cout << endl << "\n LESPremixed2D ERROR: Unable to open LESPremixed2D flat plate output file." << endl;
	cout.flush();
      }
      CFFC_Broadcast_MPI(&error_flag,1);
      if (error_flag) return error_flag;
     ...
     *************************************************************************/ 
      ...
      error_flag = Output_Driven_Cavity_Flow(Local_SolnBlk,
					     ...
					     Input_Parameters);
      if (error_flag) {
	cout << endl << "\n LESPremixed2D ERROR: Unable to open LESPremixed2D driven cavity flow." << endl;
	cout.flush();
      }
      CFFC_Broadcast_MPI(&error_flag,1);
      if (error_flag) return error_flag;
     ...
     *************************************************************************/ 
      ...
      // Output solution data.
      if (!batch_flag) cout << "\n Writing LESPremixed2D quasi3D solution to output data file(s).";
     ...
     *************************************************************************/ 
      ...
      if (error_flag) {
	cout << endl << "\n LESPremixed2D ERROR: Problem in LESPremixed2D SWITCH_BCS_TO_FIXED. " << endl;
	cout.flush();
      }
      CFFC_Broadcast_MPI(&error_flag,1);
      if (error_flag) return error_flag;      
    ...
    /*************************************************************************
     ****************  POSTPROCESS 1D FLAME   ********************************
     *************************************************************************/
    else if (command_flag == POSTPROCESS_1DFLAME) {
      // Output the species masss.
      if (!batch_flag) cout << "\n\n Species mass per unit length:";
      double species_mass, burning_rate;
      for (int k=0; k < Input_Parameters.Wo.ns; ++k) { 
	species_mass = Total_Species_Mass(Local_SolnBlk, 
					  List_of_Local_Solution_Blocks, k);
      }
      burning_rate = Turbulent_Burning_Rate(Local_SolnBlk, 
					    List_of_Local_Solution_Blocks,
					    Input_Parameters);
      if (CFFC_Primary_MPI_Processor()) cout << "\n\n Burning rate based on fuel = " 
					       << burning_rate;
      // CFFC_Broadcast_MPI(&error_flag, 1);
//       if (error_flag) return (error_flag);
    }
     ...
     *************************************************************************/
    else if (command_flag == POSTPROCESS_TURBULENCE) {
      // Postprocess turbulence data.
      if (!batch_flag) cout << "\n Postprocessing LESPremixed2D turbulence data.";
...
//             Input_Parameters.i_ICs == IC_HOMOGENEOUS_TURBULENCE) &&
//             Input_Parameters.i_Grid != GRID_1DFLAME) {
          double u_ave, v_ave, sqr_u; 

          // Average viscosity for non-reacting turbulent flow
          viscosity = Average_viscosity(Local_SolnBlk, List_of_Local_Solution_Blocks,
                                       Input_Parameters);
         
          Average_V(Local_SolnBlk, List_of_Local_Solution_Blocks, Input_Parameters, 
                   u_ave, v_ave);
...
//                                                 List_of_Global_Solution_Blocks,  
//                                                 List_of_Local_Solution_Blocks,
//                                                 Local_SolnBlk, Input_Parameters, 
//                                                 u_ave, sqr_u);
//           }
 
          if (error_flag) {
	    cout << "\n LESPremixed2D ERROR: Longitudinal Correlation on processor "
		 << List_of_Local_Solution_Blocks.ThisCPU
		 << ".\n";
	    cout.flush();
	  } /* endif */
          ...
          // Write the solution state variables of a wrinkled flame
          // if (Input_Parameters.i_ICs == IC_LESPREMIXED_2DFLAME ||
//               (Input_Parameters.num_species > 2  &&
//               Input_Parameters.react_name == "NO_REACTIONS")) {
// 	    cout <<"\n Write_Wrinkled_Flame_"<< Input_Parameters.react_name 
// 		 <<"--"<< Input_Parameters.ICs_Type << endl;
//             Write_Wrinkled_Flame(Local_SolnBlk,
// 				 List_of_Local_Solution_Blocks,
// 				 Input_Parameters);
// 	  }
          ...
          // Write the fluctuations of the turbulent velocity field
          // Write_Turbulent_Solution(Local_SolnBlk,
//                                    List_of_Local_Solution_Blocks,
//                                    Input_Parameters);

          // if (CFFC_Primary_MPI_Processor()) {
	    // Compute_Spectrum(Local_SolnBlk, List_of_Local_Solution_Blocks, 
//                             Input_Parameters, u_ave, v_ave, viscosity);
	    
            if (last_step == 1 && Input_Parameters.Rescale_Spectrum == 1  && 
		(Input_Parameters.i_ICs == IC_LESPREMIXED_2DFLAME  || 
		 (Input_Parameters.num_species > 2  && Input_Parameters.react_name == "NO_REACTIONS"))) {
	      cout <<"\n Rescale_Velocity_"<< Input_Parameters.react_name 
		   <<"--"<< Input_Parameters.ICs_Type << endl;
              Rescale_Velocity(Local_SolnBlk, List_of_Local_Solution_Blocks,
			       Input_Parameters, u_ave, v_ave, Input_Parameters.TKEo);

              // Rescale_Spectrum(Input_Parameters, u_ave, v_ave, viscosity);
	    }
	 //  }  
	    //}

      if (error_flag) {
	cout << "\n LESPremixed2D ERROR: Unable to postprocess LESPremixed2D turbulence data "
	     << "on processor "
	     << List_of_Local_Solution_Blocks.ThisCPU
	     << ".\n";
	cout.flush();
  ...
  /********************************************************  
   * End of all LESPremixed2DSolver computations and I/O. *
   ********************************************************/    
  //should use terminate code ie. should never get here?
  cout<<"\nEND OF LESPREMIXED2DSOLVER BUT DIDN'T USE TERMINATE!!\n"; cout.flush();
  
  return (0);
  
} //end LESPremixed2DSolver
