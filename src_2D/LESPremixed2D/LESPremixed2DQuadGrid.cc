/* LESPremixed2DQuadGrid.cc:  Grid Manipulation Subroutines for 2D LES 
                              Premixed Multi-Block Quadrilateral Mesh 
                              Solution Classes. */
...
Grid2D_Quad_Block** Multi_Block_Grid(Grid2D_Quad_Block **Grid_ptr,
    ...
    switch(Input_Parameters.i_Grid) {
      case GRID_READ_FROM_DEFINITION_FILE :
        ...
        if (Grid_ptr == NULL) {
	   cout << "\n " << CFFC_Name() 
                << " LESPremixed2D ERROR: Unable to open multi-block mesh definition file "
      ...
      case GRID_READ_FROM_GRID_DATA_FILE :
        ...
        if (Grid_ptr == NULL) {
	   cout << "\n " << CFFC_Name() 
                << " LESPremixed2D ERROR: Unable to open multi-block mesh data file "
    ...
    case GRID_LAMINAR_FLAME :
      ...
      break;
        ...
        //************ FOR 2D Homogeneous Decaying Turbulence **************//
	...
	for (iBlk = 0; iBlk <= Input_Parameters.Number_of_Blocks_Idir-1; ++iBlk ) {
	  // North 
	  if (jBlk == Input_Parameters.Number_of_Blocks_Jdir-1) {
	    Grid_ptr[iBlk][jBlk].BndNorthSpline.setBCtype(BC_FREE_SLIP);
	  } else {
	    Grid_ptr[iBlk][jBlk].BndNorthSpline.setBCtype(BC_NONE);
	  } 
	  // South
	  if (jBlk == 0) {
	    Grid_ptr[iBlk][jBlk].BndSouthSpline.setBCtype(BC_FREE_SLIP);
	  } else {
	    Grid_ptr[iBlk][jBlk].BndSouthSpline.setBCtype(BC_NONE);
	  } 
	  // East
	...
	for (iBlk = 0; iBlk <= Input_Parameters.Number_of_Blocks_Idir-1; ++iBlk ) {
      ...
      break;
           ...
           //***************  FOR 2D Turbulent Premixed Flame  *****************//
    case GRID_2DTURBULENT_PREMIXED_FLAME :

     //  Grid_ptr = Read_Multi_Block_Grid_Definition(Grid_ptr, Input_Parameters);
//         if (Grid_ptr == NULL) {
// 	   cout << "\n " << CFDkit_Name() 
//                 << " LESPremixed2D ERROR: Unable to open multi-block mesh definition file "
//                 << Input_Parameters.Grid_Definition_File_Name << ".\n";
//         } /* endif */
 ...
 //      Grid_ptr = Read_Multi_Block_Grid(Grid_ptr, Input_Parameters);
//         if (Grid_ptr == NULL) {
// 	   cout << "\n " << CFDkit_Name() 
//                 << " LESPremixed2D ERROR: Unable to open multi-block mesh data file "
//                 << Input_Parameters.Grid_File_Name << ".\n";
