...
double PowerLaw::efficiency_function(const double &lam_speed,
  ...
  if (u_p == 0.0){
    ...
    fu = FOUR*sqrt(27.0*CK/110.0)*(18.0*CK/55.0)*sqr(u_p/lam_speed);
    fRe = sqrt((9.0/55.0) * exp(-3.0*CK*pow(PI, 4.0/3.0)/(2.0*sfs_Re))) * sqrt(sfs_Re);
  }

  fdelta = sqrt((27.0*CK*pow(PI,4.0/3.0)/110.0)*(pow(Delta/lam_thickness, 4.0/3.0) - 1.0));
...
void PowerLaw::wrinkling_factor(const double &lam_speed,
  ...
  WF = pow(1.0 + min(filter_size(lam_thickness)/lam_thickness, filter_size(lam_thickness)*mean_c), beta);  
}  
