/*******************************************************************
 *******************************************************************
 **********       LESPremixed2D - Built using CFFC         *********
 ******                                                       ******   
 ****                                                           ****
 ...
 ******************************************************************* 
 ...
 This program is a standalone version of the LESPremixed2D Solver that 
 is part of the CFFC library of 2D solvers.  It currently can handle
 planar and axisymmetric multi-species thermally perfect premixed 
 reacting flows.
...
int main(int num_arg, char *arg_ptr[]) {
  ...
  char *program_title_ptr = 
     "LESPremixed2D: Stand alone Solver for Premixed Reacting Flows.";
  ...
  // Input file name:
  char *Input_File_Name_ptr = "lespremixed2D.in";

  // Equation type indicator:
  char *Equation_Type_ptr = "LESPremixed2D";
  ...
  /* Parse and interpret command line arguments.  Note that there
     ...
     6) -f name  uses "name" as the input data file rather than
                 the standard input data file "lespremixed2D.in". */
...
//         strcpy(Equation_Type, Equation_Type_ptr);
  ...
  if (error_flag) {
    cout << "\n LESPremixed2D ERROR: Invalid command line argument.\n";
    return (error_flag);
  } /* endif */
  ...
  if (CFFC_Primary_MPI_Processor() && help_flag) {
     cout << "Usage:\n";
     cout << "lespremixed2D [-v] [-h] [-i] [-b] [-f name] [-t name number]\n";
     ...
     cout << " -f name  use `name' input data file (`lespremixed2D.in' is default)\n";
     cout << " -t              run all available test suites\n" 
	  ...
	  << "                 example: lespremixed2D -t MyTestSuit 3\n";
     cout.flush();
  } /* endif */
  if (help_flag) {
     CFFC_Finalize_MPI();
  ...
  if (CFFC_Primary_MPI_Processor() && !batch_flag) 
     cout << "\n\nLESPremixed2D: Execution complete.\n";
  return (0);

/* End LESPremixed2D program. */
