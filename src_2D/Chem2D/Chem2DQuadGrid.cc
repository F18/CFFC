/* Chem2DQuadGrid.cc:  Grid Manipulation Subroutines for 2D Euler 
...
Grid2D_Quad_Block** Multi_Block_Grid(Grid2D_Quad_Block **Grid_ptr,
    ...
    switch(Input_Parameters.i_Grid) {
      case GRID_READ_FROM_DEFINITION_FILE :
        ...
        if (Grid_ptr == NULL) {
	   cout << "\n " << CFFC_Name() 
                << " Chem2D ERROR: Unable to open multi-block mesh definition file "
      ...
      case GRID_READ_FROM_GRID_DATA_FILE :
        ...
        if (Grid_ptr == NULL) {
	   cout << "\n " << CFFC_Name() 
                << " Chem2D ERROR: Unable to open multi-block mesh data file "
