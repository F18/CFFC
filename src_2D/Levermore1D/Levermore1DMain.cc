 ...
 *******************************************************************
 ...
 This program is a standalone version of the Levermore1D Solver
...
int main(int num_arg, char *arg_ptr[]) {
  ...
  char *program_title_ptr =
     "Levermore1D: Stand alone Solver for the Levermore equations in 1 space dimension.";
  ...
  // Input file name:
  char *Input_File_Name_ptr = "levermore1D.in";

  // Equation type indicator:
  char *Equation_Type_ptr = "Levermore1D";
  ...
  /* Parse and interpret command line arguments.  Note that there
     ...
     5) -f name  uses "name" as the input data file rather than
                 the standard input data file "levermore1D.in". */
  ...
  if (error_flag) {
    cout << "\n Levermore1D ERROR: Invalid command line argument.\n";
    return (error_flag);
  } /* endif */
  ...
  if ( help_flag) {
     cout << "Usage:\n";
     cout << "levermore1D [-v] [-h] [-i] [-b] [-f name] [-t name number]\n";
     ...
     cout << " -f name         use `name' input data file (`levermore1D.in' is default)\n";
     cout << " -t              run all available test suites\n"
	  ...
	  << "                 example: levermore1D -t MyTestSuit 3\n"
  ...
  if (!batch_flag)
     cout << "\n\nLevermore1D: Execution complete.\n";

  //Ending properly
  return (0);

/* End Levermore1D program. */
