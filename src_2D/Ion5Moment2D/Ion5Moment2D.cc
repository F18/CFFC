 ...
 ******************************************************************* 
 ...
 This program is a standalone version of the Ion5Moment2D Solver that 
...
int main(int num_arg, char *arg_ptr[]) {
  ...
  char *program_title_ptr = 
     "Ion5Moment2D: Stand alone Solver for Ion Transport Modelling in 2 space dimensions.";
  ...
  // Input file name:
  char *Input_File_Name_ptr = "ion5moment2D.in";

  // Equation type indicator:
  char *Equation_Type_ptr = "Ion5Moment2D";
  ...
  /* Parse and interpret command line arguments.  Note that there
     ...
     6) -f name  uses "name" as the input data file rather than
                 the standard input data file "ion5moment2D.in". */
...
//         strcpy(Equation_Type, Equation_Type_ptr);
  ...
  if (error_flag) {
    cout << "\n Ion5Moment2D ERROR: Invalid command line argument.\n";
    return (error_flag);
  } /* endif */
  ...
  if (CFFC_Primary_MPI_Processor() && help_flag) {
     cout << "Usage:\n";
     cout << "ion5moment2D [-v] [-h] [-i] [-b] [-f name] [-t name number]\n";
     ...
     cout << " -f name  use `name' input data file (`ion5moment2D.in' is default)\n";
     cout << " -t              run all available test suites\n" 
	  ...
	  << "                 example: ion5moment2D -t MyTestSuit 3\n";
     cout.flush();
  } /* endif */
  if (help_flag) {
     CFFC_Finalize_MPI();
