/* MHD2DInput.h:  Header file defining 
                  2D MHD Input Parameter Class. */

#ifndef _MHD2D_INPUT_INCLUDED
#define _MHD2D_INPUT_INCLUDED
...
#include "MHD3DState.h"   // Include 2D MHD solution state header file
...
#define	INPUT_PARAMETER_LENGTH_MHD2D    128

// Enviroment flag for CFFC root directory path
#define PATHVAR_MHD2D "CFFC_Path"

/*!
 * Class: MHD2D_Input_Parameters
 *
 * @brief Definition and manipulation of 2D MHD input variables.
 *
 */
class MHD2D_Input_Parameters{
  private:
  ...
  char CFFC_Path[INPUT_PARAMETER_LENGTH_MHD2D];
  //! Input file name:
  char Input_File_Name[INPUT_PARAMETER_LENGTH_MHD2D];
  //! Input file stream:
  ifstream Input_File;
  ...
  char Time_Integration_Type[INPUT_PARAMETER_LENGTH_MHD2D];
  ...
  char Reconstruction_Type[INPUT_PARAMETER_LENGTH_MHD2D]; /*!< Spatial reconstruction type. */
  ...
  char Limiter_Type[INPUT_PARAMETER_LENGTH_MHD2D];
  int i_Limiter;
  int  Freeze_Limiter;
  ...
  char Flux_Function_Type[INPUT_PARAMETER_LENGTH_MHD2D];
  int i_Flux_Function;
  //@}

  //@{ @name Initial condition type indicator and related input parameters:
  char ICs_Type[INPUT_PARAMETER_LENGTH_MHD2D];
  char Gas_Type[INPUT_PARAMETER_LENGTH_MHD2D];
  int i_ICs;
  MHD3D_pState Wo, W1, W2;
  MHD3D_cState Uo;
  double Pressure, Temperature, Mach_Number, Flow_Angle;
  Vector2D Wave_Position;
  double Wave_Width;
  MHD2D_ExactSolutions *ExactSoln; /*!< Pointer to the exact solution */
  MHD3D_pState RefW;		     /*!< Reference state, used by CENO to normalize the
  ...
  char Flow_Geometry_Type[INPUT_PARAMETER_LENGTH_MHD2D];
  int Axisymmetric;
  //@}

  //@{ @name Grid type indicator and related input parameters:
  char Grid_Type[INPUT_PARAMETER_LENGTH_MHD2D];
  char NACA_Aerofoil_Type[INPUT_PARAMETER_LENGTH_MHD2D];
  int i_Grid;
  ...
  char Boundary_Conditions_Specified[INPUT_PARAMETER_LENGTH_MHD2D];
  int BCs_Specified;
  char BC_North_Type[INPUT_PARAMETER_LENGTH_MHD2D];
  char BC_South_Type[INPUT_PARAMETER_LENGTH_MHD2D];
  char BC_East_Type[INPUT_PARAMETER_LENGTH_MHD2D];
  char BC_West_Type[INPUT_PARAMETER_LENGTH_MHD2D];
  ...
  //! Output file name:
  char Output_File_Name[INPUT_PARAMETER_LENGTH_MHD2D];
  //! Multi-block mesh definition input file names:
  char Grid_File_Name[INPUT_PARAMETER_LENGTH_MHD2D];
  char Grid_Definition_File_Name[INPUT_PARAMETER_LENGTH_MHD2D];
  //! Restart file name:
  char Restart_File_Name[INPUT_PARAMETER_LENGTH_MHD2D];
  //! Gnuplot file name:
  char Gnuplot_File_Name[INPUT_PARAMETER_LENGTH_MHD2D];
  //! Next_Control_Parameter:
  char Next_Control_Parameter[INPUT_PARAMETER_LENGTH_MHD2D];
  //! Output format type indicator:
  char Output_Format_Type[INPUT_PARAMETER_LENGTH_MHD2D];
  int i_Output_Format;
  ...
  friend ostream &operator << (ostream &out_file, const MHD2D_Input_Parameters &IP);
  friend istream &operator >> (istream &in_file, MHD2D_Input_Parameters &IP);
  //@}

};

/********************************************************
 * MHD2D_Input_Parameters::MHD2D_Input_Parameters() *
 * -->  Default Constructor                             *
 *******************************************************/
inline MHD2D_Input_Parameters::MHD2D_Input_Parameters(void): verbose_flag(OFF){
 ...
 * -->  Default Destructor                               *
 ********************************************************/
inline MHD2D_Input_Parameters::~MHD2D_Input_Parameters(void){
...
/*************************************************************
 * MHD2D_Input_Parameters::get_cffc_path -- Get CFFC path. *
 *************************************************************/
inline void MHD2D_Input_Parameters::get_cffc_path(){
  char *string_ptr;
  // Check to see if environment varible exists.
  if (getenv(PATHVAR_MHD2D) == NULL) {
    //Set default path
     string_ptr = "CFFC";
     strcpy(CFFC_Path, string_ptr);
  } else {
     //Set path specified by environment variable
     strcpy(CFFC_Path, getenv(PATHVAR_MHD2D));
  }
}

/*************************************************************
 * MHD2D_Input_Parameters -- Input-output operators.       *
 *************************************************************/
inline ostream &operator << (ostream &out_file,
    ...
    if (IP.i_Grid == GRID_CARTESIAN_UNIFORM) {
       out_file << "\n\n Solving 2D MHD equations (IBVP/BVP) on uniform Cartesian mesh.";
    } else {
       out_file << "\n\n Solving 2D MHD equations (IBVP/BVP) on multi-block solution-adaptive quadrilateral mesh.";
    } /* endif */
    out_file << "\n  -> Input File Name: " 
             << IP.Input_File_Name;
...
extern void Open_Input_File(MHD2D_Input_Parameters &IP);

extern void Close_Input_File(MHD2D_Input_Parameters &IP);

extern void Set_Default_Input_Parameters(MHD2D_Input_Parameters &IP);

extern void Broadcast_Input_Parameters(MHD2D_Input_Parameters &IP);
...
extern void Get_Next_Input_Control_Parameter(MHD2D_Input_Parameters &IP);

extern int Parse_Next_Input_Control_Parameter(MHD2D_Input_Parameters &IP);

extern int Process_Input_Control_Parameter_File(MHD2D_Input_Parameters &Input_Parameters,
                                                char *Input_File_Name_ptr,
                                                int &Command_Flag);

#endif /* _MHD2D_INPUT_INCLUDED  */
