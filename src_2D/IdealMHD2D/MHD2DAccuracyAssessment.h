/*!\file MHD2DAccuracyAssessment.h
  \brief Header file defining specializations of 2D accuracy assessment class for MHD solution. */

#ifndef _MHD2D_ACCURACY_ASSESSMENT_2D_INCLUDED
#define _MHD2D_ACCURACY_ASSESSMENT_2D_INCLUDED
 ...
 * \param AccessToHighOrderVars MHD2D_Quad_Block member function which returns the 
 ...
 */
template<> inline
void AccuracyAssessment2D<MHD2D_Quad_Block>::
  ...
  if (SolnBlk->ExactSolution()->IsExactSolutionSet()){  // exact solution is set
    ...
    SolnBlk->HighOrderVariable(Pos).
      ComputeSolutionErrors(wrapped_member_function_one_parameter(SolnBlk->ExactSolution(),
								  &MHD2D_Quad_Block::Exact_Solution_Type::SolutionForParameter,
								  parameter,
								  _dummy_param),
			    parameter,
			    accuracy_digits);
 ...
 *
 * \param parameter the state variable which is used for computing the errors
 * \param ComputeSolutionAt MHD2D_Quad_Block member function which returns the solution 
...
ComputeSolutionErrors(const unsigned int &parameter,
		      const unsigned int &accuracy_digits,
		      double (MHD2D_Quad_Block::*ComputeSolutionAt)(const int &, const int &,
								      const Vector2D &,
...
#endif	// _MHD2D_ACCURACY_ASSESSMENT_2D_INCLUDED
