/*******************************************************************
 *******************************************************************
 **********         Ideal MHD2D - Built using CFFC         *********
 ******                                                       ******   
 ****                                                           ****
 ******       UTIAS, CFD & Propulsion Group, 1999-2009        ******
 **********                                                *********
 *******************************************************************
 ******************************************************************* 

 This program is a standalone version of the ideal MHD2D solver that 
 is part of the CFFC library of 2D solvers.
...
int main(int num_arg, char *arg_ptr[]) {
  ...
  char *program_title_ptr = 
     "MHD2D: Stand alone solver for the ideal MHD Equations in 2 space dimensions.";

  // Version of code:
  char *program_version_ptr = 
     "Version 1.00, UTIAS CFD & Propulsion Group, 1999-2009.";
  
  // Input file name:
  char *Input_File_Name_ptr = "mhd2D.in";

  // Equation type indicator:
  char *Equation_Type_ptr = "MHD2D";
  ...
  /* Parse and interpret command line arguments.  Note that there
     ...
     5) -pde type   sets type of partial differential equation
                    to be solve to "type" (default is "MHD2D"),
     6) -f name  uses "name" as the input data file rather than
                 the standard input data file "mhd2D.in". */
  ...
  if (error_flag) {
    cout << "\nMHD2D ERROR: Invalid command line argument.\n";
    return (error_flag);
  } /* endif */
  ...
  if (CFFC_Primary_MPI_Processor() && help_flag) {
     cout << "Usage:\n";
     cout << "mhd2D [-v] [-h] [-i] [-b] [-f name] [-t name number]\n";
     ...
     cout << " -f name  use `name' input data file (`mhd2D.in' is default)\n";
  ...
  //   error_flag = MHD2DQuadSolver(Input_File_Name_ptr,
  // 			       batch_flag);
