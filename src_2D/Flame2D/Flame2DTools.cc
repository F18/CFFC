 ...
 *                                                                    *
 **********************************************************************/
void Output_Ringleb_Solution(Flame2D_Quad_Block &SolnBlk,
			     const int Block_Number,
			     const int Output_Title,
			     ostream &Out_File) {

  Flame2D_pState We, Ww;
 ...
 *                                                                    *
 **********************************************************************/
void Output_Ringleb_Error(Flame2D_Quad_Block &SolnBlk,
			  double &l1_norm,
			  double &l2_norm,
			  double &max_norm,
...
//    // Allocate memory.
//    w = new double[3];
//    f = new Flame2D_pState*[3];
//    epsilon = new double[3];
//    eta = new double[3];
//    for (int i = 0; i < 3; i++) f[i] = new Flame2D_pState[3];
...
void Output_Viscous_Channel(Flame2D_Quad_Block &SolnBlk,
  ...
  Out_File << setprecision(14);
  if (Output_Title)
    Out_File << "TITLE = \"" << CFFC_Name() << ": 2D Flame2D Channel Flow Solution "
	     << "\"" << "\n"
	     << "VARIABLES = \"x\" \\ \n"
	     << "\"y\" \\ \n"
	     << "\"rho\" \\ \n"
...
void Output_Flat_Plate(Flame2D_Quad_Block &SolnBlk,
  ...
  if (Output_Title_Soln) {
    Out_File_Soln << "TITLE = \"" << CFFC_Name() << ": 2D Flame2D Solution, "
		  << "\"" << "\n"
		  << "VARIABLES = \"x\" \\ \n"
		  << "\"y\" \\ \n"
		  << "\"rho\" \\ \n"
...
//   if (Output_Title_Skin) {
      Out_File_Skin << "TITLE = \"" << CFFC_Name() << ": 2D Flame2D Solution, "
		    << "\"" << "\n"
		    << "VARIABLES = \"x\" \\ \n"
		    << "\"Rex\" \\ \n"
		    << "\"Cf\" \\ \n"
 ...
 *                                                      *
 ********************************************************/
void Output_Quasi3D_Tecplot(Flame2D_Quad_Block &SolnBlk,
 ...
 *                                                                    *
 **********************************************************************/
int Output_Quasi3D_Tecplot(Flame2D_Quad_Block *Soln_ptr,
...
int Set_Equilibrium_State(Flame2D_Quad_Block &SolnBlk) {
  ...
  for ( int j  = SolnBlk.JCl-SolnBlk.Nghost ; j <= SolnBlk.JCu+SolnBlk.Nghost ; ++j ) {
    for ( int i = SolnBlk.ICl-SolnBlk.Nghost ; i <= SolnBlk.ICu+SolnBlk.Nghost ; ++i ) {
      SolnBlk.W[i][j].React.ct_equilibrium_TP<Flame2D_pState>( SolnBlk.W[i][j] );
  ...
  //
  for ( int j  = SolnBlk.JCl-SolnBlk.Nghost ; j <= SolnBlk.JCu+SolnBlk.Nghost ; ++j ) {
    SolnBlk.WoW[j].React.ct_equilibrium_TP<Flame2D_pState>( SolnBlk.WoW[j] );
    SolnBlk.WoE[j].React.ct_equilibrium_TP<Flame2D_pState>( SolnBlk.WoE[j] );
  } // endfor
  ...
  //
  for ( int i = SolnBlk.ICl-SolnBlk.Nghost ; i <= SolnBlk.ICu+SolnBlk.Nghost ; ++i ) {
    SolnBlk.WoS[i].React.ct_equilibrium_TP<Flame2D_pState>( SolnBlk.WoS[i] );
    SolnBlk.WoN[i].React.ct_equilibrium_TP<Flame2D_pState>( SolnBlk.WoN[i] );
  } // endfor
