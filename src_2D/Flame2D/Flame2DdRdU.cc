...
void SemiImplicitBlockJacobi_dSdU(DenseMatrix &dSdU,
  ...
  if (SolnBlk.W[ii][jj].React.reactset_flag != NO_REACTIONS){    
    SolnBlk.W[ii][jj].dSwdU(dSdU, solver_type);
  }  
...
void dFIdW_Inviscid(DenseMatrix &dRdW, Flame2D_Quad_Block &SolnBlk, Flame2D_Input_Parameters 
 ...
 *                                                      *
 ********************************************************/
void dFIdW_Inviscid_HLLE(DenseMatrix &dRdW, Flame2D_Quad_Block &SolnBlk,
			 Flame2D_Input_Parameters &Input_Parameters, 
  ...
  int overlap = Input_Parameters.NKS_IP.GMRES_Overlap;
   ...
   DenseMatrix A( Rotation_Matrix2(nface,NUM_VAR_FLAME2D, 1));
   DenseMatrix AI( Rotation_Matrix2(nface,NUM_VAR_FLAME2D, 0));
   DenseMatrix II(NUM_VAR_FLAME2D, NUM_VAR_FLAME2D,ZERO); 
   II.identity();
   ...
   //Jacobian
   dFIdW(dFidW, Rotate(SolnBlk.Uo[ii][jj].W(), nface) );     
...
void dFIdW_Inviscid_ROE(DenseMatrix& dRdW, Flame2D_Quad_Block &SolnBlk,  
  ...
  if (ii < SolnBlk.ICl -overlap || ii > SolnBlk.ICu + overlap ||
    ...
    int NUM_VAR_FLAME2D = dRdW.get_n();  //  SolnBlk.NumVar()-1;    
    DenseMatrix dFidW(NUM_VAR_FLAME2D, NUM_VAR_FLAME2D,ZERO);
    
    Vector2D nface,DX; double lface;   
    Flame2D_pState Wa, wavespeeds, Left, Right, Wl, Wr;   
    Left.Vacuum();   Right.Vacuum();  Wl.Vacuum();  Wr.Vacuum();
     ...
     if(!Input_Parameters.Preconditioning){
       ...
       for (int i=1; i <= NUM_VAR_FLAME2D; i++) {		   
	 for(int irow =0; irow< NUM_VAR_FLAME2D; irow++){
	   for(int jcol =0; jcol< NUM_VAR_FLAME2D; jcol++){
       ...
       DenseMatrix Flux_dissipation(NUM_VAR_FLAME2D,NUM_VAR_FLAME2D,ZERO);        
       for (int i=1; i <=  NUM_VAR_FLAME2D; i++) {		   
	 for(int irow =0; irow < NUM_VAR_FLAME2D; irow++){
	   for(int jcol =0; jcol < NUM_VAR_FLAME2D; jcol++){	   
       ...
       DenseMatrix P( NUM_VAR_FLAME2D, NUM_VAR_FLAME2D,ZERO);          
 ...
 *                                                      *
 ********************************************************/
void dFIdW_Inviscid_ROE_FD(DenseMatrix& dRdW, Flame2D_Quad_Block &SolnBlk,  
			   Flame2D_Input_Parameters &Input_Parameters,
			   const int &ii, const int &jj, const int Orient){
  ...
  if (ii < SolnBlk.ICl -overlap || ii > SolnBlk.ICu + overlap ||
     ...
     int NUM_VAR_FLAME2D = dRdW.get_n();   
     DenseMatrix dFidW(NUM_VAR_FLAME2D, NUM_VAR_FLAME2D,ZERO);
     ...
     Flame2D_cState FluxA, FluxB; 
     Flame2D_pState WA, WB;
	 ...
	 WA[jcol+1] += a;
	 WA[NUM_VAR_FLAME2D+1] -= a;      
	 WB[jcol+1] -= a;
	 WB[NUM_VAR_FLAME2D+1] += a;
       }
...
int Inviscid_Flux_Used_Reconstructed_LeftandRight_States(Flame2D_pState &Wl, Flame2D_pState &Wr, 
                                                         Vector2D &DX, Flame2D_Quad_Block &SolnBlk, 
 ...
 * dF/dW_R                                              *
 ********************************************************/
void dFIdW_Inviscid_AUSM_plus_up(DenseMatrix& dRdW, Flame2D_Quad_Block &SolnBlk,  
				 Flame2D_Input_Parameters &Input_Parameters,
				 const int &ii, const int &jj, const int Orient){
  ...
  if (ii < SolnBlk.ICl -overlap || ii > SolnBlk.ICu + overlap ||
    ...
    int NUM_VAR_FLAME2D = dRdW.get_n();  //  SolnBlk.NumVar();    
    DenseMatrix dFidW(NUM_VAR_FLAME2D, NUM_VAR_FLAME2D,ZERO);
 ...
 *                                                      *
 ********************************************************/
void dGVdW_Viscous(DenseMatrix &dRdW, Flame2D_Quad_Block &SolnBlk, 
		   Flame2D_Input_Parameters &Input_Parameters,
		   const int &ii, const int &jj){
   ...
   DenseMatrix dFvdWf(NUM_VAR_FLAME2D, 14+(ns),ZERO);
   DenseMatrix dWfdWx(14+(ns), NUM_VAR_FLAME2D,ZERO);  
   DenseMatrix dGvdWf(NUM_VAR_FLAME2D, 14+(ns),ZERO);
   DenseMatrix dWfdWy(14+(ns), NUM_VAR_FLAME2D,ZERO);
...
void dFvdWf_Diamond(DenseMatrix &dFvdWf, DenseMatrix &dGvdWf, 
   ...
   Flame2D_pState QuadraturePoint_W;
 ...
 *                                                      *
 ********************************************************/
void dWfdWc_Diamond(DenseMatrix &dWfdWc_x,DenseMatrix &dWfdWc_y, Flame2D_Quad_Block &SolnBlk,
 ...
 *                                                      *
 ********************************************************/
void d_dWd_dW_Diamond(double &d_dWdx_dW, double &d_dWdy_dW, Flame2D_Quad_Block &SolnBlk, 
