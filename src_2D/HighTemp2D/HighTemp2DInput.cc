/**********************************************************************
 * HighTem2DInput.cc: Subroutines for the 2D High-Temp input class.   *
 *                                                                    *
 **********************************************************************/
 ...
 *                                                                    *
 **********************************************************************/
void Open_Input_File(HighTemp2D_Input_Parameters &IP) {
 ...
 *                                                                    *
 **********************************************************************/
void Close_Input_File(HighTemp2D_Input_Parameters &IP) {
 ...
 *                                                                    *
 **********************************************************************/
void Set_Default_Input_Parameters(HighTemp2D_Input_Parameters &IP) {
  ...
  */
  ...
  strcpy(IP.Turbulent_BC_Type,string_ptr);
  ...
  // IP.Nozzle_Downstream_Indicator = 0;
  ...
  // string_ptr = "Linear";
  // strcpy(IP.SubCell_Reconstruction,string_ptr);
  // IP.i_SubCell_Reconstruction = SUBCELL_RECONSTRUCTION_LINEAR;
...
void Broadcast_Input_Parameters(HighTemp2D_Input_Parameters &IP) {
  ...
  MPI::COMM_WORLD.Bcast(&(IP.i_Viscous_Reconstruction),
			...
			MPI::INT,0);
  // Turbulence parameters:
  MPI::COMM_WORLD.Bcast(IP.Turbulent_BC_Type,
			INPUT_PARAMETER_LENGTH_HIGHTEMP2D,
			MPI::CHAR,0);
  MPI::COMM_WORLD.Bcast(&(IP.i_Turbulent_BCs),
			1,
			MPI::INT,0);
  ...
  MPI::COMM_WORLD.Bcast(&(IP.Plate_Length),
			1,
			MPI::DOUBLE,0);
  MPI::COMM_WORLD.Bcast(&(IP.Grid_Height),
			1,
			MPI::DOUBLE,0);
  MPI::COMM_WORLD.Bcast(&(IP.Grid_Width),
			1,
			MPI::DOUBLE,0);
  ...
  MPI::COMM_WORLD.Bcast(&(IP.i_Smooth_Quad_Block),
			...
			MPI::INT,0);
  // Sub-cell reconstruction indicator:
  // MPI::COMM_WORLD.Bcast(IP.SubCell_Reconstruction,
  //		INPUT_PARAMETER_LENGTH_HIGHTEMP2D,
  //		MPI::CHAR,0);
...
void Broadcast_Input_Parameters(HighTemp2D_Input_Parameters &IP,
  ...
  Communicator.Bcast(&(IP.i_Viscous_Reconstruction),
		     ...
		     MPI::INT,Source_Rank);
  // Turbulence parameters:
  Communicator.Bcast(IP.Turbulent_BC_Type,
		     INPUT_PARAMETER_LENGTH_HIGHTEMP2D,
		     MPI::CHAR,Source_Rank);
  Communicator.Bcast(&(IP.i_Turbulent_BCs),
		     1,
		     MPI::INT,Source_Rank);
  ...
  Communicator.Bcast(&(IP.Plate_Length),
		     1,
		     MPI::DOUBLE,Source_Rank);
  Communicator.Bcast(&(IP.Grid_Height),
		     1,
		     MPI::DOUBLE,Source_Rank);
  Communicator.Bcast(&(IP.Grid_Width),
		     1,
		     MPI::DOUBLE,Source_Rank);
  ...
  Communicator.Bcast(&(IP.i_Smooth_Quad_Block),
		     ...
		     MPI::INT,Source_Rank);
  // Sub-cell reconstruction indicator:
  // Communicator.Bcast(IP.SubCell_Reconstruction,
  //	     INPUT_PARAMETER_LENGTH_HIGHTEMP2D,
  //	     MPI::CHAR,Source_Rank);
  // Communicator.Bcast(&(IP.i_SubCell_Reconstruction),
  //	     1,
  //	     MPI::INT,Source_Rank);
 ...
 *                                                                    *
 **********************************************************************/
void Get_Next_Input_Control_Parameter(HighTemp2D_Input_Parameters &IP) {

  int i;
  char buffer[INPUT_PARAMETER_LENGTH_HIGHTEMP2D];
 ...
 *                                                                    *
 **********************************************************************/
int Parse_Next_Input_Control_Parameter(HighTemp2D_Input_Parameters &IP) {
  ...
  if (strcmp(IP.Next_Control_Parameter,"Equation_Of_State_Type") == 0) {
    ...
    if (strcmp(IP.Equation_Of_State_Type,"Ideal_Gas") == 0){
      IP.EOSType = EOS_IDEAL;
   } else if (strcmp(IP.Equation_Of_State_Type,"Tgas") == 0) {
      IP.EOSType = EOS_TGAS;
   } else {
    ...
    if (strcmp(IP.ICs_Type,"Constant") == 0) {
      ...
      IP.i_ICs = IC_VISCOUS_FLAT_PLATE;
			// The initial conditions should not specify boundary conditions.
			// Especially considering that there is an isothermal flat plate.
    ...
    } else if (strcmp(IP.ICs_Type,"Forward_Facing_Step") == 0) {
      IP.i_ICs = IC_FORWARD_FACING_STEP;
    } else if (strcmp(IP.ICs_Type,"Blunt_Body") == 0) {
      IP.i_ICs = IC_BLUNT_BODY;
      ...
      // } else if (strcmp(IP.ICs_Type,"Rocket_Cold_Flow_Steady_State") == 0) {
      //  IP.i_ICs = IC_ROCKET_COLD_FLOW_STEADY_STATE;
      //} else if (strcmp(IP.ICs_Type,"Rocket_Cold_Flow_Unsteady") == 0) {
      // IP.i_ICs = IC_ROCKET_COLD_FLOW_UNSTEADY;
  ...
  } else if (strcmp(IP.Next_Control_Parameter, "Explicit_Relative_Tolerance") == 0) {
    i_command = 16;
    IP.Line_Number += 1;
    IP.Input_File >> IP.Explicit_Rel_Tol;
    IP.Input_File.getline(buffer,sizeof(buffer));
  ...
  } else if (strcmp(IP.Next_Control_Parameter,"Grid_Height") == 0) {
    i_command = 22;
    IP.Line_Number = IP.Line_Number + 1;
    IP.Input_File >> IP.Grid_Height;
    IP.Input_File.getline(buffer,sizeof(buffer));
    if (IP.Grid_Height <= ZERO) i_command = INVALID_INPUT_VALUE; 

  } else if (strcmp(IP.Next_Control_Parameter,"Grid_Width") == 0) {
    i_command = 23;
    IP.Line_Number = IP.Line_Number + 1;
    IP.Input_File >> IP.Grid_Width;
    IP.Input_File.getline(buffer,sizeof(buffer));
    if (IP.Grid_Width <= ZERO) i_command = INVALID_INPUT_VALUE; 
    ...
    */
    ...
    strcpy(IP.Output_Format_Type,IP.Next_Control_Parameter);
		for (unsigned ii = 0; ii < strlen(IP.Output_Format_Type); ii++) {
			IP.Output_Format_Type[ii] = tolower(IP.Output_Format_Type[ii]);
		}
  ...
  } else if (strcmp(IP.Next_Control_Parameter,"Explicit_Write_Output_Freq") == 0) {
    i_command = 51;
    IP.Line_Number++;
    IP.Input_File >> IP.Explicit_Write_Output_Freq;
    IP.Input_File.getline(buffer,sizeof(buffer));
    ...
    /*
  } else if (strcmp(IP.Next_Control_Parameter,"SubCell_Reconstruction") == 0) {
    i_command = 107;
    Get_Next_Input_Control_Parameter(IP);
    strcpy(IP.SubCell_Reconstruction,IP.Next_Control_Parameter);
    if (strcmp(IP.SubCell_Reconstruction,"Linear") == 0) {
      IP.i_SubCell_Reconstruction = SUBCELL_RECONSTRUCTION_LINEAR;
    } else if (strcmp(IP.SubCell_Reconstruction,"Quadratic") == 0) {
      IP.i_SubCell_Reconstruction = SUBCELL_RECONSTRUCTION_QUADRATIC;
    } else {
      i_command = INVALID_INPUT_VALUE;
    }
    */

  } else if (strcmp(IP.Next_Control_Parameter,"Turbulent_BC_Type") == 0) {
    i_command = 161;
    Get_Next_Input_Control_Parameter(IP);;
    strcpy(IP.Turbulent_BC_Type,IP.Next_Control_Parameter);
    if (strcmp(IP.Turbulent_BC_Type,"Direct_Integration") == 0) {
      IP.i_Turbulent_BCs = TURBULENT_BC_DIRECT_INTEGRATION;
    } else if (strcmp(IP.Turbulent_BC_Type,"Standard_Wall_Function") == 0) {
      IP.i_Turbulent_BCs = TURBULENT_BC_STANDARD_WALL_FUNCTION;
    } else if (strcmp(IP.Turbulent_BC_Type,"Two_Layer_Wall_Function") == 0) {
      IP.i_Turbulent_BCs = TURBULENT_BC_TWO_LAYER_WALL_FUNCTION;
    } else if (strcmp(IP.Turbulent_BC_Type,"Three_Layer_Wall_Function") == 0) {
      IP.i_Turbulent_BCs = TURBULENT_BC_THREE_LAYER_WALL_FUNCTION;
    } else if (strcmp(IP.Turbulent_BC_Type,"Automatic_Wall_Treatment") == 0) {
    ...
    if (strcmp(IP.Interface_IP.Type,"Circle") == 0) {
      ...
      //      IP.Interface_IP.Component_List[IP.Interface_IP.ci].Type = INTERFACE_NACA_AEROFOIL;
    ...
    IP.Input_File >> HTTOL;
    IP.Input_File.getline(buffer,sizeof(buffer));
    if (HTTOL < ZERO) i_command = INVALID_INPUT_VALUE;
  ...
  } else if (strcmp(IP.Next_Control_Parameter,"Write_Output_Forward_Facing_Step") == 0) {
    i_command = WRITE_OUTPUT_FORWARD_FACING_STEP_CODE;
 ...
 *                                                                    *
 **********************************************************************/
int Process_Input_Control_Parameter_File(HighTemp2D_Input_Parameters &IP,
  ...
  if (error_flag) {
    cout << "\n HighTemp2D ERROR: Unable to open HighTemp2D input data file.\n";
    return error_flag;
  }
  ...
  if (IP.i_Time_Integration == TIME_STEPPING_MULTIGRID) {
    error_flag = Check_Input_Parameters<HighTemp2D_Input_Parameters>(IP);
    if (error_flag) {
      cout << "\n HighTemp2D ERROR: Input Parameters consistency check failure\n";
      return error_flag;
    }
  }
 ...
 *                                                                    *
 **********************************************************************/
void Initialize_Reference_State(HighTemp2D_Input_Parameters &IP) {
  ...
  if (IP.i_Grid == GRID_FLAT_PLATE && (IP.FlowType == FLOWTYPE_LAMINAR ||
    ...
    */
    ...
    IP.Wo.rho = (IP.Reynolds_Number/IP.Plate_Length)*IP.Wo.mu()/(sqrt(IP.Wo.g*IP.Wo.R*IP.Temperature)*IP.Mach_Number);
    IP.Wo.p = IP.Wo.rho*IP.Wo.R*IP.Temperature;
    IP.Wo.v.x = sqrt(IP.Wo.g*IP.Wo.p/IP.Wo.rho)*IP.Mach_Number;
    //IP.Wo.v.x = sqrt(1.4*IP.Wo.p/IP.Wo.rho)*IP.Mach_Number;
    //-----------------
    IP.Wo.v.y = ZERO;
    IP.Uo = U(IP.Wo);
  ...
  */
  ...
  //Glaister 1988 IC's for Forward-Step and Blunt Body cases
  if (IP.i_Grid == GRID_FORWARD_FACING_STEP) {
      IP.Wo.rho = 1.715;
      IP.Wo.p = PRESSURE_STDATM;
      soundspeed = sqrt(1.4*IP.Wo.p/IP.Wo.rho);
      IP.Wo.v.x = IP.Mach_Number*soundspeed;
      IP.Wo.v.y = ZERO;
      IP.Wo.Mref = IP.Mach_Number;
      IP.Uo = U(IP.Wo);
    }
 ...
 *                                                                    *
 **********************************************************************/
void Reinitialize_Reference_State(HighTemp2D_Input_Parameters &IP) {
  ...
  */
  ...
  //Glaister 1988 IC's for front-step case
  if (IP.i_Grid == GRID_FORWARD_FACING_STEP) {
      IP.Wo.rho = 1.715;
      IP.Wo.p = PRESSURE_STDATM;
      soundspeed = sqrt(1.4*IP.Wo.p/IP.Wo.rho);
