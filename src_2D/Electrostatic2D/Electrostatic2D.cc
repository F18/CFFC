...
int main(int num_arg, char *arg_ptr[]) {
  ...
  char *program_title_ptr = 
     "Electrostatic2D: Numerical Solvers for the Electrostatic2D PDEs.";
  ...
  // Input file name:
  char *Input_File_Name_ptr = "electrostatic2D.in";

  // Equation type indicator:
  char *Equation_Type_ptr = "Electrostatic2D";
  ...
  //                to be solve to "type" (default is "Electrostatic2D"),
  // 6) -f name  uses "name" as the input data file rather than
  //             the standard input data file "electrostatic2D.in".
  ...
  // program as required.
  if (error_flag) {
    cout << "\n Electrostatic2D ERROR: Invalid command line argument.\n";
    return error_flag;
  }
  ...
  if (CFDkit_Primary_MPI_Processor() && help_flag) {
     cout << "Usage:\n";
     //cout << "electrostatic2D [-v] [-h] [-i] [-b] [-pde type] [-f name]\n";
     cout << "electrostatic2D [-v] [-h] [-i] [-b] [-f name]\n";
     ...
     cout << " -f name  use `name' input data file (`electrostatic2D.in' is default)\n";
     cout.flush();
     return 0;
  }
  ...
  error_flag = Electrostatic2DQuadSolver(Input_File_Name_ptr,batch_flag);
  ...
  if (CFDkit_Primary_MPI_Processor() && !batch_flag) 
    cout << "\n\nElectrostatic2D: Execution complete.\n";

  // Electrostatic2D program finished.
  return 0;
