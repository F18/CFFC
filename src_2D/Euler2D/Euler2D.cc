 ...
 ******************************************************************* 
 ...
 This program is a standalone version of the Euler2D solver that 
...
int main(int num_arg, char *arg_ptr[]) {
  ...
  char *program_title_ptr = 
     "Euler2D: Stand alone solver for the Euler Equations in 2 space dimensions.";
  ...
  // Input file name:
  char *Input_File_Name_ptr = "euler2D.in";
  ...
  if (error_flag) {
    cout << "\nEuler2D ERROR: Invalid command line argument.\n";
    return (error_flag);
  } /* endif */
  ...
  if (CFFC_Primary_MPI_Processor() && help_flag) {
     cout << "Usage:\n";
     cout << "euler2D [-v] [-h] [-i] [-b] [-f name] [-t name number]\n";
     ...
     cout << " -f name  use `name' input data file (`euler2D.in' is default)\n";
     cout << " -t              run all available test suites\n" 
	  ...
	  << "                 example: euler2D -t MyTestSuit 3\n"
