...
namespace tut {
  ...
  public:
    ...
    /* Relative path to the "/src" directory for outputing and inputing data */
    static const char* const Global_Output_Path;
    static const char* const Global_Input_Path;
    ...
    /* Check_Input_Output_Operator --> outputs the initial Obj to a file and then reads the file in order
                     ...
                     Prerequisites:  Class T must have defined the following operators:
                                       --> operator << (output)
                                       --> operator >> (input)
      NB: You should check before the output operator independently, to make sure that what you output
          is what you want!!!
    */
    template<class T>
  ...
  inline void TestData::open_output_stream(ofstream & output_stream, const char * output_stream_file_name){
    ...
    if (!output_stream.is_open()){
      string msg;
      msg = "\nUnable to open output file " + string(output_stream_file_name);
  ...
  inline void TestData::open_input_stream(ifstream & input_stream, const char * input_stream_file_name){
    ...
    if (!input_stream.is_open()){
      string msg;
      msg = "\nUnable to open input file " + string(input_stream_file_name);
      throw runtime_error(msg);
    }
  }
  ...
  inline void TestData::InitializeOutputFileName(void){
    if (Local_Output_Path != NULL){
      ...
      strcpy(output_file_name, Global_Output_Path);
    }
  }
  ...
  inline void TestData::InitializeInputFileName(void){
    if (Local_Input_Path != NULL){
      ...
      strcpy(input_file_name, Global_Input_Path);
    }
  }
