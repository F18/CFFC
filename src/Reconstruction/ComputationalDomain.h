...
void ComputationalDomain<SpaceDimension,GeometryType,SolutionType>::
  ...
  case OneD:
	...
	for(SGPoint=0; SGPoint<SolnPtr[0][0][cell].iSubgridPoints(); ++SGPoint){
	  output_file << SolnPtr[0][0][StencilCell].CellSubgridSolution(SGPoint).GetNode() << "\t"
		      << SolnPtr[0][0][cell].SolutionAtCoordinates(SolnPtr[0][0][StencilCell].CellSubgridSolution(SGPoint).GetNode())
		      << "\n";
	}
      }
    }
  ...
  void ComputationalDomain<SpaceDimension,GeometryType,SolutionType>::ReconstructSolution(const Reconstruct1D_Input_Parameters & IP)
{
  ...
  case DD_ENO:
    for (i=iStart(); i<=iEnd(); ++i){
      ...
      DataDependent_ENO_Reconstruction(*this,i_index,i);
    }
    break;
  ...
  void ComputationalDomain<SpaceDimension,GeometryType,SolutionType>::ReconstructSolution(const Reconstruct2D_Input_Parameters  & IP)
{
  ...
  case CENO:
	...
	/* Make Stencil */
	MakeReconstructionStencil(SolnPtr[0][0][0].CellRings(),i,j,i_index,j_index);
	MultipleCorrelationCoefficient(*this,i_index,j_index,StencilSize,i,j);
    }
  ...
  case DD_ENO:
    ...
    NumOfCellsInOneDirection = 2*SolnPtr[0][0][0].CellRings() + 1;
    StencilSize = NumOfCellsInOneDirection*NumOfCellsInOneDirection;
    i_index = new int [NumOfCellsInOneDirection*NumOfCellsInOneDirection];
    j_index = new int [NumOfCellsInOneDirection*NumOfCellsInOneDirection];
	...
	MakeReconstructionStencil(SolnPtr[0][0][0].CellRings(),i,j,i_index,j_index);
	/* Solve reconstruction for the current cell */
	DataDependent_ENO_Reconstruction(*this,i_index,j_index,StencilSize,i,j);
	/* Update the node solution */
	SolnPtr[0][j][i].UpdateSubgridSolution();
	/* Analyze order */
	if( SolnPtr[0][j][i].CellFOrder() < SolnPtr[0][0][0].CellRecOrder() ){
	  ++CellsWithSmallerOrder;
	}
	++FinishedCell;
	Print_Progress(FinishedCell,ProgressFrequency);
      }
    /* Print out how many cells didn't get the nominal reconstruction order */
    std::cout << "\n There are " << CellsWithSmallerOrder
	      << " cells that didn't get the nominal reconstruction order.\n";
    break;
  ...
  void ComputationalDomain<SpaceDimension,GeometryType,SolutionType>::ReconstructSolution(bool ChangeMe)
{
  ...
  case DD_ENO:
    ...
    k_index.reserve(CellsInOneDirection);
    for (int i=iStart(); i<=iEnd(); ++i)
      for (int j=jStart(); j<=jEnd(); ++j)
	for (int k=kStart(); k<=kEnd(); ++k){
	  /* Make Stencil */
	  ...
	  /* Solve reconstruction for the current cell */
	  //	  DataDependent_ENO_Reconstruction(*this,i_index,j_index,k_index,i,j,k);
	}
    break;
